    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse response)
        ...
        if (FROM.equals(DownloadConstants.DATA_SETS)) {
            ...
            if (TYPE.startsWith(DownloadConstants.DATA_FILE)) {
                Datafile file = null;
                Collection<Datafile> dfs = visit.getVisitData().getCurrentDatafiles();
                for (Datafile datafile : dfs) {
                    // log.trace(datafile.getId()  +" == "+ID);
                    if (datafile.getId().equals(new Long(ID))) {
                        file = datafile;
                    }
                }
                ...
                if (file == null) {
                    ...
                    detail = visit.getFacility() + ":" + file.getId();
                    name = file.getName();
            ...
            //this will get DATASETS and DATASETINFOLDER types
                ...
                name = visit.getVisitData().getCurrentDataset() + ".zip";
                ...
                ArrayList<String> urls = new ArrayList<String>();
                for (Datafile datafile : visit.getVisitData().getCurrentDatafiles()) {
                    if (datafile.isSelected() && datafile.getLocation() != null) {
                        urls.add(datafile.getLocation());
                        detail += visit.getFacility().split("-")[0] + ":" + datafile.getId() + "-";
                    }
                }
                if (urls.size() == 0) {
                    noneSelected = true;
        ...
        //check if any items are selected for download
        if(noneSelected){
            RequestDispatcher dispatcher =
                ...
                if (man.getException() == null) {
                    ...
                    if (dispatcher != null) dispatcher.forward(req, response);*/
                        ...
                        if (TYPE.equals(DPUrlRefType.FILE.toString())) {
                            log.trace("deleting " + srbDownload.getParentFile().getAbsolutePath() + "?:" + srbDownload.getParentFile().delete());
                        }
