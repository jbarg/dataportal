...
sqlplus metaematview/viewb4rk1ng <<EOF
...
select study_id id, study_id as visit_id, name title, purpose inv_abstract,'experiment' inv_type, '-99' mod_id, row_mod_time mod_time 
...
select study_id as investigation_id, role, person_id as facility_user_id
from metaematnew.study_person;

create or replace view facility_user as
select dn as federal_id, forename as first_name, surname as last_name,
person_id as facility_user_id
from metaematnew.person;
commit;
exit;
EOF
