...
public class TestDPAccessLayerEmat
{
   ...
   public static void main(String []args)
   {
     ...
     try
     {
         ...
         r_i_l = dpal.getInvestigationsById(inv_id_list, fed_id, false) ;
         for(Investigation i : r_i_l) {
            System.out.println("\t"+i.toString()) ;
         }

         System.out.println("AND FUZZY - The list of INVESTIGATIONS for the keywords"+keyword_list.toString()+":") ;
         r_i_l = dpal.getInvestigations(keyword_list, fed_id, LogicalOperator.AND, true, 500, false ) ;
         ...
         r_i_l = dpal.getInvestigations(keyword_list, fed_id, LogicalOperator.AND, false, 500, false ) ;
         for(Investigation i : r_i_l) System.out.println("\t"+i) ;
         System.out.println("-") ;

         System.out.println("OR FUZZY - The list of INVESTIGATIONS for the keywords"+keyword_list.toString()+":") ;
         r_i_l = dpal.getInvestigations(keyword_list, fed_id, LogicalOperator.OR, true, 500, false ) ;
         for(Investigation i : r_i_l) System.out.println("\t"+i) ;
         System.out.println("-") ;
         ...
         r_i_l = dpal.getInvestigations(keyword_list, fed_id, LogicalOperator.OR, false, 500, false ) ;
         for(Investigation i : r_i_l) {
        ...
        // inv_id_list.remove(0);
         r_d_l = dpal.getDataSets(inv_id_list, fed_id, false) ;
         ...
         r_f_l = dpal.getDataFiles(ds_id_list, fed_id, false) ;
