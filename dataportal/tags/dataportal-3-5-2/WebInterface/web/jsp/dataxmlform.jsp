...
<% 
	 ...
	 Logger logger = Logger.getLogger("dataxml.jsp");
	...
	conf.load(new FileInputStream(wd+File.separatorChar+"xml"+File.separator+"list.txt"));
       //ArrayList perlist = (ArrayList)session.getAttribute("facs");


        //main document of the xml
		...
		value = "start";
               ...
               // System.out.println("value of list2 is "+list2[0]);
                ...
                File[] filename = new File [(list2.length)];
                 ...
                 if(list2.length == 1) doc = XML_DOMBuilder.parse(new File(wd+File.separator+"xml"+File.separator+conf.getProperty(list2[0]).trim()));
                 
                 else{

                     for(int i = 0 ; i< list2.length;i++){

                        filename[i] = new File(wd+File.separator+"xml"+File.separator+conf.getProperty(list2[i]).trim());
...
//alert(type);
	...
	window.location.href="dataxmlform.jsp?level="+level+"&value="+value+"&list="+list+"&max_wait="+max_wait+"&no_max="+no_max+"&checked="+type;
...
//window.location.href="dataxmlform.jsp?level=0&value=start&list="+value;
