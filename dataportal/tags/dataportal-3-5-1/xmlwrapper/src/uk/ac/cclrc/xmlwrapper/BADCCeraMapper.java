...
public class BADCCeraMapper implements CeraMapper
{
   ...
   public String getKeys(String query) 
   {
	 ...
	 if(discipline.compareTo("") != 0)
         {
            ...
            temp_str = ("SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_ID IN " +
                        "(SELECT ENTRY_ID FROM PARAMETER WHERE TOPIC_ID IN " +
                        "(SELECT TOPIC_ID FROM TOPIC WHERE lower(TOPIC_NAME) LIKE '%" + discipline + "%'))") ;
	    discipline = temp_str ;
	 }
	 ...
	 if(subject.compareTo("") != 0)
         {
            temp_str = ("SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_ID IN " +
                        "(SELECT ENTRY_ID FROM LOCATION_CONNECT WHERE LOCATION_ID IN " +
                        "(SELECT LOCATION_ID FROM LOCATION WHERE lower(LOCATION_NAME) LIKE '%" + subject + "%'))") ;
	    subject = temp_str ;
	 }
	 ...
	 if(studyname.compareTo("") != 0)
	    ...
	    temp_str = ("(SELECT ENTRY_ID FROM ENTRY " +
		        "WHERE lower(ENTRY_NAME) LIKE '%" + studyname + "%' " +
			 "UNION " +
                         "SELECT ENTRY_ID FROM CAMPAIGN WHERE PROJECT_ID IN " +
                          "(SELECT PROJECT_ID FROM PROJECT WHERE lower(PROJECT_NAME) LIKE '%" + studyname + "%') " +
			")") ;
	 ...
	 if(studyid.compareTo("") != 0)
         {
            ...
            temp_str = ("(SELECT ENTRY_ID FROM ENTRY WHERE lower(ENTRY_ACRONYM) LIKE '%" + studyid + "%' " +
			 "UNION " +
                         "SELECT ENTRY_ID FROM CAMPAIGN WHERE PROJECT_ID IN " +
                          "(SELECT PROJECT_ID FROM PROJECT WHERE lower(PROJECT_ACRONYM) LIKE '%" + studyid + "%') " +
			")") ;
	 ...
	 if(dminstitution.compareTo("") != 0)
         {
            temp_str = ("SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_ID IN " +
                        "(SELECT ENTRY_ID FROM CONTACT WHERE INSTITUTE_ID IN " +
                       "(SELECT INSTITUTE_ID FROM INSTITUTE WHERE lower(INSTITUTE_NAME) LIKE '%" + dminstitution + "%'))") ;
	 ...
	 if(startdate.compareTo("") != 0)
         {
            temp_str = ("SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_ID IN " +
                        "(SELECT ENTRY_ID FROM COVERAGE WHERE TEMPORAL_COVERAGE_ID IN " +
                        "(SELECT TEMPORAL_COVERAGE_ID FROM TEMPORAL_COVERAGE " +
	 ...
	 if(enddate.compareTo("") != 0)
      ...
      {
         sql.append("SELECT ENTRY_ID FROM ENTRY") ;
         //sql.append("SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_ID IN ('600','601','602')") ;
      }
      ...
      sql.append(" minus select entry_id from entry where entry_type_id in " +
                 " (select entry_type_id from entry_type where entry_type not like '%STUDY%')") ;
   ...
   public void buildMetadataRecord(String entry_id, StringBuffer sbr) throws SQLException
   {
      ...
      r = s.executeQuery("select entry_type from entry_type where entry_type_id in " +
	                 "(select entry_type_id from entry where entry_id = '" + entry_id + "')") ;
      if(r.next())
      {
	 r.getRow() ;
	 entry_type = sr.LitWithEnt(xt.makeValid(r.getString("ENTRY_TYPE") ));
      ...
      r = s.executeQuery("select entry_acronym from entry where entry_id = '" + entry_id + "'") ;
      if(r.next())
      {
	 r.getRow() ;
	 entry_acronym = sr.LitWithEnt(xt.makeValid(r.getString("ENTRY_ACRONYM") ));
      ...
      //sbr.append("   <MetadataRecord metadataID=\"badc" + entry_id + "\">\n");
      ...
      r = s.executeQuery("select topic_name from topic where topic_id in" +
                        "(select distinct topic_id  from parameter where entry_id='" + entry_id + "')" ) ;
      ...
      r = s.executeQuery("SELECT GENERAL_KEY FROM GENERAL_KEY WHERE GENERAL_KEY_ID IN " +
                        "(SELECT DISTINCT GENERAL_KEY_ID FROM KEY_CONNECT WHERE " +
                        " ENTRY_ID='" + entry_id + "' )") ;
      ...
      r = s.executeQuery("SELECT PROJECT_NAME FROM PROJECT WHERE PROJECT_ID "+
                       " IN ( SELECT PROJECT_ID FROM CAMPAIGN WHERE ENTRY_ID='" + entry_id + "' )") ;
      ...
      r = s.executeQuery("SELECT ENTRY_NAME, CREATION_DATE FROM ENTRY WHERE ENTRY_ID='" + entry_id + "'") ;
      ...
      // - no need for this after re-load on 10/01/02 entry_name = project_name + entry_id ;
      ...
      r = s.executeQuery("SELECT STREET, PLACE, POBOX_POSTAL_CODE, COUNTRY, " +
                       "INSTITUTE_NAME, INSTITUTE_ID FROM INSTITUTE WHERE INSTITUTE_ID IN " +
                       "(SELECT INSTITUTE_ID FROM CONTACT WHERE ENTRY_ID='" + entry_id + "'" +
                           " AND CONTACT_TYPE_ID IN ( SELECT CONTACT_TYPE_ID FROM CONTACT_TYPE " +
      ...
      //sbr.append("         <StudyName>" + entry_name + "." + topic_name + ".cera" + entry_id + "</StudyName>\n" ) ;
      ...
      r = s.executeQuery("SELECT TITLE, FIRST_NAME, SECOND_NAME, LAST_NAME, " +
                       "TELEPHONE, EMAIL, FAX FROM PERSON WHERE" +
                       " PERSON_ID IN ( SELECT PERSON_ID FROM CONTACT WHERE ENTRY_ID='" + entry_id + "'" +
                       " AND CONTACT_TYPE_ID IN (SELECT CONTACT_TYPE_ID FROM CONTACT_TYPE WHERE " +
      ...
      // The Role
      r = s.executeQuery("SELECT CONTACT_TYPE FROM CONTACT_TYPE WHERE CONTACT_TYPE_ID IN " +
                        "(SELECT CONTACT_TYPE_ID FROM CONTACT WHERE ENTRY_ID='" + entry_id + "')" ) ;
      ...
      r = s.executeQuery("SELECT SUMMARY FROM SUMMARY WHERE SUMMARY_ID IN " +
	                 "(SELECT SUMMARY_ID FROM ENTRY WHERE ENTRY_ID='" + entry_id + "')" ) ;
      if(r.next()) 
      {
	 r.getRow() ;
	 summary = sr.LitWithEnt(xt.makeValid(r.getString("SUMMARY") ));
      ...
      appendConditionsForEntry(entry_id, 9, sbr) ; // 9 = 3 spaces per level - and 3 level 3x3
      ...
      r = s.executeQuery("SELECT CONSTRAINT_DESCR FROM  ACCESS_CONSTRAINT WHERE ACCESS_CONSTRAINT_ID IN " +
                           "(SELECT ACCESS_CONSTRAINT_ID FROM DISTRIBUTION WHERE ENTRY_ID='" + entry_id + "')") ;
      ...
      r = s.executeQuery("SELECT DISTINCT SPEC_ENTRY_ID FROM ENTRY_CONNECT WHERE GEN_ENTRY_ID = '" + entry_id +
	 ...
	 r = s.executeQuery("SELECT ENTRY_NAME, ENTRY_ACRONYM FROM ENTRY WHERE ENTRY_ID = '" + exp_id + "'") ;
	 if(r.next())
	 {
	    r.getRow() ;
	 ...
	 r = s.executeQuery("SELECT PROGRESS_DESCR FROM PROGRESS WHERE PROGRESS_ID IN " +
                           "(SELECT PROGRESS_ID FROM ENTRY WHERE ENTRY_ID='" + exp_id + "')") ;
	 ...
	 sbr_dhl.append("         <Locator type=\"absolute\">\n") ;
         ...
         r = s.executeQuery("SELECT CONSTRAINT_DESCR FROM  ACCESS_CONSTRAINT WHERE ACCESS_CONSTRAINT_ID IN " +
                           "(SELECT ACCESS_CONSTRAINT_ID FROM DISTRIBUTION WHERE ENTRY_ID='" + exp_id + "')") ;
         if(r.next())
         {
            r.getRow() ;
	 ...
	 log.debug("ENTRY_ID at this point is: " + exp_id) ;
         ...
         r = s.executeQuery("SELECT DISTINCT SPEC_ENTRY_ID FROM ENTRY_CONNECT WHERE GEN_ENTRY_ID = '" + exp_id +
	                    "' INTERSECT SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_TYPE_ID IN " +
	 ...
	 // now below 
         ...
         String dataset_entry_id = place_holder ;
      
         while(g.hasNext())
         {
	    dataset_entry_id = (String) g.next() ;

	    // fill in the ras_collection from this list
            r = s.executeQuery("SELECT DISTINCT RAS_COLLECTION FROM RAS_CONNECT WHERE ENTRY_ID = '" + dataset_entry_id + 
		               "'") ;
	    while(r.next())
	    {
	       r.getRow() ;
	    ...
	    r = s.executeQuery("SELECT ENTRY_NAME, ENTRY_ACRONYM FROM ENTRY WHERE ENTRY_ID = '" + dataset_entry_id + "'") ;
	    if(r.next())
	    {
	       r.getRow() ;
	       dataid = sr.LitWithEnt(xt.makeValid(r.getString("ENTRY_ACRONYM") ));
	    ...
	    r = s.executeQuery("SELECT PROGRESS_DESCR FROM PROGRESS WHERE PROGRESS_ID IN " +
                              "(SELECT PROGRESS_ID FROM ENTRY WHERE ENTRY_ID='" + dataset_entry_id + "')") ;
	    ...
	    sbr.append("             <LogicalDescription>\n") ;
            ...
            appendConditionsForEntry(dataset_entry_id, 15, sbr) ; // 5 levels in
	    ...
	    // now select the data_files associated with this data_set
            ...
            r = s.executeQuery("SELECT DISTINCT SPEC_ENTRY_ID FROM ENTRY_CONNECT WHERE GEN_ENTRY_ID = '" + dataset_entry_id +
	                    "' INTERSECT SELECT ENTRY_ID FROM ENTRY WHERE ENTRY_TYPE_ID IN " +
	       ...
	       datafiles.add(Integer.toString(r.getInt("SPEC_ENTRY_ID"))) ;
            ...
            String datafile_entry_id = place_holder ;
	    ...
	    //put in the child references for files
            while(ro.hasNext())
            {
	       datafile_entry_id = (String) ro.next() ;
	       
	       r = s.executeQuery("SELECT ENTRY_NAME FROM ENTRY WHERE ENTRY_ID = '" + datafile_entry_id + "'") ;
	       if(r.next())
	       {
		  r.getRow() ;
		  data_file_entry_name = sr.LitWithEnt(xt.makeValid(r.getString("ENTRY_NAME") ));
	       ...
	       //sbr.append("               <URI>" + getStoragePath(datafile_entry_id, 3) + "</URI>\n") ;
	       ...
	       sbr_dhl_files.append("            <Locator type=\"absolute\">\n") ;
               sbr_dhl_files.append("               <URL>" + getStoragePath(datafile_entry_id, 3) + "</URL>\n") ;
	    ...
	    //////
            ...
            sbr_dhl.append("            <DataName>" + dataname + "</DataName>\n") ;
	    sbr_dhl.append("            <Locator type=\"absolute\">\n") ;
	    //System.err.println("We are populating the data sets e_name= " + e_name + " e_id= " + e_id ) ;
            sbr_dhl.append("               <URL>" + getStoragePath(dataset_entry_id, 2) + "</URL>\n") ;
	    sbr_dhl.append("            </Locator>\n") ;
	    ...
	    while(i.hasNext())
	    {
	       ...
	       //extract the topic_name for this rasdaman collection
               r = s.executeQuery("select distinct topic_name from topic where topic_id in " + 
                                  ...
                                  "intersect " +
                                  "select distinct entry_id from ras_connect where " +
		  ...
		  topic_name = sr.LitWithEnt(xt.makeValid(r.getString("TOPIC_NAME") ));
	       ...
	       r = s.executeQuery("select distinct unit_acronym from unit where unit_id in " + 
                                  "(select unit_id from parameter where entry_id in " +
                                  "(select entry_id from entry where entry_type_id in " + 
                                  "(select entry_type_id from entry_type where entry_type like '%Grib message%') " + 
                                  "intersect " +
                                  "select distinct entry_id from ras_connect where " +
      ...
      //The Grib datafile
      //
      //sbr.append("          <DataSet dataid=\"grib" + entry_id + "\">\n") ;
      //sbr.append("             <DataName>" + entry_name + ".cera" + entry_id + "</DataName>\n") ;
      ...
      //sbr.append("          <DataSet dataid=\"rasdaman" + entry_id + "\">\n") ;
      //sbr.append("             <DataName>" + ras_collection + ".cera" + entry_id + "</DataName>\n") ;
   ...
   void appendConditionsForEntry(String entry_id, int num_spaces, StringBuffer sbr) throws SQLException
   {
	 ...
	 spcs.append(" ") ;
      ...
      r = s.executeQuery("SELECT MIN_LAT, MAX_LAT, MIN_LON, MAX_LON, MIN_ALTITUDE, MAX_ALTITUDE " +
                         "FROM SPATIAL_COVERAGE WHERE SPATIAL_COVERAGE_ID IN " +
                         "(SELECT SPATIAL_COVERAGE_ID FROM COVERAGE WHERE ENTRY_ID='" + entry_id + "')") ;

      int min_lat = i_place_holder ;
      int max_lat = i_place_holder ;
      int min_lon = i_place_holder ;
      int max_lon = i_place_holder ;
      int min_alt = i_place_holder ;
      int max_alt = i_place_holder ;

      if(r.next())
      {
         r.getRow() ;

         min_lat = r.getInt("MIN_LAT") ;
         max_lat = r.getInt("MAX_LAT") ;
         min_lon = r.getInt("MIN_LON") ;
         max_lon = r.getInt("MAX_LON") ;
         min_alt = r.getInt("MIN_ALTITUDE") ;
         max_alt = r.getInt("MAX_ALTITUDE") ;
      }

      r.close() ;

      r = s.executeQuery("SELECT START_YEAR, START_MONTH, START_DAY, STOP_YEAR, STOP_MONTH, STOP_DAY " +
                         "FROM TEMPORAL_COVERAGE WHERE TEMPORAL_COVERAGE_ID IN " +
                         "(SELECT TEMPORAL_COVERAGE_ID FROM COVERAGE WHERE ENTRY_ID='" + entry_id + "')") ;
      ...
      r = s.executeQuery("SELECT UNIT_NAME FROM UNIT WHERE UNIT_ID IN " +
                         "(SELECT MIN_ALT_UNIT_ID FROM SPATIAL_COVERAGE WHERE SPATIAL_COVERAGE_ID IN " +
                            "(SELECT SPATIAL_COVERAGE_ID FROM COVERAGE WHERE ENTRY_ID='" + entry_id + "')" +
                         ")") ;
      ...
      r = s.executeQuery("SELECT UNIT_NAME FROM UNIT WHERE UNIT_ID IN " +
                       "(SELECT MAX_ALT_UNIT_ID FROM SPATIAL_COVERAGE WHERE SPATIAL_COVERAGE_ID IN " +
                          "(SELECT SPATIAL_COVERAGE_ID FROM COVERAGE WHERE ENTRY_ID='" + entry_id + "')" +
                       ")") ;
      ...
      r = s.executeQuery("SELECT LOCATION_NAME FROM LOCATION WHERE LOCATION_ID IN " +
                         "(SELECT LOCATION_ID FROM LOCATION_CONNECT WHERE ENTRY_ID='" + entry_id + "')") ;
      if(r.next())
      {
         r.getRow() ;
      ...
      r = s.executeQuery("select year||lpad(month,2,0)||lpad(day,2,0)||lpad(hour,2,0)||" +
                         "lpad(minute,2,0)||lpad(second,2,0) \"TIME\", UTC_DIFFERENCE \"TZ\" " +
                         "from moment where moment_id in " +
                         "(select moment_id from time_connect where time_id in " +
                         "(select time_id from data_org where data_org_id in " +
                         "(select data_org_id from parameter where entry_id='" + entry_id + "') ) ) ") ;
   ...
   String getRasPath(String ras_collection)
   {
       return "http://twister.badc.rl.ac.uk:8080/examples/RasGribServlet?operation_mode=minimum&amp;" + 
              "ras_collection=" + ras_collection ;
   }
   ...
   String getStoragePath(String entry_id, int path_depth) throws SQLException
   {
      ...
      r = s.executeQuery("SELECT STORAGE1_ID, STORAGE2_ID, STORAGE3_ID, STORAGE4_ID FROM DATA_ACCESS " +
                         " WHERE DATA_ACCESS_ID IN (SELECT DATA_ACCESS_ID FROM PARAMETER WHERE " + 
                           "ENTRY_ID='" + entry_id + "')") ;
      ...
      r = s.executeQuery("SELECT STORAGE_NAME FROM STORAGE WHERE STORAGE_ID='" + stor1 + "'") ;
      if(r.next())
      {
         r.getRow() ;
      ...
      r = s.executeQuery("SELECT STORAGE_NAME FROM STORAGE WHERE STORAGE_ID='" + stor2 + "'") ;
      if(r.next())
      {
         r.getRow() ;
      ...
      r = s.executeQuery("SELECT STORAGE_NAME FROM STORAGE WHERE STORAGE_ID='" + stor3 + "'") ;
      if(r.next())
      {
         r.getRow() ;
      ...
      r = s.executeQuery("SELECT STORAGE_NAME FROM STORAGE WHERE STORAGE_ID='" + stor4 + "'") ;
      if(r.next())
      {
         r.getRow() ;
