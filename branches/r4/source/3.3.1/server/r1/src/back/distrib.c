...
char *fileName, int flags, int mode, int chdirFlag, char *hostAddrBuf)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        fd = _srbFileOpen (hostTabPtr->conn, storSysType, hostAddrBuf, 
         fileName, flags, mode, chdirFlag);
    } else {
        fd = _srbFileOpen (hostTabPtr->conn, storSysType, NULL, 
	 fileName, flags, mode, chdirFlag);
    }
...
char *fileName, int mode, int chdirFlag, srb_long_t size, char *hostAddr)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        fd = _srbFileCreate (hostTabPtr->conn, storSysType, hostAddr, fileName, 
          mode, chdirFlag, size);
    } else {
...
char *fileName, int chdirFlag, char *hostAddrBuf)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        status = _srbFileUnlink (hostTabPtr->conn, storSysType, 
	 hostAddrBuf, fileName, chdirFlag);
    } else {
...
char *fileName, struct srbStat *retStat, char *hostAddrBuf)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        status = srbFileStat (hostTabPtr->conn, storSysType, hostAddrBuf, 
          fileName, retStat);
    } else {
...
char *fileName, int mode, int chdirFlag, char *hostAddr)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        status = _srbFileMkdir (hostTabPtr->conn, storSysType, 
	  hostAddr, fileName, mode, chdirFlag);
    } else {
...
char *fileName, int chdirFlag, char *hostAddr)
{
    ...
    if (storSysType == GRIDFTP_FT) {
        status = _srbFileRmdir (hostTabPtr->conn, storSysType, 
	 hostAddr, fileName, chdirFlag);
    } else {
        status = _srbFileRmdir (hostTabPtr->conn, storSysType, 
	 NULL, fileName, chdirFlag);
    }
...
char *container, struct hostElement  *hostTabPtr)
{
    ...
    status = srbObjMove (hostTabPtr->conn, catType, objID,
      collectionName, srcResource, newResourceName, newPathName, container);
...
char *fileName, int flags)
{
    ...
    if (hostTabPtr == NULL) {
	elog (NOTICE, "remoteFileFStage: Invalid hostTabPtr");
	return SYS_ERR_HOST_ADDR;
    }
    ...
    status = srbFileFStage (hostTabPtr->conn, storSysType, NULL, fileName, 
     flags);

    if (status < 0) {	/* Failure */
	elog (NOTICE, "Remote FileFStage failed: %s", 
	  clErrorMessage(hostTabPtr->conn));
    }
...
char *rsrcLoc, char *fileName, int flags, int mode)
{
    ...
    if (hostTabPtr == NULL) {
	elog (NOTICE, "remoteMDriverOpen: Invalid hostTabPtr");
	return SYS_ERR_HOST_ADDR;
    }
    ...
    fd = srbMDriverOpen (hostTabPtr->conn, storSysType, rsrcLoc, fileName, flags, 
      mode);
...
int
remoteMDriverCreate (int storSysType, struct hostElement  *hostTabPtr, 
		     char  *rsrcLoc, char *fileName, int mode)
{
    int fd;

    if (hostTabPtr == NULL) {
	elog (NOTICE, "remoteMDriverCreate: Invalid hostTabPtr");
	return SYS_ERR_HOST_ADDR;
    }
    ...
    fd = srbMDriverCreate (hostTabPtr->conn, storSysType, rsrcLoc, fileName, 
      mode);
...
char *resourceLoc, char *dataPath)
{
    ...
    if (hostTabPtr == NULL) {
        elog (NOTICE, "remoteMDriverUnlink: Invalid hostTabPtr");
        return SYS_ERR_HOST_ADDR;
    }
    ...
    status = srbMDriverUnlink (hostTabPtr->conn, storSysType, resourceLoc,dataPath);
...
char *fileName, char *fileUser, char *fileGroup, int chownFlag)
{
    ...
    if (hostTabPtr == NULL) {
        elog (NOTICE, "remoteFileChown: Invalid hostTabPtr");
        return SYS_ERR_HOST_ADDR;
    }
    ...
    status = _srbFileChown (hostTabPtr->conn, storSysType, NULL, fileName,
      fileUser, fileGroup, chownFlag);
 
    if (status < 0) {       /* Failure */
        elog (NOTICE, "Remote chown failed: %s",
          clErrorMessage(hostTabPtr->conn));
    }
