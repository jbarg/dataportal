...
**** RAJA removed  January 24, 2003 as these checks are made in the layer above ****/
   ...
   if ( i != 0 && !strcmp(TokenSwitch,"DataType")) {
     i = isUserDomainAdmin(registrarName,  registrarDomain);
   }
   if ( i != 0)           
     return(SYS_USER_AUTHORIZATION_ERROR);
...
*** RAJA removed  January 24, 2003 as these checks are made in the layer above ****/
  ...
  if (i != 0)
    return(i);
  sprintf(query_string,
          "insert into %s%s (rsrc_id,metadatanum) values (%i,-1)",MDASSCHEMENAME,"MDAS_AR_MDATA", reg_obj_id);
  res = ExecSqlDb2(henv, hdbc, query_string);
  if (res != 0)
    return(MDAS_AR_MDATA_INSERTION_ERROR);
	    ...
	    char *userName, char *domainName)
{
  ...
  if (objName != NULL  && strcmp( objName,""))
      ...
      if (!strcmp(flag, "R")) 
	{
	  ...
	  */
	  tabledIsOkay =  1;
          get_ctype_value(newcollections[0],
...
deleteValueUser(char *delValue)
{
  ...
  for (i = 0; i < chkIdCnt ; i++) {
    ...
    /* done for MDAS_TD_DATA_GRP */
  sprintf(sqlq, "select DATA_GRP_ID from %sMDAS_TD_DATA_GRP where DATA_GRP_NAME = '/home/%s.%s' or  DATA_GRP_NAME = '/container/%s.%s' or DATA_GRP_NAME = '/trash/%s.%s' or DATA_GRP_NAME like '/%%/home/%s.%s' or  DATA_GRP_NAME like '/%%/container/%s.%s' or DATA_GRP_NAME like '/%%/trash/%s.%s'", 
	  MDASSCHEMENAME,userName, domainName,userName, domainName,userName, domainName,
	  userName, domainName,userName, domainName,userName, domainName);
  if ( check_exists(sqlq) == 0) { /* something exists */
    sprintf(logMsg,"Error: Value Not Removed. Value exists in MDAS_TD_DATA_GRP");
    elog(NOTICE, logMsg);
      return(ACTION_NOT_ALLOWED);
  }
  ...
  /*   these deletes  have been removed as we check previously for the collection's existance. if they exist the user cannot be removed **** RAJA Jan 31, 2005 


       

       
  sprintf(sqlq, "select DATA_GRP_ID from %sMDAS_TD_DATA_GRP where DATA_GRP_NAME = '/home/%s.%s' or  DATA_GRP_NAME = '/container/%s.%s' or DATA_GRP_NAME = '/trash/%s.%s' or DATA_GRP_NAME = '/%%/home/%s.%s' or  DATA_GRP_NAME = '/%%/container/%s.%s' or DATA_GRP_NAME = '/%%/trash/%s.%s' or DATA_GRP_NAME like '/home/%s.%s/%%' or  DATA_GRP_NAME like '/container/%s.%s/%%' or DATA_GRP_NAME like '/trash/%s.%s/%%' or DATA_GRP_NAME like '/%%/home/%s.%s/%%' or  DATA_GRP_NAME like '/%%/container/%s.%s/%%' or DATA_GRP_NAME like '/%%/trash/%s.%s/%%' ",
	  MDASSCHEMENAME,
	  userName, domainName,userName, domainName,userName, domainName,
          userName, domainName,userName, domainName,userName, domainName,
	  userName, domainName,userName, domainName,userName, domainName,
  ...
  ****** everything below CAN  replaced by above statement 
...
int
deleteValueInfo(int valueType, char *deleteValue, char *registrarName, char *registrarDomain)
{
  int status;

   i = isUserSysAdmin(registrarName,  registrarDomain);
   if ( i != 0 && valueType == DELETE_TYPE_USER) {
     i = isUserDomainAdmin(registrarName,  registrarDomain);
   }
   if ( i != 0)
     return(SYS_USER_AUTHORIZATION_ERROR);
