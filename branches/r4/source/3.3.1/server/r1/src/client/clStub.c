...
char *srcResource, char *newResourceName, char *newPathName, char *container)
{
    ...
    argv[4] = addrToSRBLong (newResourceName);
    argv[5] = addrToSRBLong (newPathName);
    argv[6] = addrToSRBLong (container);
...
/*
 * srbFileLockRange - perform locking operations on all or a portion of a file;
 *              block and wait if the lock cannot be acquired.
 *              The arguments are following the MDSPlus convention.
 *
 * Input - srbConn* conn - From srbConnect ().
 *         int fd - The file descriptor to seek (from srbFileOpen or 
 * 	   srbFileCreate).
 *         int64 offset - offset for the lock range.
 *         int size - size of the lock range.
 *         int mode - 0 F_UNLCK  unlock
 *                    1 F_RDLCK  set a read lock
 *                    2 F_WRLCK  set a write lock
 *
 * Output - Returns 0 upon success.
 *          Returns a negative value upon failure.
 */
int
srbFileLockRange(srbConn *conn, 
		 int fd, srb_long_t offset, int size, int mode)
{
    clArgBlock argv[4];
    srbResult *res;
    int resultLen;
    ...
    argv[2].isint = 1;
    argv[2].len = 4;
    argv[2].u.integer = size;

    argv[3].isint = 1;
    argv[3].len = 4;
    argv[3].u.integer = mode;

    res = clFunct(conn, F_E_LOCK_RANGE,
		  (char *) &retval,&resultLen,1,argv,4);
...
/*
 * srbObjLockRange - perform locking operations on all or a portion of a file;
 *              block and wait if the lock cannot be acquired.
 *              The arguments are following the MDSPlus convention.
 *
 * Input - srbConn* conn - From srbConnect ().
 ...
 */
int
srbObjLockRange(srbConn *conn, 
		int desc, srb_long_t offset, int size, int mode)
{
    clArgBlock argv[4];
    srbResult *res;
    int resultLen;
    ...
    res = clFunct(conn, F_SRBO_LOCK_RANGE,
		  (char *) &retval,&resultLen,1,argv,4);
		      ...
		      int retractionType)
{
    ...
    argv[0] = catType;
    argv[1] = addrToSRBLong (dataName);
    argv[2] = addrToSRBLong (collName);
    ...
    argv[7] = addrToSRBLong (dataValue5);
    argv[8] = retractionType;

    retval = clCall (conn, F_MODIFY_EXT_META_DATA, argv, &clOut);
    return retval;
}
   ...
   status = pthread_create(&portalManagerThread, pthread_attr_default,
    ...
    if (retval < 0) {
        ...
        if (retval == MSG_USE_SINGLE_PORT) {
            retval = seqObjPut (conn, destObjID, destCollection, 
	     destResLoc, dataType, destPath, locFilePath, size, forceFlag);
        }
...
srb_long_t size, int forceFlag, int numThreads)
{
    ...
    if (status < 0) {
        if (status == MSG_USE_SINGLE_PORT) {
            status = seqObjPut (conn, destObjID, destCollection, 
             destResLoc, dataType, destPath, locFilePath, size, forceFlag);
        }
	retval = status;
        return retval;
...
#endif
	if (retval == MSG_USE_SINGLE_PORT) {
	    retval = seqObjGet (conn, srcObjID, srcCollection, locFilePath);
	}
        return retval;
    }
...
char *locFilePath, int flag, int numThreads)
{
    ...
    if (status < 0) {
        if (status == MSG_USE_SINGLE_PORT) {
            status = seqObjGet (conn, srcObjID, srcCollection, locFilePath);
        }
        return status;
    }
...
#ifdef _WIN32
    nbytes = recv(newSock,(char *)&myCookie,sizeof(myCookie),0);
#else
...
char *collection, mdasC_sql_result_struct **myresult)
{
    ...
    if (status == MSG_USE_SINGLE_PORT) {
	newSock = dup (conn->sock);
        if (newSock < 0) {
            (void) sprintf(conn->errorMessage,
             "srbBulkUnloadC()-dup failed: status = %d, errno =%d\n",
              newSock, errno);
            return CLI_ERR_SOCKET;
        }
	offset = clGetAllChar ((char *)&myHeader, conn->commBuf, conn->sock, 
	 conn->authSchemeInx, sizeof (myHeader));
        status = readPartialHeader (newSock, &myHeader, offset);
	useSinglePort = 1;
    } else if (status < 0) {
        return status;
    } else {
        ...
        if (newSock < 0) {
            (void) sprintf(conn->errorMessage,
             "srbBulkUnloadC()-portalConnect failed: status = %d, errno =%d\n",
              newSock, errno);
            return CLI_ERR_SOCKET;
        }
        status = readHeader (newSock, &myHeader);
    ...
    if (toread == 0) {
        ...
        fprintf (stderr,
         "srbBulkUnloadC: Received zero length for result struct\n");
        return (MCAT_INQUIRE_ERROR);
    }
    ...
    bufptr = buf;
    if (useSinglePort > 0) {
        offset = clGetAllChar (bufptr, conn->commBuf, conn->sock,
         conn->authSchemeInx, toread);
	if (offset > 0) {
	    toread -= offset;
	    bufptr += offset;
	    /* do it one more time to make sure */
	    offset = clGetAllChar (bufptr, conn->commBuf, conn->sock,
             conn->authSchemeInx, toread);
	    if (offset > 0) {
#ifdef _WIN32
                ...
                free (buf);
		fprintf (stderr, 
		 "srbBulkUnloadC: Internal buffer management error\n");
                return (SYS_ERR_TRANSFER_FAILED);
	    }
	}
    }
...
int flags)
{
    ...
    retval = clCall (conn, F_E_FSTAGE, argv, &clOut);
    return retval;
}
...
char *resource, int flag)
{
    ...
    retval = clCall (conn, F_SRBO_FSTAGE, argv, &clOut);
    return retval;
}
...
int flags, int mode)
{
    ...
    argv[0] = storSysType;
    argv[1] = (int) resourceLoc;
    argv[2] = (int) dataPath;
    argv[3] = flags;
    argv[4] = mode;
 
    retval = clCall (conn, F_MDRIVER_OPEN, argv, &clOut);
    return retval;
}
...
int mode)
{
    ...
    retval = clCall (conn, F_MDRIVER_CREATE, argv, &clOut);
    return retval;
}
...
srbMDriverClose(srbConn* conn, int fd)
{
    ...
    retval = clCall (conn, F_MDRIVER_CLOSE, argv, &clOut);
    return retval;
}
...
srbMDriverRead(srbConn *conn, int fd, char *buf, int len)
{
    ...
    retval = clCall (conn, F_MDRIVER_READ, argv, &clOut);
    return retval;
}
...
srbMDriverWrite(srbConn *conn, int fd, char *buf, int len)
{
    ...
    res = clFunct(conn, F_MDRIVER_WRITE,(char *) &retval,&resultLen,1,argv,2);
    if (clResultStatus(res) == CLI_COMMAND_OK) {
        clClear(res);
        return retval;
    } else {
...
srbMDriverSeek(srbConn *conn, int fd, int offset, int whence)
{
    ...
    retval = clCall (conn, F_MDRIVER_SEEK, argv, &clOut);
    return retval;
}
...
char *dataPath)
{
    ...
    retval = clCall (conn, F_MDRIVER_UNLINK, argv, &clOut);
    return retval;
}
...
srbMDriverSync(srbConn *conn, int fd)
{
    ...
    argv[0] = fd;
    retval = clCall (conn, F_MDRIVER_SYNC, argv, &clOut);
    return retval;
}

/*
 * srbObjProc - Execute Procedure on srbObject
 *
 * Input - srbConn* conn - From srbConnect ().
 *         int desc - The object descriptor (from the srbObjOpen call) to read.
 *         char *procName - name of Remote Procedure.
 *         char *inBuf - The input buffer.
 *         int inBufLen - The number of bytes in inBuf.
 *         char *outBuf - The output buffer.
 *         int outBufLen - The size of pre-allocated outBuf.
 *
 * Output - Returns the length of outBuf.
 *          Returns a negative value upon failure.
 * 
 */
 
int
srbObjProc(srbConn *conn, int desc, char *procName, 
           ...
           char *outBuf, int outBufLen)
{
    ...
    argv[1].isint = 1;
    argv[1].len = 4;
    argv[1].u.integer = outBufLen;

    argv[2].isint = 0;
    argv[2].len = strlen(procName);
    argv[2].u.ptr = (char*) procName;

    argv[3].isint = 0;
    argv[3].len = inBufLen;
    argv[3].u.ptr = (char*) inBuf;
   
   
    res = clFunct(conn, F_SRBO_PROC, outBuf,&resultLen,0,argv,4);
    if (clResultStatus(res) == CLI_COMMAND_OK) {
        clClear(res);
        return resultLen;
    } else {
		...
		char *outBuf, int outBufLen)
{
    ...
    res = clFunct(conn, F_MDRIVER_PROC, outBuf,&resultLen,0,argv,4);
    if (clResultStatus(res) == CLI_COMMAND_OK) {
        clClear(res);
        return resultLen;
    } else {
...
char *fileUser, char *fileGroup, int chownFlag) 
{
    ...
    argv[1] = addrToSRBLong (hostAddr);
    argv[2] = addrToSRBLong (filename);
    argv[3] = addrToSRBLong (fileUser);
    argv[4] = addrToSRBLong (fileGroup);
    argv[5] = chownFlag;
 
    retval = clCall (conn, F_E_CHOWN, argv, &clOut);
    return retval;
}
...
char *locFilePath)
{
    ...
    if (in_fd < 0) {
        fprintf(stderr,
         "seqObjGet: Unable to open object '%s/%s', status = %d.\n",
...
#ifdef _WIN32
    status = stat_file (locFilePath, &statbuf);
#else
    status = stat (locFilePath, &statbuf);
#endif
    if (status == 0 && (statbuf.st_mode & S_IFREG) != 0) {  /* A file */
#ifdef _WIN32
        out_fd = SrbNtFileBinaryOpen(locFilePath, O_RDWR|O_TRUNC);
#else
        out_fd = open (locFilePath, O_RDWR|O_TRUNC, 0666);
#endif
        if (out_fd < 0) {
            fprintf (stderr,
             "seqObjGet: Unable to open local file %s, status = %d\n",
              locFilePath, out_fd);
...
#ifdef _WIN32
        out_fd = SrbNtFileBinaryCreate(locFilePath);
#else
        out_fd = creat (locFilePath, 0644);
#endif
        if (out_fd < 0) {
            fprintf (stderr,
             "seqObjGet: Unable to create local file %s, status = %d\n",
              locFilePath, out_fd);
            srbObjClose (conn, in_fd);
            return (out_fd);
        }
    }
    buf = malloc (2 * BUF_SIZE);
    if (buf == NULL) {
        fprintf (stderr, "seqObjGet: Unable to malloc buf\n");
        return (-1);
    }
    bytesCopied = 0;
    while ((cnt =  srbObjRead (conn, in_fd, buf, 2 * BUF_SIZE)) > 0) {
        tmp = write (out_fd, buf, cnt);
        if (tmp < cnt) {
            fprintf(stderr,
              "seqObjGet: Read %d bytes, Wrote %d bytes.\n ",
               cnt, tmp);
            free (buf);
            return (-1);
        }
...
srb_long_t size, int forceFlag)
{
    ...
    buf = malloc (2 * BUF_SIZE);
    if (buf == NULL) {
        fprintf (stderr, "seqObjPut: Unable to malloc buf\n");
        return (-1);
    }

#ifdef _WIN32
    in_fd = SrbNtFileBinaryOpen(locFilePath, O_RDONLY);
#else
    in_fd = open (locFilePath, O_RDONLY, 0);
#endif
    if (in_fd < 0) {
        fprintf(stderr,
         "seqObjPut: Unable to open local file '%s', errno = %d.\n",
              locFilePath, errno);
        free (buf);
        return(in_fd);
    }

    out_fd = srbObjCreate (conn, MDAS_CATALOG, destObjID,
      dataType, destResLoc, destCollection, destPath, size);
    if (out_fd < 0) {
        out_fd = srbObjOpen (conn, destObjID, O_WR_REPL|O_TRUNC,
          destCollection);
	if (out_fd < 0) {
            fprintf (stderr,
             "seqObjPut: Unable to open object %s/%s, status = %d\n",
    ...
    bytesCopied = 0;
    while ((i = read(in_fd, buf, 2 * BUF_SIZE)) > 0) {
        tmp = srbObjWrite(conn, out_fd, buf, i);
        if (tmp < i) {
            fprintf(stderr, "seqObjPut: Read %d bytes, Wrote %d bytes.\n ",
              i, tmp);
...
char **outBuf)
{
    ...
    *outBuf = malloc (MAX_TOKEN);
    ...
    argv[2] = addrToSRBLong (userDn);

    retval = clCall (conn, F_GET_USER_BY_DN, argv, &clOut);
    if (retval < 0) {
	free (*outBuf);
	*outBuf = NULL;
    }
...
/*
 * srbObjMoveColl - Phymove a collecion
 *
 * Input - srbConn* conn    - From clConnect ().
 *         int catType      - catalog type. e,g., MDAS_CATALOG.
 *         char *collectionName  - The collection
...
int srbObjMoveColl (srbConn* conn, int catType, char *collectionName, int flag,
char *srcResource, char *destResource, char *containerName)
{
    int retval;
    srb_long_t argv[MAXFMGRARGS];
    ...
    retval = clCall (conn, F_MOVE_COLL, argv, &clOut);
    return retval;
}
