   ...
   while (myMoverInput->mySession->transferStatus == HPSS_E_NOERROR) {
	 ...
	 break;
...
#ifdef PARA_DEBUG
      {
         ...
         time(&tim);
         printf ("Thread # %d - %d bytes received at offset %lld %s",
          index + 1, low32m(completionMessage.BytesMoved),
          (srb_long_t) initMessage.Offset, ctime(tim)); /* ctime(tim) adds "\n" */
         fflush (stdout);
      }
#endif
      if (status != HPSS_E_NOERROR) {
   ...
   while (myMoverInput->mySession->transferStatus == HPSS_E_NOERROR) {
	 ...
	 break;
...
#ifdef PARA_DEBUG
      {
         ...
         printf ("Thread #%d - %d bytes sent at offset %lld %s",
          index + 1, low32m(completionMessage.BytesMoved),
          (srb_long_t) initMessage.Offset, ctime(tim)); /* ctime(tim) adds "\n" */
         fflush (stdout);
      }
      ...
      if (dBuf->operation == WRITE_OPR) {
         ...
         if (nbytes < 0) {
            elog (NOTICE, "writeRelay : _svrFileRead error, status = %d",
             nbytes);
            if (dBuf->myMoverInput != NULL)
               dBuf->myMoverInput->mySession->transferStatus = nbytes;
            else
               dBuf->status[curInx] = nbytes;
            break;
         }
...
#ifdef HPSS5
      mySession->fileSize = myAttr.Attrs.DataLength;
#else
