...
main(int argc, char **argv)
{
    ...
    while ((c=getopt(argc, argv,"aChcA:rflVpvz:")) != EOF) {
        switch (c) {
            case 'h':
		usage (argv[0]);
                exit (0);
   ...
   i = initSrbClientEnv();
    ...
    if (lcval == 1) {
      strcpy(inCondition,argv[argc-1]);
      strcpy(collName,"");
    ...
    if (strlen(zoneName) == 0) {
      ...
      if (strcmp(zoneName,"ALL") && has_like_chars(zoneName) == 0) {
	if (strlen(collName) == 0) {
	  sprintf(collName,"/%s/*/*",zoneName);
	  strcpy(nameArray[0].outArgv,collName);
	}
	else {
	  strcpy(tmpcollName,collName);
	  sprintf(collName,"/%s/%s",zoneName,tmpcollName);
	  strcpy(nameArray[0].outArgv,collName);
	}
	nameArray[0].type = DATANAME_T;
	fprintf(stdout,"Querying Zone=%s\n",zoneName);fflush(stdout);
	status = queryPrtCollData (conn, MDAS_CATALOG, nArgv, nameArray, flagval,
				   L_val);
      }
      else {
	n = getZoneNameList(conn, MDAS_CATALOG,zoneList,zoneName);
	if (n <= 0) {
	  fprintf(stderr,"Sls:Error in Getting Zone Names: %i\n",n);
	  clFinish(conn); exit(3);
	}
	strcpy(tmpcollName,collName);
	for (i = 0; i < n ; i++){
	  if (strlen(tmpcollName) == 0)
	    sprintf(collName,"/%s/*/*",zoneList[i]);
	  else {
	    sprintf(collName,"/%s/%s",zoneList[i],tmpcollName);
	  }
	  nameArray[0].type = DATANAME_T;
	  strcpy(nameArray[0].outArgv,collName);
...
prtColl (srbConn *conn, mdasC_sql_result_struct *myresult, int flagval)
{
    ...
    tmpList = (char *) getFromResultStruct(
      (mdasC_sql_result_struct *) myresult,
        dcs_tname[DATA_GRP_NAME], dcs_aname[DATA_GRP_NAME]);
    if (flagval & C_FLAG) {
        inheritList = (char *) getFromResultStruct (myresult,
            dcs_tname[ACL_INHERITANCE_BIT],
            dcs_aname[ACL_INHERITANCE_BIT]);
    }
...
*/
	if (flagval & C_FLAG) {
	    ...
	    if (status >= 0) {
		  ...
		  dcs_aname[GROUP_ACCESS_CONSTRAINT]);
			...
			if (strlen(tmpList) > 45)
                        fprintf (stdout, "  C-%s %1.1s %8.8s  %s@%s\n", 
		          tmpList, inheritPtr, accessList, 
			  userList, domainList);
			else
			fprintf (stdout, "  C-%-46.46s %1.1s %8.8s  %s@%s\n",
		          tmpList, inheritPtr, accessList, 
			  userList, domainList);
		    }
        ...
        if(disp_meta)
        {
           ...
           DisplayUserDefinedMetatData(disp_coll,NULL);
        }

        tmpList += MAX_DATA_SIZE;
        inheritList += MAX_DATA_SIZE;
    }
}
...
*/
    ...
    if (versionFlag == 1) {
      verList = (char *) getFromResultStruct(
          (mdasC_sql_result_struct *) myresult,
...
*/
	toPrint = 1;
	    ...
	    if ((iSize + iOffset > iContSize) && iContSize >= 0) {
		toPrint = 0;
	    } else {
                inCont = "IC";
	    }
	...
	if (toPrint > 0) {
		...
		fprintf (stdout,
                  "  %-10.12s %2s %-8.8s %-20.20s %2s %8.12s %16.16s %s %s/%s\n",
                  ownerList, replNumList, verList, rsrcName, inCont, sizeList,
		  timeList,  dirty, collList, dataList);
		...
		fprintf (stdout,
                  "  %-10.12s %2s %-8.8s %-20.20s %2s %8.12s %16.16s %s %s\n",
                  ownerList, replNumList,verList, rsrcName, inCont, sizeList,
		  timeList, dirty, dataList);
            }
/*
        }
*/
	        ...
	        if (collList != NULL)
	          ...
	          strcpy(tmpCollName,disp_parcoll);
                ...
                DisplayUserDefinedMetatData(tmpCollName,tmpDataName);
             }
	}
...
usage (char *prog)
{
    fprintf(stderr,"Usage  :%s [-havlpCV] [-r|-f] [-A condition] [collection(or srbObj) ...]\n",
      prog);
    fprintf(stderr,"Usage  :%s -c [-havlC]  [-z zone]  condition\n",
      prog);
}
...
{
    int i, status;
    char *dataList, *collList, *fileList, 
