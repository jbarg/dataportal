...
main(int argc, char **argv)
{
	...
	{
		fprintf(stderr, "srbrmdir: aborting on srbUserArgs error\n");
		exit(42) ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbrmdir : trying clConnect with\n\t host = %s\n\t port = %s\n\t auth = %s\n", host, port, auth) ;
	conn = clConnect (host, port, auth);

	/* check to see that the backend connection was successfully made */
	if (clStatus(conn) != CLI_CONNECTION_OK)
	{
		fprintf(stderr,"srbrmdir: Connection to srbMaster failed.\n");
		fprintf(stderr,"%s",clErrorMessage(conn));
		clFinish(conn);
		exit(1);
	}
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbrmdir: connected.\n\n") ;
	...
	for (n=0 ; n<ndirs ; n++)
	{
		...
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr,"srbrmdir: Attempting srbFileRmdir with\n\t storSysType = %s (%d)\n\t hostAddr = %s\n\t dir = %s\n", systv, syst, rsrc, dir) ;

        	status = srbFileRmdir(conn, syst, rsrc, dir) ;
		if (status != 0)
		{
			fprintf(stderr,"srbrmdir: srbFileRmdir failed (status = %d)\n", status) ;
			fprintf(stderr,"%s",clErrorMessage(conn));
			exit(1) ;
	        }
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbrmdir: closing connection ...\n") ;
	clFinish(conn);
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbrmdir: finished.\n\n") ;
