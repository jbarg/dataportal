...
$Version="install.pl version 3.x, last updated March 30, 2005";
...
$SRB_FILE="SRB3.3rele_pgp.tar";   # The SRB tar file
...
$SRB_FILE_ALREADY_DECRYPTED=0;    # Set this to 1 if the SRB Tar file is
                                  ...
                                  # manually).
$SRB_DIR="";                      # Subdirectory that untar'ing the 
...
if ($arg1 eq "clean") {
    ...
    printf("Enter yes (again) to do so and remove everything:");
    $cmd=<STDIN>;
    chomp($cmd);
    if ($cmd ne "yes") {
	die("Aborted by user");
