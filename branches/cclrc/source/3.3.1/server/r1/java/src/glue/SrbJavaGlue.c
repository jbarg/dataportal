...
int TicketUserFlag;
char ResourceName[MAX_TOKEN];    /* needed by bloadLib.c */
char newpathname[MAX_TOKEN];    /* needed by bloadLib.c */
char datatype[MAX_TOKEN];    /* needed by bloadLib.c */
...
extern char* ContainerName;

int
setTransferModeFlag (char *transferMode, int inpMode);
...
char *transferMode) {
    ...
    flagval = setTransferModeFlag (transferMode, flagval);
    flagval &= ~b_FLAG;	/* don't do bulk for individual file */
...
char * resourceName, char * localDir, char *container, char *transferMode)
{
    int status;
    srbPathName mySrbPathName;
    int flagval = 0;

    mySrbPathName.inpArgv = localDir;

    /* fill in size, etc */
    status = chkFileName (&mySrbPathName);
	...
	if (flagval & c_FLAG) {
	    strcpy (ContainerName, container);
	}
        targName.inpArgv = collectionName;
        targName.outArgv = collectionName;
        targName.type = COLLECTION_T;
	strcpy (ResourceName, resourceName);
        status = bloadMain (g_conn[cid], catType, 1, &mySrbPathName, 
	 &targName, flagval);
    } else {
        status = dirToCollCopy (g_conn[cid], catType, flagval, &mySrbPathName,
          collectionName, resourceName, container, "");
    }

    /* clean up */
    free (mySrbPathName.outArgv);

    return status;
}

int srbDataToFileCopyJ (int cid, int catType, char * objID, int replNum,
char * collectionName, char *targFile, int forceFlag, char *transferMode) {
    int status;
    mdasC_sql_result_struct myresult;
    int flagval;
...
char * collectionName, char *targDir, char *transferMode) {
    int status;
    mdasC_sql_result_struct myresult;
    int flagval;
    ...
    status = dataToDirCopy (g_conn[cid], catType, flagval, &myresult,
     replNum, targDir, "");
...
char *targDir, char *transferMode) {
    int status;
    mdasC_sql_result_struct myresult;
    int flagval = 0;
    ...
    if (flagval & b_FLAG) {
	srbPathName nameArray, targName, *contnameArray;
	int nVal;
	
	nameArray.inpArgv = collectionName;
	nameArray.outArgv = collectionName;
	nameArray.type = COLLECTION_T;
	targName.inpArgv = targDir;
	targName.outArgv = targDir;
	targName.type = COLLECTION_T;
	nVal = getContainerByColl (g_conn[cid], &nameArray, &contnameArray);
        if (nVal <= 0) {
            status = nVal;
	} else {
            status = unloadContMain (g_conn[cid], catType, nVal,
             contnameArray, flagval, targName.outArgv,
             &nameArray);
        }
    } else {
...
char *targColl, char * newResource, char *container, char *transferMode) {
    int status;
    mdasC_sql_result_struct myresult;
    int flagval = 0;
...
/*                                                                      */
/************************************************************************/
int srbListResourceJ(int cid, char *zone)
{
	int	i, j, k;
	mdasC_sql_result_struct myresult;
	int clStat;
	...
	selval[RSRC_NAME] = 1;
	j = srbGetDataDirInfoWithZone( g_conn[cid], MDAS_CATALOG, zone,
	 qval, selval, &myresult, MY_MAX_ROW);
...
int srbRmColl ( int cid, int catType, char * collectionName )
{
    ...
    myresult.sqlresult[0].att_name = dcs_aname[DATA_GRP_NAME];
    myresult.sqlresult[0].values = (mdasC_handle)  collectionName;
    return( rmColl ( g_conn[cid], catType, f_FLAG, &myresult, -1, 0 ) );
}
...
jstring jContainer, jstring jTransferMode)
{
    int status;
    int cConn;
    int cCatType;
...
jstring localDir, jstring jContainer, jstring jTransferMode)
{
    int status;
    int cConn;
    int cCatType;
...
jstring jTargResource, jstring jContainer, jstring jTransferMode)
{
    int status;
    int cConn;
    int cCatType;
    ...
    cTargResource = (*env)->GetStringUTFChars( env, jTargResource, 0 );
    cContainer = (*env)->GetStringUTFChars( env, jContainer, 0 );
    cTransferMode = (*env)->GetStringUTFChars( env, jTransferMode, 0 );

    status = srbCopyCollToCollJ (cConn, cCatType,
     (char *) cCollectionName, (char *) cTargColl, (char *) cTargResource,
      (char *) cContainer, (char *) cTransferMode);
    (*env)->ReleaseStringUTFChars( env, jCollectionName, cCollectionName);
    (*env)->ReleaseStringUTFChars( env, jTargColl, cTargColl );
    (*env)->ReleaseStringUTFChars( env, jTargResource, cTargResource );
    (*env)->ReleaseStringUTFChars( env, jContainer, cContainer );
    (*env)->ReleaseStringUTFChars( env, jTransferMode, cTransferMode );
...
( JNIEnv * env, jobject obj,
        jint jConn, jint jCatType, jstring jObjID, jstring jReplNum,
        jstring jCollectionName, jstring jTargDir, jstring jTransferMode)
{
    int status;
    int cConn;
    int cCatType;
	...
	jstring jTransferMode)
{
    ...
    status = srbCollToDirCopyJ (cConn, cCatType, (char *) cCollection, 
     (char *) cTargDir, (char *) transferMode);
...
( JNIEnv * env, jobject obj,
	jint jConn, jint index, jstring jZone )
{
	int cConn;
	const char * cZone;
	int status;
	...
	cConn = jConn;
        cZone = (*env)->GetStringUTFChars( env, jZone, 0 );

#ifdef DEBUG
fprintf( stderr, "DEBUG: SrbJavaGlue.c Java_edu_sdsc_SrbGlue_SrbJavaGlue_srbListResourceJ\n" );
#endif
	status = srbListResourceJ(cConn, (char *) cZone);


	(*env)->ReleaseStringUTFChars( env, jZone, cZone );
...
jstring jClass, jint jSize)
{
    ...
    status = srbModifyRescInfo(g_conn[cConn], MDAS_CATALOG, (char *) cRescName, 
...
jint jConn, jint jValueType, jstring jDeleteValue)
{
    ...
    status = srbDeleteValue(g_conn[cConn], cValueType, (char *) cDeleteValue);
...
setTransferModeFlag (char *transferMode, int inpMode)
{
	...
	fprintf (stderr, "setTransferModeFlag: input transferMode is NULL\n");
    } else if (strcmp (transferMode, "parallel/clientInitiated") == 0) {
	outMode |= m_FLAG;
	outMode |= M_FLAG;
    } else if (strcmp (transferMode, "parallel/serverInitiated") == 0) {
        outMode |= m_FLAG;
    } else if (strcmp (transferMode, "bulk") == 0) {
        outMode |= b_FLAG;
    } else if (strcmp (transferMode, "serial") != 0) {
	fprintf (stderr, "setTransferModeFlag:invalid input transferMode %s\n");
    }
    return outMode;
}
