...
public class srbClObj
{
   ...
   // From srbC_mdas_externs.h:
    ...
    public final static String [] transferModeType = {
        "serial",
        "parallel/clientInitiated",
        "parallel/serverInitiated",
        "bulk"
    };
   ...
   int catType = 0;
    ...
    public String defTransferMode = null; // the default Transfer mode
   ...
   int types[] = new int[200];
    ...
    public String thisResZone = null;	// The zone when we last did listResc
	...
	this.defTransferMode = transferModeType[0];
	this.userName = srbJG.getUser (conn);
	this.domain = srbJG.getDomain (conn);
	this.rescList = listResc (this.cwd);
    ...
    public String[] listResc(String myPath) {
	String myResZone;
	int chrIndex;
	String[] list = this.rescList;

	if (myPath == null) {
	    myResZone = new String ("/home");
	} else {
	    chrIndex = myPath.indexOf ('/', 1);
	    if (chrIndex < 0) {
		myResZone = new String (myPath);
	    } else {
		myResZone = myPath.substring (0, chrIndex);
	    }
	}

	if (thisResZone == null || myResZone == null ||
	 !myResZone.equals (thisResZone)) {
	    thisResZone = myResZone;
            int count = srbJG.srbListResourceJ(conn, 0, myResZone);
	    if (count > 0) {
    ...
    public int importToNewObjJ (String objID, String collection, 
      String resource, String localFile, String container) {
        ...
        status = srbJG.importToNewObjJ (conn, catType,
          objID, collection, resource, localFile, container, defTransferMode);
    ...
    public int importDirJ (String collection,
      String resource, String localDir, String container) {
        ...
        status = srbJG.importDirJ (conn, catType,
          collection, resource, localDir, container, defTransferMode);
	...
	int status;
        ...
        status = srbJG.exportToFileJ (conn, catType,
          objID, replNum, srcColl, targFile, forceFlag, defTransferMode);
	...
	int status;
        ...
        status = srbJG.exportToDirJ (conn, catType,
          objID, replNum, srcColl, targDir, defTransferMode);
    ...
    public int exportCollToDirJ (String srcColl, String targDir) {
        ...
        status = srbJG.exportCollToDirJ (conn, catType,
          srcColl, targDir, defTransferMode);
    ...
    public int copyCollToCollJ (String collection, String targColl, 
      String resource, String container) {
        ...
        status = srbJG.copyCollToCollJ (conn, catType,
          collection, targColl, resource, container, defTransferMode);
