...
public class SRBQueryMapping {
	/*
		...
		*/
	private String[][] srbVariableMapping = { {"OFFSET", SRBMetaDataSet.OFFSET}
...
//	   , {"USERDEFFUNC05", SRBMetaDataSet.USERDEFFUNC05}
	...
	/*
		...
		*/
	public SRBQueryWrapper mapMatrixQueryToSRBQuery(MatrixQueryWrapper
													matrixQuery) {
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixQueryToSRBQuery():BEGIN",
		...
		conditionArray = mapMatrixConditionToSRBCondition(matrixQuery.
					getMatrixQueryWhereClause());
		// Map Selection Array
		selectArray = mapMatrixReturnClauseToSRBSelection(matrixQuery.
					getMatrixQueryReturnClause());
		...
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixQueryToSRBQuery():END",
							MatrixUtil.DEBUG_LOG);
	...
	public String getFileNameForQuery(MatrixQueryForClause forClause) {
		MatrixUtil.toLog("SRBQueryMapping-getFileNameForQuery():BEGIN",
							MatrixUtil.DEBUG_LOG);
		String fileName = null;
		fileName = forClause.getFunctionParameter();
		MatrixUtil.toLog("SRBQueryMapping-getFileNameForQuery():fileName:" +
							fileName, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-getFileNameForQuery():END",
							MatrixUtil.DEBUG_LOG);
		return fileName;
	}
	...
	/*
		...
		*/
	private MetaDataCondition createConditionForBetweenOrNotBetweenOperator(
				String variableName, int srbOperator, String conditionValue) {
		MatrixUtil.toLog(
					"SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():BEGIN",
					MatrixUtil.DEBUG_LOG);
		MetaDataCondition betCond = null;
		String[] rangeValues = null;
		int condValue1 = -99999;
		int condValue2 = -99999;
		String tempCondValue = new String(conditionValue.substring(1,
					conditionValue.length() - 1));
		// Parse Range Values
		MatrixUtil.toLog("SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():conditionValue:" +
							conditionValue, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():tempCondValue:" +
					tempCondValue, MatrixUtil.DEBUG_LOG);
		rangeValues = tempCondValue.split("and");
		MatrixUtil.toLog("SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():rangeValues-length:" +
							rangeValues.length, MatrixUtil.DEBUG_LOG);
		for (int i = 0; i < rangeValues.length; i++) {
			MatrixUtil.toLog(
						"SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():rangeValues[" +
						i + "]:" + rangeValues[i], MatrixUtil.DEBUG_LOG);
		...
		betCond = MetaDataSet.newCondition(variableName, srbOperator,
													condValue1, condValue2);
		MatrixUtil.toLog(
					"SRBQueryMapping-createConditionForBetweenOrNotBetweenOperator():END",
					MatrixUtil.DEBUG_LOG);
		return (betCond);
	}
	...
	/*
		...
		*/
	private String[] convertCommaDelimitedStringToStringArray(String
																	valueString) {
		MatrixUtil.toLog(
					"SRBQueryMapping-convertCommaDelimitedStringToStringArray():BEGIN",
					MatrixUtil.DEBUG_LOG);
		...
		if (valueString != null) {
			...
			if (tokenizer != null && tokenizer.countTokens() > 0) {
				...
				MatrixUtil.toLog(
							"SRBQueryMapping-convertCommaDelimitedStringToStringArray():valueList - length" +
							valueList.length, MatrixUtil.DEBUG_LOG);
				int index = 0;
				while (tokenizer.hasMoreTokens()) {
					valueList[index] = tokenizer.nextToken();
					MatrixUtil.toLog(
								"SRBQueryMapping-convertCommaDelimitedStringToStringArray():valueList" +
		...
		MatrixUtil.toLog(
					"SRBQueryMapping-convertCommaDelimitedStringToStringArray():END",
					MatrixUtil.DEBUG_LOG);
		return (valueList);
	}
	...
	/*
		...
		*/
	public MetaDataCondition[] mapMatrixConditionToSRBCondition(
				MatrixQueryWhereClause whereClause) {
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixConditionToSRBCondition():BEGIN",
					MatrixUtil.DEBUG_LOG);
		...
		for (int i = 0; i < conditionClauseList.size(); i++) {
			MatrixQueryConditionClause conditionClause = (
						MatrixQueryConditionClause) conditionClauseList.get(i);
			SRBVariable srbFieldName = mapMatrixConditionVariable(
						conditionClause.getConditionVariable());
			int srbOperator = mapMatrixOperator(conditionClause.getOperator());
			if (srbOperator == MetaDataCondition.IN ||
				srbOperator == MetaDataCondition.NOT_IN) {
				srbValueList = convertCommaDelimitedStringToStringArray(
							conditionClause.getConditionValue().substring(1,
							conditionClause.getConditionValue().length() - 1));
			} else if (srbOperator == MetaDataCondition.BETWEEN ||
								srbOperator == MetaDataCondition.NOT_BETWEEN) {
				betCondition = createConditionForBetweenOrNotBetweenOperator(
							srbFieldName.variableName, srbOperator,
							conditionClause.getConditionValue());
			}
			String srbValue = mapMatrixConditionValue(conditionClause.
															getConditionValue());
			if (srbFieldName.srbSystemVariable) {
				...
				if (srbOperator == MetaDataCondition.IN ||
					srbOperator == MetaDataCondition.NOT_IN) {
					tempCondition = MetaDataSet.newCondition(srbFieldName.
																variableName,
																srbOperator,
																srbValueList);
				} else if (srbOperator == MetaDataCondition.BETWEEN ||
					...
					try {
						...
						tempCondition = MetaDataSet.newCondition(srbFieldName.
																	...
																	paramInt);
						MatrixUtil.toLog(
									"SRBQueryMapping - mapMatrixConditionToSRBCondition() -INT PARAM VALUE",
									MatrixUtil.DEBUG_LOG);
					} catch (Exception e) {
						tempCondition = MetaDataSet.newCondition(srbFieldName.
																	variableName,
																	...
																	srbValue);
						MatrixUtil.toLog(
									"SRBQueryMapping - mapMatrixConditionToSRBCondition() -STRING PARAM VALUE",
									MatrixUtil.DEBUG_LOG);
					}
				}
				srbSystemDefinedConditionList.add(tempCondition);
				MatrixUtil.toLog(
							"SRBQueryMapping - mapMatrixConditionToSRBCondition() - System Condition:" +
							srbFieldName.variableName + conditionClause.getOperator() +
							srbValue, MatrixUtil.DEBUG_LOG);
			} else {
				...
				if (srbOperator == MetaDataCondition.IN ||
					srbOperator == MetaDataCondition.NOT_IN) {
					userDefCond = new SRBUserDefinedCondition(srbFieldName.
																	variableName,
																	srbValueList,
																	srbOperator);
				} else {
				...
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition-userDefVar:" +
									userDefCond.userDefVar, MatrixUtil.DEBUG_LOG);
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition-userDefValue:" +
									userDefCond.userDefValue, MatrixUtil.DEBUG_LOG);
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition-userDefOperator:" +
									userDefCond.userDefOperator,
									MatrixUtil.DEBUG_LOG);
				srbUserDefinedConditionList.add(userDefCond);
				MatrixUtil.toLog(
							"SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition:" +
							srbFieldName.variableName + conditionClause.getOperator() +
							srbValue, MatrixUtil.DEBUG_LOG);
			}
		}
		...
		// System Defined
		if (srbSystemDefinedConditionList != null) {
			for (int index = 0; index < srbSystemDefinedConditionList.size();
					index++) {
				tempCondition = (MetaDataCondition)
							srbSystemDefinedConditionList.get(index);
				srbConditions[index] = tempCondition;
			}
		}
		// User Defined
		if (srbUserDefinedConditionList != null) {
			String[][] definableMetaDataValues = new String[
						srbUserDefinedConditionList.size()][2];
			MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition List Length:" +
								definableMetaDataValues.length,
								MatrixUtil.DEBUG_LOG);
			int[] operators = new int[definableMetaDataValues.length];
			for (int index = 0; index < srbUserDefinedConditionList.size();
					index++) {
				// Condition
				SRBUserDefinedCondition userCond = (SRBUserDefinedCondition)
							srbUserDefinedConditionList.get(index);
				definableMetaDataValues[index][0] = userCond.userDefVar.toString();
				definableMetaDataValues[index][1] = userCond.userDefValue.toString();
				operators[index] = userCond.userDefOperator;
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition :(" +
									index + ")-userDefVar:" + userCond.userDefVar,
									MatrixUtil.DEBUG_LOG);
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition :(" +
									index + "):-userDefValue:" +
									userCond.userDefValue, MatrixUtil.DEBUG_LOG);
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition() - User Defined Condition :(" +
									index + "):-userDefOperator:" +
									userCond.userDefOperator, MatrixUtil.DEBUG_LOG);
			}
			MetaDataTable metaDataTable = new MetaDataTable(operators,
						definableMetaDataValues);
			tempCondition = MetaDataSet.newCondition(SRBMetaDataSet.
						DEFINABLE_METADATA_FOR_FILES, metaDataTable);
			srbConditions[srbConditions.length - 1] = tempCondition;
		}
		MatrixUtil.toLog("SRBQueryMapping - mapMatrixConditionToSRBCondition()-FINAL Condition List Length :" +
							srbConditions.length, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixConditionToSRBCondition():END",
					MatrixUtil.DEBUG_LOG);
	...
	public SRBVariable mapMatrixConditionVariable(String variable) {
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixConditionVariable():BEGIN",
							MatrixUtil.DEBUG_LOG);
		SRBVariable srbConditionVar = null;
		String srbVariable = null;
		MatrixUtil.toLog(
					"SRBQueryMapping - mapMatrixConditionVariable() - variable:" +
					variable, MatrixUtil.DEBUG_LOG);
		int indexOfLastFSlash = variable.lastIndexOf('/');
		srbVariable = variable.substring(indexOfLastFSlash + 1, variable.length());
		MatrixUtil.toLog(
					"SRBQueryMapping - mapMatrixConditionVariable() - srbVariable:" +
					srbVariable, MatrixUtil.DEBUG_LOG);
		srbConditionVar = mapMatrixVariableToSRBVariable(srbVariable);
		MatrixUtil.toLog(
					"SRBQueryMapping - mapMatrixConditionVariable() - srbConditionVar:" +
					srbConditionVar.variableName, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixConditionVariable():BEGIN",
							MatrixUtil.DEBUG_LOG);
	...
	public int mapMatrixOperator(String operator) {
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixOperator():BEGIN",
							MatrixUtil.DEBUG_LOG);
		int srbOperator = 0;
		if (operator != null) {
			operator = operator.trim();
		}
		MatrixUtil.toLog("SRBQueryMapping - mapMatrixOperator() - operator:" +
		...
		MatrixUtil.toLog("SRBQueryMapping - mapMatrixOperator() - srbOperator:" +
							srbOperator, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixOperator():END",
							MatrixUtil.DEBUG_LOG);
		return srbOperator;
	}
	...
	public String mapMatrixConditionValue(String condValue) {
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixConditionValue():BEGIN",
							MatrixUtil.DEBUG_LOG);
		...
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixConditionValue():srbCondValue:" +
					srbCondValue, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixConditionValue():END",
							MatrixUtil.DEBUG_LOG);
		return srbCondValue;
	}
	...
	/*
		...
		*/
	public MetaDataSelect[] mapMatrixReturnClauseToSRBSelection(
				MatrixQueryReturnClause returnClause) {
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixReturnClauseToSRBSelection():BEGIN",
					MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixReturnClauseToSRBSelection():returnClause:" +
					returnClause.getReturnExpression(), MatrixUtil.DEBUG_LOG);
		...
		ArrayList fieldNamesForReturnClause = returnClause.
					getSelectionFieldNames();
		...
		if (fieldNamesForReturnClause != null) {
			for (int count = 0; count < fieldNamesForReturnClause.size(); count++) {
				...
				fieldName = fieldName.substring(indexOfLastFSlash + 1,
												fieldName.length());
				...
				if (fieldName.equals("USER_DEFINED_ATTRIBUTES_NAMES") ||
					fieldName.equals("USER_DEFINED_ATTRIBUTES_VALUES") ||
					fieldName.equals("USER_DEFINED_ATTRIBUTES_UNITS")) {
					queryForAllAVUPairs = queryForAllAVUPairs || true;
					String resultTemplate = returnClause.getReturnExpression();
					MatrixUtil.toLog("SRBQueryMapping - mapMatrixReturnClauseToSRBSelection() - resultTemplate BEFORE:" +
										resultTemplate, MatrixUtil.DEBUG_LOG);
					resultTemplate = replaceVariableWithValueInResultString(
								matrixFieldName, fieldName, resultTemplate);
					returnClause.setReturnExpression(resultTemplate);
					MatrixUtil.toLog("SRBQueryMapping - mapMatrixReturnClauseToSRBSelection() - resultTemplate AFTER:" +
										resultTemplate, MatrixUtil.DEBUG_LOG);
				}
				MatrixUtil.toLog("SRBQueryMapping - mapMatrixReturnClauseToSRBSelection() - ###### queryForAllAVUPairs :" +
									queryForAllAVUPairs + ", fieldName:" +
				...
				MatrixUtil.toLog(
							"SRBQueryMapping - mapMatrixReturnClauseToSRBSelection() - srbField:" +
							srbField.variableName + ", matrixFieldName:" +
							matrixFieldName, MatrixUtil.DEBUG_LOG);
				srbFieldNameToJargonFieldName.put(srbField.variableName,
														matrixFieldName);
			}
		}
		...
		// Copy System Fields
		if (tempListOfSelectionForSystem != null) {
			for (int count = 0; count < tempListOfSelectionForSystem.size();
					count++) {
		...
		if (tempListOfSelectionForUserDefined != null) {
			selectFieldNames[selectFieldNames.length -
						1] = new String(SRBMetaDataSet.DEFINABLE_METADATA_FOR_FILES);
		}
		MatrixUtil.toLog("SRBQueryMapping - mapMatrixReturnClauseToSRBSelection - selection field count :" +
							selectFieldNames.length, MatrixUtil.DEBUG_LOG);
		...
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixReturnClauseToSRBSelection():END",
					MatrixUtil.DEBUG_LOG);
		return selectArray;
	}
	...
	public SRBVariable mapMatrixVariableToSRBVariable(String variable) {
		MatrixUtil.toLog(
					"SRBQueryMapping-mapMatrixVariableToSRBVariable():BEGIN",
					MatrixUtil.DEBUG_LOG);
		String srbVariableName = null;
		boolean variableMapped = false;
		MatrixUtil.toLog(
					"SRBQueryMapping - mapMatrixVariableToSRBVariable() - variable:" +
					variable, MatrixUtil.DEBUG_LOG);
		for (int variableIndex = 0; variableIndex < srbVariableMapping.length;
				variableIndex++) {
		...
		SRBVariable srbVariable = new SRBVariable(srbVariableName,
														variableMapped);
		MatrixUtil.toLog(
					"SRBQueryMapping - mapMatrixVariableToSRBVariable() - srbVariableName:" +
					srbVariableName, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapMatrixVariableToSRBVariable():END",
							MatrixUtil.DEBUG_LOG);
		return srbVariable;
	}
	...
	/*
		...
		*/
	public String mapSRBQueryResultToMatrixQueryResult(MetaDataRecordList[] rl,
																MatrixQueryReturnClause
																returnClause) {
		MatrixUtil.toLog(
					"SRBQueryMapping-mapSRBQueryResultToMatrixQueryResult():BEGIN",
					MatrixUtil.DEBUG_LOG);
		printQueryResults(rl);
		String resultString = new String("");
		String resultTemplate = returnClause.getReturnExpression();
		MatrixUtil.toLog(
					"SRBQueryMapping-mapSRBQueryResultToMatrixQueryResult():resultTemplate:" +
					resultTemplate, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping-mapSRBQueryResultToMatrixQueryResult():queryReturnExpression:" +
					queryReturnExpression, MatrixUtil.DEBUG_LOG);
		try {
			...
			if (keySet != null) {
				...
				if (keyIterator != null) {
					while (keyIterator.hasNext()) {
						...
						String value = (String) srbFieldToMatrixFieldMapping.
									get(keyStr);
						MatrixUtil.toLog(
									"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-Key :" +
									keyStr + ", Value :" + value, MatrixUtil.DEBUG_LOG);
					}
				} else {
					MatrixUtil.toLog(
								"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-keyIterator is NULL",
								MatrixUtil.DEBUG_LOG);
				}
			} else {
				MatrixUtil.toLog(
							"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-keySet is NULL",
							MatrixUtil.DEBUG_LOG);
			}
			if (rl != null) {
				...
				for (int i = 0; i < rl.length; i++) {
					if (!queryForAllAVUPairs) {
						resultString +=
									mapResultForSingleResult(resultTemplate, rl[i]);
					} else {
						resultString +=
									mapResultForAllAVUPairs(resultTemplate, rl[i]);
					}
				}
			} else { // No Results
			...
			MatrixUtil.toLog(
						"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-resultString:" +
						resultString, MatrixUtil.DEBUG_LOG);
			MatrixUtil.toLog(
						"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-END",
						MatrixUtil.DEBUG_LOG);
			// return (resultString);
		} catch (Exception e) {
			MatrixUtil.toLog(
						"SRBQueryMapping - mapSRBQueryResultToMatrixQueryResult()-EXCEPTION:" +
						e, MatrixUtil.DEBUG_LOG);
		}
		return (resultString);
	}

	private String mapResultForAllAVUPairs(String resultStringTemplate,
													MetaDataRecordList resultRecord) {
		MatrixUtil.toLog("SRBQueryMapping-mapResultForAllAVUPairs():BEGIN",
							MatrixUtil.DEBUG_LOG);
		...
		for (int j = 0; j < resultRecord.getRecordCount(); j++) {
			...
			MatrixUtil.toLog(
						"SRBQueryMapping-mapResultForAllAVUPairs():RECORD:" + j,
						MatrixUtil.DEBUG_LOG);
			if (resultRecord.getFieldType(j) == MetaDataField.TABLE) {
				MatrixUtil.toLog(
							"SRBQueryMapping-mapResultForAllAVUPairs():MetaDataField.TABLE",
							MatrixUtil.DEBUG_LOG);
				MetaDataTable table = resultRecord.getTableValue(j);
				if (table != null) {
					if (table.getRowCount() > 0) {
						userDefinedMetaDataValues = new String[table.
				...
				for (int k = 0; k < table.getRowCount(); k++) {
					for (int l = 0; l < table.getColumnCount(); l++) {
						MatrixUtil.toLog(
									"SRBQueryMapping-mapResultForAllAVUPairs():Table Value(" +
									k + "," + l + "):" + table.getStringValue(k, l) +
									"\t", MatrixUtil.DEBUG_LOG);
						//if((l==0)||(l==1))
						{
							userDefinedMetaDataValues[k][l] = table.
										getStringValue(k, l);
						}
					}
				}
				...
				if (userDefinedMetaDataValues != null) {
					for (int index = 0;
							index < userDefinedMetaDataValues.length; index++) {
						Object matrixFieldObject = srbFieldToMatrixFieldMapping.
									get(userDefinedMetaDataValues[index][0]);
						...
						if (attrObject != null || valueObject != null ||
							unitObject != null) {
							template = new String(resultStringTemplate);
							template = replaceVariableWithValueInResultString(
										"{USER_DEFINED_ATTRIBUTES_NAMES}", attrString,
										template);
							template = replaceVariableWithValueInResultString(
										"{USER_DEFINED_ATTRIBUTES_VALUES}", valueString,
										template);
							template = replaceVariableWithValueInResultString(
										"{USER_DEFINED_ATTRIBUTES_UNITS}", unitString,
										template);
		...
		MatrixUtil.toLog(
					"SRBQueryMapping - mapResultForAllAVUPairs()-resultStringForRecord:" +
					resultStringForRecord, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapResultForAllAVUPairs():END",
							MatrixUtil.DEBUG_LOG);
	...
	/*
		...
		*/
	public String mapResultForSingleResult(String resultStringTemplate,
													MetaDataRecordList resultRecord) {
		MatrixUtil.toLog("SRBQueryMapping-mapResultForSingleResult():BEGIN",
							MatrixUtil.DEBUG_LOG);
		String resultStringForRecord = resultStringTemplate;
		HashMap fieldMapping = srbFieldToMatrixFieldMapping;
		String userDefinedMetaDataValues[][] = null;
		MatrixUtil.toLog(
					"SRBQueryMapping-mapResultForSingleResult():resultStringForRecord:" +
					resultStringForRecord, MatrixUtil.DEBUG_LOG);
		for (int j = 0; j < resultRecord.getRecordCount(); j++) {
			// Extract user defined value
			// Map to temp table
			// Use table for final result creation
			MatrixUtil.toLog(
						"SRBQueryMapping-mapResultForSingleResult():RECORD:" + j,
						MatrixUtil.DEBUG_LOG);
			if (resultRecord.getFieldType(j) == MetaDataField.TABLE) {
				MatrixUtil.toLog(
							"SRBQueryMapping-mapResultForSingleResult():MetaDataField.TABLE",
							MatrixUtil.DEBUG_LOG);
				MetaDataTable table = resultRecord.getTableValue(j);
				if (table != null) {
					if (table.getRowCount() > 0) {
				...
				for (int k = 0; k < table.getRowCount(); k++) {
					for (int l = 0; l < table.getColumnCount(); l++) {
						MatrixUtil.toLog(
									"SRBQueryMapping-mapResultForSingleResult():Table Value(" +
									k + "," + l + "):" + table.getStringValue(k, l) +
									"\t", MatrixUtil.DEBUG_LOG);
						if ( (l == 0) || (l == 1)) {
							userDefinedMetaDataValues[k][l] = table.
				...
				if (userDefinedMetaDataValues != null) {
					for (int index = 0;
						...
						if (matrixFieldObject != null) {
							...
							String matrixFieldValue = userDefinedMetaDataValues[
										index][1];
							//Since the resultTemplate has the variables embedded in Curly Braces {}
							matrixFieldName = "{" + matrixFieldName + "}";
							resultStringForRecord =
										replaceVariableWithValueInResultString(
										matrixFieldName, matrixFieldValue,
										resultStringForRecord);
						}
					}
				}
				...
				MatrixUtil.toLog(
							"SRBQueryMapping-mapResultForSingleResult():SYSTEM LEVEL METADATA",
							MatrixUtil.DEBUG_LOG);
				// Field Name fro SRB
				String srbFieldName = resultRecord.getFieldName(j);
				MatrixUtil.toLog(
							"SRBQueryMapping-mapResultForSingleResult():srbFieldName:" +
							srbFieldName, MatrixUtil.DEBUG_LOG);
				//	Mapping to Variable in Result Template
				Object matrixFieldObject = srbFieldToMatrixFieldMapping.get(
							srbFieldName);
				if (matrixFieldObject != null) {
					MatrixUtil.toLog(
								"SRBQueryMapping-mapResultForSingleResult():SRB FIELD FOUND IN FIELD MAP",
								MatrixUtil.DEBUG_LOG);
					String matrixFieldName = (String) matrixFieldObject;
					...
					matrixFieldName = "{" + matrixFieldName + "}";
					resultStringForRecord =
								replaceVariableWithValueInResultString(matrixFieldName,
																	matrixFieldValue,
								resultStringForRecord);
				} else {
					MatrixUtil.toLog(
								"SRBQueryMapping-mapResultForSingleResult():SRB FIELD NOT FOUND IN FIELD MAP",
								MatrixUtil.DEBUG_LOG);
				}
			}
		}
		// Replace non-filled fields with empty space
		resultStringForRecord = processResultStringToRemoveUnfoundResults(
					resultStringForRecord);
		MatrixUtil.toLog(
					"SRBQueryMapping - mapResultForSingleResult()-resultStringForRecord:" +
					resultStringForRecord, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping-mapResultForSingleResult()-END",
							MatrixUtil.DEBUG_LOG);
		return (resultStringForRecord);
	}
	...
	public String processResultStringToRemoveUnfoundResults(String resultString) {
		MatrixUtil.toLog(
					"SRBQueryMapping - processResultStringToRemoveUnfoundResults()-BEGIN",
					MatrixUtil.DEBUG_LOG);
		...
		if (keySet != null) {
			...
			if (keyIterator != null) {
				while (keyIterator.hasNext()) {
					String keyStr = (String) keyIterator.next();
					String value = (String) srbFieldToMatrixFieldMapping.get(
								keyStr);
					MatrixUtil.toLog(
								"SRBQueryMapping - processResultStringToRemoveUnfoundResults()-Key :" +
								keyStr + ", Value :" + value, MatrixUtil.DEBUG_LOG);
					//Since the resultTemplate has the variables embedded in Curly Braces {}
					value = "{" + value + "}";
					formattedResultString =
								replaceVariableWithValueInResultString(value, "",
								formattedResultString);
				}
			} else {
				MatrixUtil.toLog("SRBQueryMapping - processResultStringToRemoveUnfoundResults()-keyIterator is NULL",
									MatrixUtil.DEBUG_LOG);
			}
		}
		MatrixUtil.toLog("SRBQueryMapping - processResultStringToRemoveUnfoundResults()-formattedResultString:" +
							formattedResultString, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping - processResultStringToRemoveUnfoundResults()-END",
					MatrixUtil.DEBUG_LOG);
	...
	/*
		...
		*/
	public String replaceVariableWithValueInResultString(String resultVariable,
															String resultValue,
															String resultTemplate) {
		MatrixUtil.toLog(
					"SRBQueryMapping-replaceVariableWithValueInResultString():BEGIN",
					MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping - replaceVariableWithValueInResultString()-resultVariable:" +
					resultVariable, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping - replaceVariableWithValueInResultString()-resultValue:" +
					resultValue, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping - replaceVariableWithValueInResultString()-resultTemplate:" +
					resultTemplate, MatrixUtil.DEBUG_LOG);
		...
		if (resultVariable == null || resultValue == null || resultTemplate == null) {
			...
			if (tokenPos == -1) {
				...
				String end = resultTemplate.substring(tokenPos +
															resultVariable.length());
				resultString = start + resultValue + end;
			}
		}
		MatrixUtil.toLog(
					"SRBQueryMapping - replaceVariableWithValueInResultString()-resultString:" +
					resultString, MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog(
					"SRBQueryMapping - replaceVariableWithValueInResultString()-END",
					MatrixUtil.DEBUG_LOG);
		return resultString;
	}
	...
	public void printQueryResults(MetaDataRecordList[] rl) {
		//print query results
		MatrixUtil.toLog("SRBQueryMapping - printQueryResults()-BEGIN",
							MatrixUtil.DEBUG_LOG);
		if (rl != null) { //Nothing in the database matched the query
			...
			for (int i = 0; i < rl.length; i++) {
				...
				for (int j = 0; j < rl[i].getRecordCount(); j++) {
					MatrixUtil.toLog(rl[i].getFieldName(j) + "\t",
										MatrixUtil.DEBUG_LOG);
				}
				System.out.print("\n");
				for (int j = 0; j < rl[i].getRecordCount(); j++) {
					if (rl[i].getFieldType(j) == MetaDataField.TABLE) {
						...
						MatrixUtil.toLog(rl[i].getStringValue(j) + "\t",
											MatrixUtil.DEBUG_LOG);
					}
				}
			}
		} else {
			MatrixUtil.toLog("SRBQueryMapping - printQueryResults()-NO RESULTS",
								MatrixUtil.DEBUG_LOG);
		}
		MatrixUtil.toLog("\n", MatrixUtil.DEBUG_LOG);
		MatrixUtil.toLog("SRBQueryMapping - printQueryResults()-END",
							MatrixUtil.DEBUG_LOG);
	}
}
