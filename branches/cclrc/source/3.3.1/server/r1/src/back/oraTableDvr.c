...
int
oraTableOpen(dbTableDesc *exfd, char *resourceLocationName,
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "oraTableOpen: Error in resourceLocationName : %s",
    ...
    if ((i =  open_env_oraTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "oraTableOpen: Error in Opening Environment: %i",i);
      oraTableErrorReport(NULL, (Lda_Def *) stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_oraTab( (long int)  stateInfo->environ, 
			   (long int *) &stateInfo->connect, 
			   (unsigned char*) database, 
			   (unsigned char*) uid, 
			   (unsigned char*) pwd))!= 0)  { 

      elog (NOTICE, "oraTableOpen: Error in Connecting to %s: %i",
	    database,i);
      oraTableErrorReport(NULL, (Lda_Def *) stateInfo->environ);
      close_env_oraTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if (i != 0) {
      elog (NOTICE, "oraTableOpen: Error in Extracting Options from Command %s: %i",
	    dataPathName,i);
      oraTableErrorReport(NULL, (Lda_Def *) stateInfo->environ);
      close_env_oraTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
	...
	if ((tmpPtr2 = strstr(tmpPtr, " ")) == NULL &&
	  ...
	  exfd->dbSpecificInfo = (char *)stateInfo; 
	  i = getColumnInfoFororaTable((long int)stateInfo->environ, 
	...
	*tmpPtr2 = tmpChar;
	...
	if ((tmpPtr3 = strstr(tmpPtr2, ")")) == NULL) {
	  elog (NOTICE, "oraTableOpen: Insert Statement Format Error: %s",
		dataPathName);
	  disconnect_oraTab( (long int)stateInfo->environ, 
	...
	*tmpPtr3 = '\0';
	...
	for (i = 0; i < stateInfo->num_of_cols; i++) {
	  ...
	  stateInfo->xmlcolNameOpen[i] = NULL;
	  stateInfo->xmlcolNameClose[i] = NULL;
	  if (getColTypeFromColNameoraTab((long int)stateInfo->environ, 
				(long int)stateInfo->connect,
	...
	if ((tmpPtr = strstr(dataPathName," from ")) == NULL &&
	  ...
	  elog (NOTICE, "oraTableOpen: Delete Statement Format Error: %s",
		dataPathName);
	  disconnect_oraTab( (long int)stateInfo->environ, 
			     (long int)stateInfo->connect, MDAS_CAT_ROLLBACK);
	  close_env_oraTab( (long int) stateInfo->environ); 
	...
	*tmpPtr3 = '\0';
	...
	if (i < 0) {
	  elog (NOTICE, "oraTableOpen:Error: %i in  getMultiInsertInformation");
	  close_env_oraTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return(i);
	}
	if ((stateInfo->storedSQLBuffer = 
	  ...
	  elog (NOTICE, "oraTableOpen:Error: Memory Allocation Error");
	  close_env_oraTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return INP_ERR_FORMAT;
	}
	...
	return MDAS_SUCCESS;
        ...
        elog (NOTICE, "oraTableOpen:Error: Not a Known Type SQL Statement: %s",
	      dataPathName);
	close_env_oraTab( (long int) stateInfo->environ); 
	free(stateInfo); stateInfo = NULL;
	return INP_ERR_FORMAT;
	...
	strstr(dataPathName,"SELECT") != dataPathName) {
      ...
      if ((i=exec_no_res_sql_oraTab(stateInfo->environ,stateInfo->connect,
			(unsigned char *) dataPathName))!= 0) { 
	elog (NOTICE, "oraTableOpen: Error in Executing SQL %s: %i",
	      dataPathName,i);
	oraTableErrorReport(NULL, (Lda_Def *) stateInfo->environ);
	disconnect_oraTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
    ...
    addToVarList(&(stateInfo->constantValues), "QT","UPDATE", "CHAR");
    if ((i=exec_sql_oraTab(stateInfo->environ,stateInfo->connect,
    ...
    if ((i= get_no_of_columns_oraTab(stateInfo->statement, 
				  &numCols))!= 0) {
      elog (NOTICE, "oraTableOpen: Error in Getting Num of Cols: %i",i);
      oraTableErrorReport((Cda_Def *) stateInfo->statement, 
       (Lda_Def *) stateInfo->environ);
      disconnect_oraTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_oraTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i = bind_default_row_oraTab((long int) stateInfo->statement, 
		   (SQLCHAR **) stateInfo->data,  
		   (SQLINTEGER *) stateInfo->data_size, (SQLSMALLINT *) &numCols)) != 0) {
      elog (NOTICE, "oraTableOpen: Error in Binding Row %i: %i",ii,i);
      oraTableErrorReport((Cda_Def *) stateInfo->statement, 
       (Lda_Def *) stateInfo->environ);
      disconnect_oraTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
    ...
    for (j=0; j < numCols; j++) {
      ...
      myprecision = 1;
      scale = -1;
      if ((i = get_column_info_oraTab((long int) stateInfo->statement,j+1, 
				   &displaysize,mycolname,
				   &coltype, buf_size, &mycolnamelen,
				   &nullable,
				   &myprecision,&scale)) != 0) {
	elog (NOTICE, 
	      "oraTableOpen: Error in Getting Column Information %i: %i",ii,i);
	oraTableErrorReport((Cda_Def *)stateInfo->statement, 
         (Lda_Def *)stateInfo->environ);
	done_with_default_result_oraTab((long int)stateInfo->statement, 
				     (SQLCHAR **) stateInfo->data,  
				     (SQLINTEGER *) stateInfo->data_size, 
	...
	return(i);
	  ...
	  (char *)malloc(sizeof(char) * (strlen((char *)mycolname)+1)))  == NULL) {
	elog (NOTICE, "oraTableOpen: Malloc error");
	done_with_default_result_oraTab((long int)stateInfo->statement, 
				     (SQLCHAR **)stateInfo->data,  
				     (SQLINTEGER *)stateInfo->data_size, 
...
int 
oraTableClose(dbTableDesc  *exfd)
{
  tableStateInfo *stateInfo;
  int i,ii;
  ...
  if (!strcmp(stateInfo->sqlType, "SELECT") &&
      ...
      stateInfo->num_of_cols > 0) {
    if ((i = done_with_default_result_oraTab((long int)stateInfo->statement, 
		       (SQLCHAR **)stateInfo->data,  
		       (SQLINTEGER *)stateInfo->data_size, stateInfo->num_of_cols)) != 0) {
      elog (NOTICE, "oraTableClose: Error in Unbinding and Closing %i",i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      return(i);
    }    
  }
  if (!strcmp(stateInfo->sqlType, "MULTIINSERT")) {
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "oraTableClose: Error in Constructing SQL To Clean List: %i",ii);
	    return(ii);
	}
	if (ii == 1) {
  ...
  if ((i = disconnect_oraTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_COMMIT)) != 0) {
    elog (NOTICE, "oraTableClose: Error in Disconnecting:%i",i);
    oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
    return(DB_TAB_DISCONNECT_ERROR);
  }
  if ((i = close_env_oraTab((long int)stateInfo->environ)) != 0) {
    elog (NOTICE, "oraTableClose: Error in Closing Environment:%i",i);
    oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
...
int 
oraTableRead(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(oraTableReadXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(oraTableReadXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(oraTableReadHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(oraTableReadHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(oraTableReadHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(oraTableReadTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(oraTableReadPACK(exfd,buffer,length));
...
oraTableReadTEMPLATE(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  while ((i = get_row_oraTab((long int) stateInfo->statement)) == 0)       
    {
      stateInfo->num_of_rows_read++;
      for (ii =  0; ii < stateInfo->num_of_cols ; ii++) {
	truncate_end_blanks(stateInfo->data_size[ii], (unsigned char *)stateInfo->data[ii]);
      }
...
oraTableReadHTMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  if ((i = get_row_oraTab((long int) stateInfo->statement)) != 0) {
    stateInfo->endOfData = 1;
    if (i == MCAT_NO_DATA_FOUND){
  ...
  while (i == 0)       
	...
	if (ii == stateInfo->num_of_cols - 1) 
	  k +=strlen("</TR>\n");
	tt  = 0;
	storPtr = (char *)malloc(sizeof(char) * (k+1));
	if (ii == 0) {
	  sprintf(storPtr,"<TR>\n");
	  tt += strlen("<TR>\n");
	}
	...
	if (k <= ll) {
	  ...
	  if (ii != stateInfo->num_of_cols - 1) 
	    k += strlen("</TR>\n");
	  stateInfo->mm_ptr =  (char *)malloc(sizeof(char) * (k+1));
	  stateInfo->mm_size = k;
	  mybuf = stateInfo->mm_ptr;
      ...
      i = get_row_oraTab((long int) stateInfo->statement);
    }
  stateInfo->endOfData = 1;
  if (i == MCAT_NO_DATA_FOUND) {
    strcpy(fillStr,"</TABLE>\n</BODY>\n</HTML>\n");
...
int 
oraTableReadHEAD(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
oraTableReadXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
oraTableWrite(dbTableDesc *exfd, char *buffer, int length) 
{
/*
 ...
 */
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(oraTableWriteXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(oraTableWriteXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(oraTableWriteHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(oraTableWriteHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(oraTableWriteHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(oraTableWriteTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "SQL"))
    return(oraTableWriteSQL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(oraTableWritePACK(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "VALUES"))
    return(oraTableWriteVALUES(exfd,buffer,length));
  else
    return(oraTableWriteSQL(exfd,buffer,length));
}
...
oraTableWriteSQL(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
  ...
  if (mybuf == NULL)  {
    elog (NOTICE, "oraTableWrite: Memory Allocation Error");
    return(DB_ERR_MALLOC);
    }
...
oraTableWriteVALUES(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
 ...
 int mmSize;
  ...
  if (stateInfo->mm_size > 0 ) {
    collapsePtr = (char *) malloc(mmSize + length+1);
    if (collapsePtr == NULL)  {
      elog (NOTICE, "oraTableWrite: Memory Allocation Error");
      return(DB_ERR_MALLOC);
    }
  ...
  while (*mybuf1 != '\0') {
    ...
    *mybuf2 = '\0';
    ...
    if ((i=exec_no_res_sql_oraTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "oraTableWrite: Error in Executing SQL %s: %i",
	    sqlString,i);
      free(mybuf);
      free(sqlString);
      return(i);
...
oraTableWritePACK(dbTableDesc *exfd, char *buffer, int length) 
{
  ...
  if (mmSize > 0) {
	...
	tmpPtr = (char *) malloc(stateInfo->mm_size + length+1);
...
oraTableWriteXMLTREE(dbTableDesc *exfd, char *inbuffer, int length)
{
  ...
  while (1) {
    for (cntData = 0 ; cntData < stateInfo->num_of_cols ; cntData++) {
      ...
      if (i == DATA_TAG_ERROR) {
	elog (NOTICE, "oraTableWrite: Error in Data Tag for Buffer: %s", mybuf);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
	...
	*(tmpmmPtr+mmSize - 1 ) =  '\0';
	...
	elog (NOTICE, "oraTableWrite: Error in Data Tag: %s", dataTagOpen);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
...
oraTableWriteTEMPLATE(dbTableDesc *exfd, char *inbuffer, int length)
{
 ...
 if (!strcmp(stateInfo->sqlType,"MULTIINSERT")) {
  while (1) {
	...
	elog (NOTICE, "oraTableWrite: Error in Processing Choice Rule: %i", j);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(j);
      }
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "oraTableWrite: Error in Constructing SQL  in List: %i",ii);
	    if (collapsePtr != NULL) free(collapsePtr);
	    collapsePtr = NULL;
	    return(ii);
	}
	...
	for (j = 0 ;  j < MAX_SQL_TAGS_COUNT ; j++) {
	  ...
	  if (!strcmp(stateInfo->compSql.tags[i][j], dataTag)) {
	    if (stateInfo->compSql.tagusage[i][j] == TAG_FULL) {
		  ...
		  < 0) {
		elog (NOTICE, 
		   "oraTableWrite: Error in Constructing SQL '%i' in List: %i",
		      i ,ii);
		if (collapsePtr != NULL) free(collapsePtr);
		collapsePtr = NULL;
		return(ii);
  ...
  while (1) {
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (!strcmp(dataTag,stateInfo->xmlcolName[i])) 
      ...
      if (i == stateInfo->num_of_cols) {
	elog (NOTICE, "oraTableWrite: Error in Data Tag: %s", dataTag);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	for (i = 0; i <= cntData; i++) {
	  if (stateInfo->data[i] != emptyStr)
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "oraTableCreate: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if(strstr(tableCreateStmt,"create") == NULL &&
       strstr(tableCreateStmt,"CREATE") == NULL) {
        elog (NOTICE, "oraTableCreate: Error: Not a Create Statement : %s",
        tableCreateStmt);
        return INP_ERR_RES_FORMAT;
    }

    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "oraTableCreate: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_oraTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "oraTableCreate: Error in Creating Environment: %i",i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_oraTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "oraTableCreate: Error in Connecting to %s: %i",
	    database,i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      close_env_oraTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_oraTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "oraTableCreate: Error in Executing SQL %s: %i",
	    tableCreateStmt,i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      disconnect_oraTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "oraTableUnlink: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "oraTableUnlink: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_oraTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "oraTableUnlink: Error in Unlinking Environment: %i",i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_oraTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "oraTableUnlink: Error in Connecting to %s: %i",
	    database,i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      close_env_oraTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_oraTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "oraTableUnlink: Error in Executing SQL %s: %i",
	    queryString,i);
      oraTableErrorReport(NULL, (Lda_Def *)stateInfo->environ);
      disconnect_oraTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
...
executeCompoundSql(tableStateInfo *stateInfo, char *sqlbuf)
{
  ...
  if (reverseExecution(stateInfo)) {
	...
	break;
      myChar = *mybuf2;
      *mybuf2 = '\0';
      if ((i=exec_no_res_sql_oraTab(environ, connect,
