...
clConnect(char *srbHost, char* srbPort, char* srbAuth)
{
    ...
    status = clSetAuthScheme (conn, myMdasEnvData, NULL, NULL);
    if (status < 0) {
	conn->status = status;
	if (myMdasEnvData != NULL)
	    free (myMdasEnvData);
...
connectSvr(srbConn *conn)
{
    ...
    /* Authenticate as required*/
        ...
        if ((status = clSendAuth(conn)) != STATUS_OK) {
          return status;
        }
    }
...
connectPort (srbConn *conn)
{
    ...
    port->sock = portalConnectWithRaddr (atoi(conn->srbPort), &port->raddr);
    if (port->sock < 0) {
        fprintf(stderr,
          "connectPort: connect to host %s on port %s failed, errno = %d\n",
    ...
    if (port->sock <= 0) {
        (void) sprintf(conn->errorMessage,
           "connectPort() -- socket() failed: sock=%d, errno=%d\n%s\n",
    ...
    if (status < 0) {
	(void) sprintf(conn->errorMessage,
	"connectPort: Connect() failed to host %s and port %s with Errno=%d.\n",
	 conn->srbHost,conn->srbPort, errno);
#ifdef _WIN32
    ...
    /* fill in the server address. This is for case where the conn->srbHost
     * is not a real host but an address that select a host from a pool
     * of hosts */
    if (getpeername(port->sock, (struct sockaddr *) &port->raddr,
        &laddrlen) < 0) {
        fprintf(stderr,
          "connectPort() -- getpeername() failed: errno=%d\n%s\n",
...
#endif /* WIN32 */
	fprintf(stderr,
	  "connectPort() -- getsockname() failed: errno=%d\n%s\n",
	    errno, strerror(errno));
	if (UserClass == CLASS_SER) {        
	    return SVR_TO_SVR_CONNECT_ERROR;
	} else {
    ...
    if (result < 0) {
        fprintf (stderr,
         "Error: setsockopt:TCP_NODELAY failed to set: %d\n",result);
    }
...
#endif
        fprintf (stderr,
	   "connectPort() --  couldn't send header packet: errno=%d\n%s\n", 
	    errno,strerror(errno));
    ...
    if (status <= 0) {
	fprintf (stderr,
          "connectPort() --  couldn't recv status1 packet: errno=%d\n%s\n",
            errno, strerror(errno));
	return CLI_ERR_RECV;
    }
    ...
    if (svrPortNum < 0) {
	fprintf (stderr,
          "connectPort() --  Connection Error from server: status=%d\n",
            svrPortNum);
        return (svrPortNum);
...
#ifdef foo	/* replaced with connectWithTimeout */
        port->sock = portalConnect (svrPortNum, conn->srbHost);
#endif
        port->sock = portalConnectWithRaddr (svrPortNum, &port->raddr);

        if (port->sock < 0) {
	    fprintf (stderr,
              "connectPort(): Connect to srbServer: %s:%d failed, errno=%d\n",
                conn->srbHost, svrPortNum, errno);
            return (port->sock);
        }
    }

    /* If svrPortNum == 0 or NO_NEW_CONNECTION, no new connection */
...
#endif /* WIN32 */
	fprintf (stderr,
           "connectPort() --  couldn't send complete packet: errno=%d\n%s\n",
            errno,strerror(errno));
        return CLI_ERR_SEND_PACKET;
    }
    ...
    if (status <= 0) {
	fprintf (stderr,
          "connectPort() --  couldn't recv status2 packet: errno=%d\n%s\n",
            errno,strerror(errno));
        return CLI_ERR_RECV;
    }
...
char *domainName, char *authScheme,  char *serverDn)
{
    ...
    ***/
    ...
    status = clSetAuthScheme (conn, myMdasEnvData, authScheme, serverDn);
    if (status < 0) {
	conn->status = status;
	if (myMdasEnvData != NULL)
	    free (myMdasEnvData);
...
clFileLock (char *filePath, int lockCmd, int lockType, int lockFd)
{
    ...
    if (lockType != F_UNLCK) {
#ifdef _WIN32
        myLockFd = SrbNtFileBinaryOpen(filePath, O_RDWR);
#else
