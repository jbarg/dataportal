...
int
mysTableOpen(dbTableDesc *exfd, char *resourceLocationName,
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "mysTableOpen: Error in resourceLocationName : %s",
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "mysTableOpen: Malloc error");
    ...
    if ((i =  open_env_mysTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "mysTableOpen: Error in Opening Environment: %i",i);
      mysTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_mysTab( (long int)  stateInfo->environ, 
			   (long int *) &stateInfo->connect, 
			   (unsigned char*) database, 
			   (unsigned char*) uid, 
			   (unsigned char*) pwd))!= 0)  { 

      elog (NOTICE, "mysTableOpen: Error in Connecting to %s: %i",
	    database,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      close_env_mysTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if (i != 0) {
      elog (NOTICE, "mysTableOpen: Error in Extracting Options from Command %s: %i",
	    dataPathName,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      close_env_mysTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
	...
	if ((tmpPtr2 = strstr(tmpPtr, " ")) == NULL &&
	  ...
	  exfd->dbSpecificInfo = (char *)stateInfo; 
	  i = getColumnInfoFormysTable((long int)stateInfo->environ, 
	...
	*tmpPtr2 = tmpChar;
	...
	if ((tmpPtr3 = strstr(tmpPtr2, ")")) == NULL) {
	  elog (NOTICE, "mysTableOpen: Insert Statement Format Error: %s",
		dataPathName);
	  disconnect_mysTab( (long int)stateInfo->environ, 
	...
	*tmpPtr3 = '\0';
	...
	for (i = 0; i < stateInfo->num_of_cols; i++) {
	  ...
	  stateInfo->xmlcolNameOpen[i] = NULL;
	  stateInfo->xmlcolNameClose[i] = NULL;
	  if (getColTypeFromColNamemysTab((long int)stateInfo->environ, 
				(long int)stateInfo->connect,
	...
	if ((tmpPtr = strstr(dataPathName," from ")) == NULL &&
	  ...
	  elog (NOTICE, "mysTableOpen: Delete Statement Format Error: %s",
		dataPathName);
	  disconnect_mysTab( (long int)stateInfo->environ, 
			     (long int)stateInfo->connect, MDAS_CAT_ROLLBACK);
	  close_env_mysTab( (long int) stateInfo->environ); 
	...
	*tmpPtr3 = '\0';
	...
	if (i < 0) {
	  elog (NOTICE, "mysTableOpen:Error: %i in  getMultiInsertInformation");
	  close_env_mysTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return(i);
	}
	if ((stateInfo->storedSQLBuffer = 
	  ...
	  elog (NOTICE, "mysTableOpen:Error: Memory Allocation Error");
	  close_env_mysTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return INP_ERR_FORMAT;
	}
	...
	return MDAS_SUCCESS;
        ...
        elog (NOTICE, "mysTableOpen:Error: Not a Known Type SQL Statement: %s",
	      dataPathName);
	close_env_mysTab( (long int) stateInfo->environ); 
	free(stateInfo); stateInfo = NULL;
	return INP_ERR_FORMAT;
	...
	strstr(dataPathName,"SELECT") != dataPathName) {
      ...
      if ((i=exec_no_res_sql_mysTab(stateInfo->environ,stateInfo->connect,
			(unsigned char *) dataPathName))!= 0) { 
	elog (NOTICE, "mysTableOpen: Error in Executing SQL %s: %i",
	      dataPathName,i);
	mysTableErrorReport(NULL, stateInfo->environ);
	disconnect_mysTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
    ...
    addToVarList(&(stateInfo->constantValues), "QT","UPDATE", "CHAR");
    if ((i=exec_sql_mysTab(stateInfo->environ,stateInfo->connect,
    ...
    if ((i= get_no_of_columns_mysTab(stateInfo->statement, 
				  &numCols))!= 0) {
      elog (NOTICE, "mysTableOpen: Error in Getting Num of Cols: %i",i);
      mysTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_mysTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_mysTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i = bind_default_row_mysTab((long int) stateInfo->statement, 
		   stateInfo->data,  
		   stateInfo->data_size, &numCols)) != 0) {
      elog (NOTICE, "mysTableOpen: Error in Binding Row %i: %i",ii,i);
      mysTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_mysTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_mysTab( (long int) stateInfo->environ); 
    ...
    for (j=0; j < numCols; j++) {
      ...
      myprecision = 1;
      scale = -1;
      if ((i = get_column_info_mysTab((long int) stateInfo->statement,j+1, 
				   &displaysize,mycolname,
				   &coltype, buf_size, &mycolnamelen,
				   &nullable,
				   &myprecision,&scale)) != 0) {
	elog (NOTICE, 
	      "mysTableOpen: Error in Getting Column Information %i: %i",ii,i);
	mysTableErrorReport(stateInfo->statement, stateInfo->environ);
	done_with_default_result_mysTab((long int)stateInfo->statement, 
				     stateInfo->data,  
				     stateInfo->data_size, 
	...
	return DB_ERR_MALLOC;
      ...
      getDataTypeFromMap_mysTab(stateInfo->coltype[j],coltype);
...
int 
mysTableClose(dbTableDesc  *exfd)
{
  tableStateInfo *stateInfo;
  int i,ii;
  ...
  if (!strcmp(stateInfo->sqlType, "SELECT") &&
      ...
      stateInfo->num_of_cols > 0) {
    if ((i = done_with_default_result_mysTab((long int)stateInfo->statement, 
		       stateInfo->data,  
		       stateInfo->data_size, stateInfo->num_of_cols)) != 0) {
      elog (NOTICE, "mysTableClose: Error in Unbinding and Closing %i",i);
      mysTableErrorReport(NULL, stateInfo->environ);
      return(i);
    }    
  }
  if (!strcmp(stateInfo->sqlType, "MULTIINSERT")) {
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "mysTableClose: Error in Constructing SQL To Clean List: %i",ii);
	    return(ii);
	}
	if (ii == 1) {
  ...
  if ((i = disconnect_mysTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_COMMIT)) != 0) {
    elog (NOTICE, "mysTableClose: Error in Disconnecting:%i",i);
    mysTableErrorReport(NULL, stateInfo->environ);
    return(DB_TAB_DISCONNECT_ERROR);
  }
  if ((i = close_env_mysTab((long int)stateInfo->environ)) != 0) {
    elog (NOTICE, "mysTableClose: Error in Closing Environment:%i",i);
    mysTableErrorReport(NULL, stateInfo->environ);
...
int 
mysTableRead(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(mysTableReadXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(mysTableReadXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(mysTableReadHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(mysTableReadHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(mysTableReadHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(mysTableReadTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(mysTableReadPACK(exfd,buffer,length));
...
int 
mysTableReadTEMPLATE(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  while ((i = get_row_mysTab((long int) stateInfo->statement)) == 0)       
    {
...
int 
mysTableReadHTMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  if ((i = get_row_mysTab((long int) stateInfo->statement)) != 0) {
    stateInfo->endOfData = 1;
    if (i == MCAT_NO_DATA_FOUND){
  ...
  while (i == 0)       
	...
	if (k <= ll) {
      ...
      i = get_row_mysTab((long int) stateInfo->statement);
    }
  stateInfo->endOfData = 1;
  if (i == MCAT_NO_DATA_FOUND) {
    strcpy(fillStr,"</TABLE>\n</BODY>\n</HTML>\n");
...
int 
mysTableReadHTMLNEST(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableReadHTMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableReadPACK(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableReadHEAD(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableReadXMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableReadXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
mysTableWrite(dbTableDesc *exfd, char *buffer, int length) 
{
/*
 ...
 */
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(mysTableWriteXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(mysTableWriteXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(mysTableWriteHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(mysTableWriteHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(mysTableWriteHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(mysTableWriteTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "SQL"))
    return(mysTableWriteSQL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(mysTableWritePACK(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "VALUES"))
    return(mysTableWriteVALUES(exfd,buffer,length));
  else
    return(mysTableWriteSQL(exfd,buffer,length));
}

int 
mysTableWriteSQL(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (mybuf == NULL)  {
    elog (NOTICE, "mysTableWrite: Memory Allocation Error");
    return(DB_ERR_MALLOC);
    }
...
int 
mysTableWriteVALUES(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
 tableStateInfo *stateInfo;
 int i, ii,j,k;
  ...
  while (*mybuf1 != '\0') {
    ...
    *mybuf2 = '\0';
    ...
    if ((i=exec_no_res_sql_mysTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "mysTableWrite: Error in Executing SQL %s: %i",
	    sqlString,i);
      free(mybuf);
      free(sqlString);
      return(i);
...
int 
mysTableWritePACK(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
mysTableWriteHTMLREL(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int
mysTableWriteHTMLTREE(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
mysTableWriteHTMLNEST(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
mysTableWriteXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
mysTableWriteXMLTREE(dbTableDesc *exfd, char *inbuffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
  ...
  while (1) {
    for (cntData = 0 ; cntData < stateInfo->num_of_cols ; cntData++) {
      ...
      if (i == DATA_TAG_ERROR) {
	elog (NOTICE, "mysTableWrite: Error in Data Tag for Buffer: %s", mybuf);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
	...
	*(tmpmmPtr+mmSize - 1 ) =  '\0';
	...
	elog (NOTICE, "mysTableWrite: Error in Data Tag: %s", dataTagOpen);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
...
int 
mysTableWriteTEMPLATE(dbTableDesc *exfd, char *inbuffer, int length)
{
 ...
 if (!strcmp(stateInfo->sqlType,"MULTIINSERT")) {
  while (1) {
	...
	elog (NOTICE, "mysTableWrite: Error in Processing Choice Rule: %i", j);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(j);
      }
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "mysTableWrite: Error in Constructing SQL  in List: %i",ii);
	    if (collapsePtr != NULL) free(collapsePtr);
	    collapsePtr = NULL;
	    return(ii);
	}
	...
	for (j = 0 ;  j < MAX_SQL_TAGS_COUNT ; j++) {
	  ...
	  if (!strcmp(stateInfo->compSql.tags[i][j], dataTag)) {
	    if (stateInfo->compSql.tagusage[i][j] == TAG_FULL) {
		  ...
		  < 0) {
		elog (NOTICE, 
		   "mysTableWrite: Error in Constructing SQL '%i' in List: %i",
		      i ,ii);
		if (collapsePtr != NULL) free(collapsePtr);
		collapsePtr = NULL;
		return(ii);
  ...
  while (1) {
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (!strcmp(dataTag,stateInfo->xmlcolName[i])) 
      ...
      if (i == stateInfo->num_of_cols) {
	elog (NOTICE, "mysTableWrite: Error in Data Tag: %s", dataTag);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	for (i = 0; i <= cntData; i++) {
	  if (stateInfo->data[i] != emptyStr)
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "mysTableCreate: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if(strstr(tableCreateStmt,"create") == NULL &&
       strstr(tableCreateStmt,"CREATE") == NULL) {
        elog (NOTICE, "mysTableCreate: Error: Not a Create Statement : %s",
        tableCreateStmt);
        return INP_ERR_RES_FORMAT;
    }

    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "mysTableCreate: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_mysTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "mysTableCreate: Error in Creating Environment: %i",i);
      mysTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_mysTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "mysTableCreate: Error in Connecting to %s: %i",
	    database,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      close_env_mysTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_mysTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "mysTableCreate: Error in Executing SQL %s: %i",
	    tableCreateStmt,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      disconnect_mysTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "mysTableUnlink: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "mysTableUnlink: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_mysTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "mysTableUnlink: Error in Unlinking Environment: %i",i);
      mysTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_mysTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "mysTableUnlink: Error in Connecting to %s: %i",
	    database,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      close_env_mysTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_mysTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "mysTableUnlink: Error in Executing SQL %s: %i",
	    queryString,i);
      mysTableErrorReport(NULL, stateInfo->environ);
      disconnect_mysTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
...
executeCompoundSql(tableStateInfo *stateInfo, char *sqlbuf)
{
  ...
  if (reverseExecution(stateInfo)) {
	...
	break;
      myChar = *mybuf2;
      *mybuf2 = '\0';
      if ((i=exec_no_res_sql_mysTab(environ, connect,
