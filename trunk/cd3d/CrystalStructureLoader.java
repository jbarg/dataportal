...
public class CrystalStructureLoader extends CrystalStructure{
  ...
  public CrystalStructureLoader( InputStream is, Component parentComponent, String dataName  ) throws IOException{
    this.is = is;
    this.dataName = dataName + "(Structure)";
    this.parentComponent = parentComponent;
    parentComponent.setCursor( Cursor.getPredefinedCursor( Cursor.WAIT_CURSOR ) );
    st = new StreamTokenizer(
	   new BufferedReader(
	     new InputStreamReader(
	       new ProgressMonitorInputStream( parentComponent, "Reading structure file", is ) ) ) );
    st.commentChar('#');
    st.resetSyntax();
	...
	case StreamTokenizer.TT_EOL:
	  ...
	  case 5:
		...
		atom[ secLine ] = new CrystalAtom( dataName, atomicNumber, location, radius, color );
	      } else {
		atom[ secLine ] = new CrystalAtom( dataName, atomicNumber, location, radius );
	      }
	    } else {
	      atom[ secLine ] = new CrystalAtom( dataName, atomicNumber, location );
	    }
