...
int
ifmxTableOpen(dbTableDesc *exfd, char *resourceLocationName,
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "ifmxTableOpen: Error in resourceLocationName : %s",
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "ifmxTableOpen: Malloc error");
    ...
    if ((i =  open_env_ifmxTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "ifmxTableOpen: Error in Opening Environment: %i",i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_ifmxTab( (long int)  stateInfo->environ, 
			   (long int *) &stateInfo->connect, 
			   (unsigned char*) database, 
			   (unsigned char*) uid, 
			   (unsigned char*) pwd))!= 0)  { 

      elog (NOTICE, "ifmxTableOpen: Error in Connecting to %s: %i",
	    database,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      close_env_ifmxTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if (i != 0) {
      elog (NOTICE, "ifmxTableOpen: Error in Extracting Options from Command %s: %i",
	    dataPathName,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      close_env_ifmxTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
	...
	if ((tmpPtr2 = strstr(tmpPtr, " ")) == NULL &&
	  ...
	  exfd->dbSpecificInfo = (char *)stateInfo; 
	  i = getColumnInfoForifmxTable((long int)stateInfo->environ, 
	...
	*tmpPtr2 = tmpChar;
	...
	if ((tmpPtr3 = strstr(tmpPtr2, ")")) == NULL) {
	  elog (NOTICE, "ifmxTableOpen: Insert Statement Format Error: %s",
		dataPathName);
	  disconnect_ifmxTab( (long int)stateInfo->environ, 
	...
	*tmpPtr3 = '\0';
	...
	for (i = 0; i < stateInfo->num_of_cols; i++) {
	  ...
	  stateInfo->xmlcolNameOpen[i] = NULL;
	  stateInfo->xmlcolNameClose[i] = NULL;
	  if (getColTypeFromColNameifmxTab((long int)stateInfo->environ, 
				(long int)stateInfo->connect,
	...
	if ((tmpPtr = strstr(dataPathName," from ")) == NULL &&
	  ...
	  elog (NOTICE, "ifmxTableOpen: Delete Statement Format Error: %s",
		dataPathName);
	  disconnect_ifmxTab( (long int)stateInfo->environ, 
			     (long int)stateInfo->connect, MDAS_CAT_ROLLBACK);
	  close_env_ifmxTab( (long int) stateInfo->environ); 
	...
	*tmpPtr3 = '\0';
	...
	if (i < 0) {
	  elog (NOTICE, "ifmxTableOpen:Error: %i in  getMultiInsertInformation");
	  close_env_ifmxTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return(i);
	}
	if ((stateInfo->storedSQLBuffer = 
	  ...
	  elog (NOTICE, "ifmxTableOpen:Error: Memory Allocation Error");
	  close_env_ifmxTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return INP_ERR_FORMAT;
	}
	...
	return MDAS_SUCCESS;
        ...
        elog (NOTICE, "ifmxTableOpen:Error: Not a Known Type SQL Statement: %s",
	      dataPathName);
	close_env_ifmxTab( (long int) stateInfo->environ); 
	free(stateInfo); stateInfo = NULL;
	return INP_ERR_FORMAT;
	...
	strstr(dataPathName,"SELECT") != dataPathName) {
      ...
      if ((i=exec_no_res_sql_ifmxTab(stateInfo->environ,stateInfo->connect,
			(unsigned char *) dataPathName))!= 0) { 
	elog (NOTICE, "ifmxTableOpen: Error in Executing SQL %s: %i",
	      dataPathName,i);
	ifmxTableErrorReport(NULL, stateInfo->environ);
	disconnect_ifmxTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
    ...
    addToVarList(&(stateInfo->constantValues), "QT","UPDATE", "CHAR");
    if ((i=exec_sql_ifmxTab(stateInfo->environ,stateInfo->connect,
    ...
    if ((i= get_no_of_columns_ifmxTab(stateInfo->statement, 
				  &numCols))!= 0) {
      elog (NOTICE, "ifmxTableOpen: Error in Getting Num of Cols: %i",i);
      ifmxTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_ifmxTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_ifmxTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i = bind_default_row_ifmxTab((long int) stateInfo->statement, 
		   stateInfo->data,  
		   stateInfo->data_size, &numCols)) != 0) {
      elog (NOTICE, "ifmxTableOpen: Error in Binding Row %i: %i",ii,i);
      ifmxTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_ifmxTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_ifmxTab( (long int) stateInfo->environ); 
    ...
    for (j=0; j < numCols; j++) {
      ...
      myprecision = 1;
      scale = -1;
      if ((i = get_column_info_ifmxTab((long int) stateInfo->statement,j+1, 
				   &displaysize,mycolname,
				   &coltype, buf_size, &mycolnamelen,
				   &nullable,
				   &myprecision,&scale)) != 0) {
	elog (NOTICE, 
	      "ifmxTableOpen: Error in Getting Column Information %i: %i",ii,i);
	ifmxTableErrorReport(stateInfo->statement, stateInfo->environ);
	done_with_default_result_ifmxTab((long int)stateInfo->statement, 
				     stateInfo->data,  
				     stateInfo->data_size, 
	...
	return DB_ERR_MALLOC;
      ...
      getDataTypeFromMap_ifmxTab(stateInfo->coltype[j],coltype);
...
int 
ifmxTableClose(dbTableDesc  *exfd)
{
  tableStateInfo *stateInfo;
  int i,ii;
  ...
  if (!strcmp(stateInfo->sqlType, "SELECT") &&
      ...
      stateInfo->num_of_cols > 0) {
    if ((i = done_with_default_result_ifmxTab((long int)stateInfo->statement, 
		       stateInfo->data,  
		       stateInfo->data_size, stateInfo->num_of_cols)) != 0) {
      elog (NOTICE, "ifmxTableClose: Error in Unbinding and Closing %i",i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      return(i);
    }    
  }
  if (!strcmp(stateInfo->sqlType, "MULTIINSERT")) {
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "ifmxTableClose: Error in Constructing SQL To Clean List: %i",ii);
	    return(ii);
	}
	if (ii == 1) {
  ...
  if ((i = disconnect_ifmxTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_COMMIT)) != 0) {
    elog (NOTICE, "ifmxTableClose: Error in Disconnecting:%i",i);
    ifmxTableErrorReport(NULL, stateInfo->environ);
    return(DB_TAB_DISCONNECT_ERROR);
  }
  if ((i = close_env_ifmxTab((long int)stateInfo->environ)) != 0) {
    elog (NOTICE, "ifmxTableClose: Error in Closing Environment:%i",i);
    ifmxTableErrorReport(NULL, stateInfo->environ);
...
int 
ifmxTableRead(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(ifmxTableReadXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(ifmxTableReadXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(ifmxTableReadHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(ifmxTableReadHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(ifmxTableReadHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(ifmxTableReadTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(ifmxTableReadPACK(exfd,buffer,length));
...
int 
ifmxTableReadTEMPLATE(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  while ((i = get_row_ifmxTab((long int) stateInfo->statement)) == 0)       
    {
...
int 
ifmxTableReadHTMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  if ((i = get_row_ifmxTab((long int) stateInfo->statement)) != 0) {
    stateInfo->endOfData = 1;
    if (i == MCAT_NO_DATA_FOUND){
  ...
  while (i == 0)       
	...
	if (k <= ll) {
      ...
      i = get_row_ifmxTab((long int) stateInfo->statement);
    }
  stateInfo->endOfData = 1;
  if (i == MCAT_NO_DATA_FOUND) {
    strcpy(fillStr,"</TABLE>\n</BODY>\n</HTML>\n");
...
int 
ifmxTableReadHTMLNEST(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableReadHTMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableReadPACK(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableReadHEAD(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableReadXMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableReadXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
ifmxTableWrite(dbTableDesc *exfd, char *buffer, int length) 
{
/*
 ...
 */
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(ifmxTableWriteXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(ifmxTableWriteXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(ifmxTableWriteHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(ifmxTableWriteHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(ifmxTableWriteHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(ifmxTableWriteTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "SQL"))
    return(ifmxTableWriteSQL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(ifmxTableWritePACK(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "VALUES"))
    return(ifmxTableWriteVALUES(exfd,buffer,length));
  else
    return(ifmxTableWriteSQL(exfd,buffer,length));
}

int 
ifmxTableWriteSQL(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (mybuf == NULL)  {
    elog (NOTICE, "ifmxTableWrite: Memory Allocation Error");
    return(DB_ERR_MALLOC);
    }
...
int 
ifmxTableWriteVALUES(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
 tableStateInfo *stateInfo;
 int i, ii,j,k;
  ...
  while (*mybuf1 != '\0') {
    ...
    *mybuf2 = '\0';
    ...
    if ((i=exec_no_res_sql_ifmxTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "ifmxTableWrite: Error in Executing SQL %s: %i",
	    sqlString,i);
      free(mybuf);
      free(sqlString);
      return(i);
...
int 
ifmxTableWritePACK(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
ifmxTableWriteHTMLREL(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int
ifmxTableWriteHTMLTREE(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
ifmxTableWriteHTMLNEST(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
ifmxTableWriteXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
ifmxTableWriteXMLTREE(dbTableDesc *exfd, char *inbuffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
  ...
  while (1) {
    for (cntData = 0 ; cntData < stateInfo->num_of_cols ; cntData++) {
      ...
      if (i == DATA_TAG_ERROR) {
	elog (NOTICE, "ifmxTableWrite: Error in Data Tag for Buffer: %s", mybuf);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
	...
	*(tmpmmPtr+mmSize - 1 ) =  '\0';
	...
	elog (NOTICE, "ifmxTableWrite: Error in Data Tag: %s", dataTagOpen);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
...
int 
ifmxTableWriteTEMPLATE(dbTableDesc *exfd, char *inbuffer, int length)
{
 ...
 if (!strcmp(stateInfo->sqlType,"MULTIINSERT")) {
  while (1) {
	...
	elog (NOTICE, "ifmxTableWrite: Error in Processing Choice Rule: %i", j);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(j);
      }
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "ifmxTableWrite: Error in Constructing SQL  in List: %i",ii);
	    if (collapsePtr != NULL) free(collapsePtr);
	    collapsePtr = NULL;
	    return(ii);
	}
	...
	for (j = 0 ;  j < MAX_SQL_TAGS_COUNT ; j++) {
	  ...
	  if (!strcmp(stateInfo->compSql.tags[i][j], dataTag)) {
	    if (stateInfo->compSql.tagusage[i][j] == TAG_FULL) {
		  ...
		  < 0) {
		elog (NOTICE, 
		   "ifmxTableWrite: Error in Constructing SQL '%i' in List: %i",
		      i ,ii);
		if (collapsePtr != NULL) free(collapsePtr);
		collapsePtr = NULL;
		return(ii);
  ...
  while (1) {
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (!strcmp(dataTag,stateInfo->xmlcolName[i])) 
      ...
      if (i == stateInfo->num_of_cols) {
	elog (NOTICE, "ifmxTableWrite: Error in Data Tag: %s", dataTag);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	for (i = 0; i <= cntData; i++) {
	  if (stateInfo->data[i] != emptyStr)
...
int 
ifmxTableSeek(dbTableDesc *exfd, 
                  int      offset, 
                  int      whence)
{
  tableStateInfo *stateInfo;
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "ifmxTableCreate: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if(strstr(tableCreateStmt,"create") == NULL &&
       strstr(tableCreateStmt,"CREATE") == NULL) {
        elog (NOTICE, "ifmxTableCreate: Error: Not a Create Statement : %s",
        tableCreateStmt);
        return INP_ERR_RES_FORMAT;
    }

    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "ifmxTableCreate: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_ifmxTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "ifmxTableCreate: Error in Creating Environment: %i",i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_ifmxTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "ifmxTableCreate: Error in Connecting to %s: %i",
	    database,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      close_env_ifmxTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_ifmxTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "ifmxTableCreate: Error in Executing SQL %s: %i",
	    tableCreateStmt,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      disconnect_ifmxTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "ifmxTableUnlink: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "ifmxTableUnlink: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_ifmxTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "ifmxTableUnlink: Error in Unlinking Environment: %i",i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_ifmxTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "ifmxTableUnlink: Error in Connecting to %s: %i",
	    database,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      close_env_ifmxTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_ifmxTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "ifmxTableUnlink: Error in Executing SQL %s: %i",
	    queryString,i);
      ifmxTableErrorReport(NULL, stateInfo->environ);
      disconnect_ifmxTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
...
executeCompoundSql(tableStateInfo *stateInfo, char *sqlbuf)
{
  ...
  if (reverseExecution(stateInfo)) {
	...
	break;
      myChar = *mybuf2;
      *mybuf2 = '\0';
      if ((i=exec_no_res_sql_ifmxTab(environ, connect,
