...
{     
  ...
  tabledIsOkay = 1;
  i =  get_ctype_value_from_query(cval, sqlq);
  tabledIsOkay = 0;
  if (i < 0) 
    return(i);
...
makeDataLink(char *srcCollName, char *srcDataName, int trgDataId, int userId)
{
  ...
  snprintf(atval[15],MAX_DATA_SIZE-1,"THIS IS A SLINK TO %s/%s",srcCollName, srcDataName);
  atval[15][MAX_DATA_SIZE-1] = '\0';
		   ...
		   mdasC_sql_result_struct *inDataInfo)
{
  ...
  for (ii = 0; ii < inDataInfo->row_count ; ii++) { /*** begin the loop ***/
    ...
    sprintf(atname[5],"version_num");
    sprintf(atval[0],"'%s'",newPathName);
    sprintf(atval[1],"%i",rsrc_id);
    sprintf(atval[2],"'%s'",cval[5]);
    sprintf(atval[3],"%i",data_id);
    sprintf(atval[4],"%i",rnum);
    if (strlen(verNum) > 0) {
      sprintf(atval[5],"'%s'",verNum);
      i = changeAttrValueSet(atname, atval,"MDAS_AD_REPL",6,3);
    }
    else
      i = changeAttrValueSet(atname, atval,"MDAS_AD_REPL",5,3);
				...
				mdasC_sql_result_struct *inDataInfo)
{
  ...
  for (ii = 0; ii < inDataInfo->row_count ; ii++) { /*** begin the loop ***/
    ...
    sprintf(atname[1],"rsrc_id");
    sprintf(atname[2],"modify_timestamp");
    sprintf(atname[3],"data_size");
    sprintf(atname[4],"offset");
    sprintf(atname[5],"container_id");
    sprintf(atname[6],"container_td_id");
    sprintf(atname[7],"repl_enum");
    sprintf(atname[8],"data_id");
    sprintf(atname[9],"repl_enum");
    sprintf(atname[10],"version_num");
    sprintf(atval[0],"' '");
    sprintf(atval[1],"%i",rsrc_id);
    sprintf(atval[2],"'%s'",cval[5]);
    sprintf(atval[3],"%lld",idataSize);
    sprintf(atval[4],"%lld",ioffSet);
    sprintf(atval[5],"%i",contId);
    sprintf(atval[6],"%i",contId);
    sprintf(atval[7],"-1");
    sprintf(atval[8],"%i",data_id);
    sprintf(atval[9],"%i",rnum);
    if (strlen(verNum) > 0) {
      sprintf(atval[10],"'%s'",verNum);
      i = changeAttrValueSet(atname, atval,"MDAS_AD_REPL",11,8);
    }
    else
      i = changeAttrValueSet(atname, atval,"MDAS_AD_REPL",10,8);
    if (i !=0) return (MDAS_AD_REPL_UPDATE_ERROR);
    */
    sprintf(atname[0],"path_name");
    sprintf(atname[1],"rsrc_id");
