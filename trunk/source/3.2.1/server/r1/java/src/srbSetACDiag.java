 ...
 *
 */
public class srbSetACDiag extends JDialog
{
    // Insets
    ...
    public final static String GroupKeyWd = new String ("groups");
    ...
    public srbSetACDiag(srbClObj mySrbClObj) {
	srbClObj = mySrbClObj;
	...
	// The user/group checkbox

	userBox = new JCheckBox("By User");
	userBox.setSelected(true);

	groupBox = new JCheckBox("By Group");
	groupBox.setSelected(false);

        CheckBoxListener boxListener = new CheckBoxListener();
        userBox.addItemListener(boxListener);
        groupBox.addItemListener(boxListener);
	
	buildConstraints (constraints, 0, 0, 1, 1, 100, 200);
        layout.setConstraints( userBox, constraints );
        getContentPane().add( userBox );

        buildConstraints (constraints, 1, 0, 1, 1, 100, 200);
        layout.setConstraints( groupBox, constraints );
        getContentPane().add( groupBox );

        // Domain control list

        domainLabel = new JLabel ("User Domains");
        ...
        buildConstraints (constraints, 0, 2, 2, 1, 100, 300);
        ...
        buildConstraints (constraints, 2, 2, 2, 1, 100, 300);
        ...
        buildConstraints (constraints, 0, 5, 2, 1, 100, 300);
    ...
    class CheckBoxListener implements ItemListener {
        public void itemStateChanged(ItemEvent e) {
            ...
            if (source == userBox) {
		if (userBox.isSelected() == false)
		    return;
		groupBox.setSelected(false);
		fillDomain();
		userModel.removeAllElements();
		domainLabel.setText("User Domains");
		userLabel.setText("Users");
	    } else if (source == groupBox) {
		if (groupBox.isSelected() == false)
		    return;
		userBox.setSelected(false);
		domainModel.removeAllElements();
		selDomain = GroupKeyWd;
		fillUser(GroupKeyWd);
		domainLabel.setText("");
		userLabel.setText("Groups");
	    }
	}
    }
    ...
    public void main(String[] args) {
        ...
        srbSetACDiag  mySrbPathDiag = new srbSetACDiag(srbClObj);
