...
static void usage(char *prog)
{
   ...
   fprintf(stderr,"Usage: %s -Q [-z zonename] AttName Op AttValue [AttName Op AttValue] ... (upto 5) \n",prog);
   fprintf(stderr,"Usage: %s -Q -c [-z zonename] AttName Op AttValue [AttName Op AttValue] ... (upto 5)\n",prog);
   fprintf(stderr,"Usage: %s -Q -u [-z zonename] AttName Op AttValue [AttName Op AttValue] ... (upto 4)\n",prog);
   fprintf(stderr,"Usage: %s -Q -r [-z zonename] AttName Op AttValue [AttName Op AttValue] ... (upto 4)\n",prog);
...
int main(int argc, char **argv)
{
  ...
  while ((c=getopt(argc, argv,"QurhRDecdC:f:z:")) != EOF) {
        switch (c) {
            case 'h':
		usage (argv[0]);
		clFinish(conn);
...
doMetaRemoveForCollection(srbConn *conn,int argc, char **argv)
{
  ...
  sprintf(tempObj,"%s/aaaa",argv[argc-1]);
...
doMetaQueryForDataset(srbConn *conn,int argc, char **inargv)
{
    ...
    for (jj =  0,i =  k; i < argc && jj < 5 ; i =  i+3, jj++) {
      if (i +2 >= argc) {
    ...
    selval[DATA_NAME] = 1;
    status = queryAndPrintAcrossZones(conn, 0 , zoneName, qval, selval, &myresult,
                                 DEFAULT_ROW_COUNT);
    return(status);
...
doMetaQueryForResource(srbConn *conn,int argc, char **inargv)
{
    ...
    for (jj =  0,i =  k; i < argc && jj < 3 ; i =  i+3, jj++) {
      if (i +2 >= argc) {
	printf("Error in number of arguments expected\n");
	return(-1);
      }
