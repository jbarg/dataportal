...
//for GSI
	 ...
	 * The srb version.
	 */
	protected static String version = SRB_VERSION_3_3;

	/**
	 * working with strings got difficult
	 */
	static HashMap versionInt = new HashMap( 6, 1 );
	static {
		versionInt.put( SRB_VERSION_3_3, new Float( 3.3 ) );
		versionInt.put( SRB_VERSION_3_0_2, new Float( 3 ) );
		versionInt.put( SRB_VERSION_2, new Float( 2 ) );
		versionInt.put( SRB_VERSION_1_1_8, new Float( 1 ) );
	}
...
//----------------------------------------------------------------------
	...
	public void setUserInfo( LocalFile userInfoDirectory )
		throws FileNotFoundException, IOException
	{
		LocalFile mdas = new LocalFile( userInfoDirectory, ".MdasEnv" );
		if (!mdas.exists()) {
			mdas = new LocalFile( userInfoDirectory, "MdasEnv" );
		}
		setMdasUserInfo( mdas );

		mdas = new LocalFile( userInfoDirectory, ".MdasAuth" );
		if (!mdas.exists()) {
			mdas = new LocalFile( userInfoDirectory, "MdasAuth" );
		}
		readMdasAuth( mdas );
