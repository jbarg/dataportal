...
initSrbHost ()
{
    ...
    hostEnt = getAndQueHostName (myHost, tmpHElement);
    if (hostEnt == NULL) {
	return (unixErrConv (errno));
    }
#ifdef foo	/* use getAndQueHostName */
    ...
    queAddrlistToHostEnt (hostEnt, tmpHElement);
#ifdef foo	/* done in queAddrlistToHostEnt */
    i = 0;
    while (1) {
...
initHostWithMCat ()
{
    ...
    status = configZoneWithFile (file);
    if (status < 0) {
	return (status);
    }
#else	/* FED_MCAT_FILE_CONFIG */
    status = configZoneWithMCat (file);
    if (status < 0) {
        return (status);
    }
#endif	/* FED_MCAT_FILE_CONFIG */
#else	/* FED_MCAT */
    status = configNonFedZone (file);
    if (status < 0) {
        return (status);
    }
#endif
    ...
    status = configRescWithMCat ();
    if (status < 0) {
        return (status);
    }

#ifdef FED_MCAT
    /* Make sure LocalMcat->mdasEnabled is 1 since there may be 
     * conflicting configuration on locality
     */
    LocalMcat->mdasEnabled = MCAT_ENABLED_LOCAL;

    /* some sanity check */
    if (LocalHostHead->mdasEnabled == MCAT_ENABLED_REMOTE) {
	/* something is wrong with the zone config. A local host cannot
	 * be a remote MCAT_ENABLED host */

	fprintf (stderr, "Warning: Zone config error:");
	fprintf (stderr, 
	 "Local host has the same host and port as remote zone %s\n", 
	 LocalHostHead->serverAuthInfo->mcatName);
	LocalHostHead->mdasEnabled = 0;
	LocalHostHead->serverAuthInfo = LocalMcat->serverAuthInfo;
	LocalHostHead->zoneHostTab = LocalMcat;
    }
#endif
...
char *mcatDn, char *myPortNum, int localFlag)
{
...
#ifdef FED_MCAT
     /* localFlag == 1 ==> local MCAT */
    if (localFlag ==  1 && LocalMcat != NULL && 
     LocalMcat->serverAuthInfo != NULL &&
     strcmp (LocalMcat->serverAuthInfo->mcatName, "defaultMcat") == 0) {
	/* overwrite mcatName from the mcatHost file */
	if (LocalMcat->serverAuthInfo->mcatName != NULL)
	...
	newHost = 1;
        ...
        hostEnt = getAndQueHostName (mcatHost, tmpHElement);
        if (hostEnt == NULL) {
	...
	if (strcmp (tmpHElement->serverAuthInfo->mcatName, 
	 myServerAuthInfo->mcatName) != 0) {
	    ...
	    if (myServerAuthInfo != NULL) {
	        free (myServerAuthInfo);
		myServerAuthInfo = NULL;
	    }
	}
    } else {
        tmpHElement->serverAuthInfo = myServerAuthInfo;
    }
    if (localFlag == 1) {	/* the first extry ==> local MCAT */
	if (newHost > 0 && LocalMcat != NULL) {
          if (UserClass == CLASS_MAS)
	    elog (NOTICE,
		"WARNING: Local zone mcatHost config conflict: MCAT- %s, will use host in mcatHost file - %s",
	      tmpHElement->alias->hostName, LocalMcat->alias->hostName);
	} else {
...
struct hostElement *hostTabPtr)
{
    ...
    if (ThisMdasEnvData == NULL || ThisUser == NULL || ClientUser == NULL) {
	elog (NOTICE, 
	 "svrConnect: run initThisUserInfo user Info\n");
...
svrConnectSvr (srbConn *conn)
{
    ...
    */
    ...
    if (status < 0) {
        fprintf (stderr, "svrConnectSvr: initPort error. status =%d\n",
        status);
        return (status);
    }

#ifdef foo	/* done in initPort */
    /*
   ...
   if (status < 0) {
        fprintf (stderr, "svrConnectSvr: connectPort error. status =%d\n",
        status);
        return (status);
    }

#ifdef foo	/* done in connectPort */
    ...
    status = connectWithTimeout (port->sock, (struct sockaddr *) &port->raddr);
#ifdef CONNECT_TIMING
    elog (NOTICE, 
    ...
    if (svrPortNum < 0) {
        ...
        if (port->sock < 0) {
            sprintf(conn->errorMessage,
              "svrConnectSvr(): Connect to srbServer: %s:%d, status = %d, errno=%d\n",
                conn->srbHost, svrPortNum, port->sock, errno);
            return (port->sock);
        }
    } 
    /* If svrPortNum == 0, no new connection */
    ...
    if ((status = svrSendAuth(conn)) != STATUS_OK) {
      return status;
    }
 ...
 */
int
svrSendAuth(srbConn *conn)
{
    int status;
    int i;

    if (conn == NULL || conn->proxyUser == NULL) {
	elog (NOTICE, "svrSendAuth: conn struct has not been initialized");
	return (STATUS_INVALID);
    }
...
openMcatConfigFile ()
{
    ...
    if (file == NULL) {
        fprintf(stderr, "Unable to open MCAT_HOST file %s. ernro = %d\n", 
	 conFile, errno);
    }
...
configZoneWithFile (FILE* file)
{
    ...
    while ((i = getNextStr(file, buf, sizeof(buf))) != EOF) {
        ...
        if (i < 0) {
            ...
            fprintf (stderr,
             "config error. %s entry must have at least 2 values\n",
             mcatName);
            fclose (file);
            return (-1);
        }
...
configRescWithMCat ()
{
    ...
    while (myresult != NULL) {
      ...
      for (i = 0; i < myresult->row_count; i++) {
        ...
        tmpResInfo->phyResMaxObjSize = strtoll (&phyResMaxObjSize[i*MAX_DATA_SIZE],
         0, 0);
...
#endif
            /* Enqueue all the address too */
	    hostEnt = getAndQueHostName (tmpHost, tmpHElement);
            if (hostEnt == NULL) {
                /* added 01/20/05 - to fix core dump */
	    ...
	    if (configVault (&logDirPathName[i*MAX_DATA_SIZE], 
	      &phyDirPathName[i*MAX_DATA_SIZE], 
	      &phyResType[i*MAX_DATA_SIZE]) < 0) {
                /* added 01/20/05 - to fix core dump */
                if (tmpHElement->zoneHostTab == NULL)
                    tmpHElement->zoneHostTab = myMcatHostElement;
		continue;
...
getAndQueHostName (char *hostName, struct hostElement *tmpHElement)
{
    ...
    if (hostName == NULL || tmpHElement == NULL)
	return NULL;

    /* Enqueue all the address too */
    /* XXXX gethostbyname could hang going to address such as csf.rl.ac.uk */
    beforeTime = time (0);
    if ((hostEnt = gethostbyname (hostName)) == NULL) {
	if (UserClass == CLASS_MAS)
            fprintf (stderr,
              "getAndQueHostName: gethostbyname error for %s ,errno = %d\n",
              hostName, errno);
        return (NULL);
    }
    afterTime = time (0);
    if (afterTime - beforeTime >= 2) {
        elog (NOTICE,
         "WARNING WARNING: gethostbyname of %s is taking %d sec. This could severely affect interactivity of your SRB system", 
	 hostName, afterTime - beforeTime);
    }
    if (strcasecmp (hostName, hostEnt->h_name) != 0) {
        tmpName = malloc (strlen (hostEnt->h_name) + 1);
        strcpy (tmpName, hostEnt->h_name);
        enqueName (tmpName, tmpHElement);
    }
...
prtZoneInfo ()
{
    ...
    fprintf (stderr, "Local Zone : \n");
    if (LocalMcat != NULL && LocalMcat->serverAuthInfo != NULL &&
     LocalMcat->serverAuthInfo->mcatName != NULL) {
	fprintf (stderr, "ZoneName = %s",  LocalMcat->serverAuthInfo->mcatName);
	if (LocalMcat->alias != NULL && LocalMcat->alias->hostName != NULL)
	    fprintf (stderr, "  HostName = %s", LocalMcat->alias->hostName); 
	if (LocalMcat->portNum != NULL)
	    fprintf (stderr, "  PortNum = %s", LocalMcat->portNum);
	fprintf (stderr, "\n");
    }
    fprintf (stderr, "Remote Zone : \n");
    tmpHElement = LocalHostHead;
    while (tmpHElement != NULL) {
	if (tmpHElement->mdasEnabled == MCAT_ENABLED_REMOTE && 
	 tmpHElement->serverAuthInfo != NULL &&
         tmpHElement->serverAuthInfo->mcatName != NULL) {
            fprintf (stderr, "ZoneName = %s",  
	     tmpHElement->serverAuthInfo->mcatName);
            if (tmpHElement->alias != NULL && 
	     tmpHElement->alias->hostName != NULL)
                fprintf (stderr, "  HostName = %s", 
		  tmpHElement->alias->hostName); 
            if (tmpHElement->portNum != NULL) 
                fprintf (stderr, "  PortNum = %s", tmpHElement->portNum);
            fprintf (stderr, "\n");
	}
	tmpHElement = tmpHElement->next;
    }
...
configVault (char *logDirPathName, char *phyDirPathName, char *phyResType)
{
    ...
    if (logDirPathName == NULL || phyDirPathName == NULL || phyResType == NULL)
    {
	fprintf (stderr, 
	 "configVault: NULL logDirPathName or phyDirPathName or phyResType\n");
	return (-1);
    }

    /* Process only the file system type */
    tinx = objTypeLookUp (phyResType);
    if (tinx < 0)
        return -2;
    ...
    if (logDirPathName[0] == '/') {
        tmpPath =
          malloc (strlen (logDirPathName) + 1);
        strcpy (tmpPath, logDirPathName);
    } else if (logDirPathName[0] == '\\' &&
      logDirPathName[1] == '/') {
        tmpPath =
          malloc (strlen (logDirPathName + 1) +
            strlen (phyDirPathName) + 1);
        strcpy (tmpPath, phyDirPathName);
        strcat (tmpPath, logDirPathName + 1);
    } else {
        tmpPath =
          malloc (strlen (phyDirPathName) + 1);
        strcpy (tmpPath, phyDirPathName);
    }
