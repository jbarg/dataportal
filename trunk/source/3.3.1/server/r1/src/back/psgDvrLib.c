...
****************************************************************************/
/***************************************************************************
  NAME   : psgTableErrorReport
  PURPOSE: Logging error message to elog
****************************************************************************/

void  psgTableErrorReport( int statement, int environ)
{
  if (statement != NULL )
    elog(NOTICE, "Postgres Table Error: environ = %i", environ);
  else if (environ != NULL )
    elog(NOTICE, "Postgres Table Error: statement = %i", statement);
  else
    elog(NOTICE, "Postgres Table Error: statement = %i and environ = %i",statement, environ);
  return;
}
