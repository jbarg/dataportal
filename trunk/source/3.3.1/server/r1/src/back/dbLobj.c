...
*/
		...
		Db2Home = readLobjConfig (DB2_HOME_KEYWD);
            }
		...
		OracleHome = readLobjConfig (ORACLE_HOME_KEYWD);
	    }
		...
		IllusHome = readLobjConfig (ILLUS_HOME_KEYWD);
            }
	    retVal = illustraOpen (&gDbLobjDesc[descInx], 
   	             resourceLoc, dataPath, IllusUser, IllusPasswd);
    	    if (retVal != 0) { 	/* open failed */
                elog(NOTICE,
		"cannot open Illustra LOBJ resLoc=%s,dataPath=%s", 
