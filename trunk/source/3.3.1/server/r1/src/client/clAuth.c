...
#include "misc_util.h"
#include "nt_extra.h"
void GetWindowsClientAuthFile(char *buf);
void GetWindowsClientEnvFile(char *buf);
#endif
 ...
 */
int
clSendAuth(srbConn *conn)
{
    int status;

    if (conn == NULL || conn->proxyUser == NULL) {
        fprintf (stderr, "clSendAuth: conn struct has not been initialized");
...
/* Send a msg to client requesting for Client Auth */
    ...
    if (status != AUTH_ENCRYPT1_INX) {
       fprintf (stderr,
         "svrCliSvrEncryptAuth: client AuthScheme %d not supported \n",
         status);
        return (AUTH_ERR_MDAS_AUTH);
    }
...
void GetWindowsClientAuthFile(char *buf)
{
   char tmpstr[MAXEXFPATH];
   struct _stat stat_buf;

   tmpstr[0] = '\0';
   if(getenv("USERPROFILE") != NULL)
   {
       /* it is stored in the subdirectory '.srb'. */
       strcpy(tmpstr,getenv("USERPROFILE"));
       strcat(tmpstr,"/.srb/MdasAuth");
   }

   if(strlen(tmpstr) > 0)
   {
      if(_stat(tmpstr, &stat_buf) == 0)  /* file exists. */
      {
         strcpy(buf,tmpstr);
         return;
      }
   ...
   /* other cases, file does not exist, etc.  try the old way for backward compat ibility */
   if(get_home_env() == NULL)
     return;

   strcpy(buf,get_home_env());
   strcat(buf, "/srb/");
...
readMdasAuth ()
{
    ...
    /* attempt alternative to MdasAuth */
    if (obfGetPw(buf)==0) {  
      retVal = malloc (strlen (buf) + 20);
      strcpy (retVal, buf);
      return (retVal);       /* alternative successful */
    }

    if (getMdasAuthFilename(buf)<0) {
      return(NULL);
    }
...
char *authScheme, char *serverDn)
{
    char *tmpAuthKeyWd = NULL;
    char *tmpAuthDn = NULL;
    char *tmpHostAddr = NULL;
	 ...
	 */ 
	if (serverDn != NULL && strlen (serverDn) > 0) {
	    tmpAuthDn = serverDn;
    	} else if ((tmpAuthDn = getenv (AUTH_SERVER_DN_KEYWD)) == NULL) {
	    if (myMdasEnvData != NULL) 
...
clGetServerDn (srbConn *conn, char *hostAddr, int authInx)
{
    ...
    if ( (authInx == GSI_AUTH_INX || authInx == GSI_DELEGATE_INX) &&
         (DefServerAuthInfo->authsvc->index  == GSI_AUTH_INX || 
	  DefServerAuthInfo->authsvc->index  == GSI_DELEGATE_INX) ) {
        if (strcasecmp (hostAddr, DefServerAuthInfo->hostAddr) == 0) {
...
procMcatServerAuthInfo (mdasC_sql_result_struct  *myresult)
{
   int i;
    ...
    struct serverAuthInfo *tmpInfo = NULL;
    struct serverAuthInfo *lastInfo = NULL;
                              ...
                              char *tab_name, char *att_name)
{
  ...
  return (NULL);
#endif
  /* borrowed from get_from_result_struct */
  int i;
  ...
  tmp_tab_name1 = stripSchema (tab_name);

  for (i = 0; i <  result->result_count ; i++){
    tmp_tab_name2 = stripSchema (result->sqlresult[i].tab_name);
...
/* stripSchema - tab_name may be in the form of schema_name.tab_name. Strip out
 * the schema name if it exists. Borrowed from stripName.
 */

char *stripSchema (char *tab_name)
{
    char *tmpStr;
...
int
clSendGsiDelegAuth (srbConn *conn)
{
#ifdef GSI_AUTH
    Port        *port = conn->port;
    int status;
    ...
    /* In an previous version, we'd skip calling clGetServerDN and
       call aid_establish_context_clientside_without_server_dn instead
       of aid_establish_context_clientside but with GT3 GSI, this
       (calling gss_init_sec_context with GSS_C_NO_NAME as the server
       DN) is not allowed.  It is also more secure to verify the
       Server's DN, altho it complicates our interzone server to
