...
{  
  ...
  if (argc != 5)
    {
      printf("Usage: ingestLogicalResource  <LogicalResourceName> <LogicalResourceType> <PhysicalResourceName> <LogicalDefaultPath>\n");
      exit(0);
    }
    ...
    i = registerLogicalResourceInfo(argv[1],argv[2],argv[3],
			     userName, domainName, userAuth,
				argv[4]);
    if (i != 0)
      {
	printf("ingestLogicalResource Error: %i\n",i);
