...
{  
  ...
  if (argc != 7)
    {
      printf("Usage: ingestUsergroup  <GroupName> <GroupPassword> <GroupType> <ContctAddress> <ContactPhone> <ContactEmail>\n");
      exit(-1);
    }
    ...
    i = register_user_group(0, argv[1],argv[2],argv[3],
			     argv[4],argv[5],argv[6],
			     userName, userAuth, domainName);
    if (i != 0)
      {
	printf("ingestUser Error: %i\n",i);
