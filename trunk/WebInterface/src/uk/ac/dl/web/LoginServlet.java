...
//Needs to implement ssl for this servlet.  This servlet checks weather user can log in, as normal(same code
// if -1 user is sent to erro page, no session created.  If yes, send to http to a servlet
// and then create session and add thing to session and send to BasciSearch.  eg back to http
// use http://request.getSer()+req.getPort() etc to get path to servlet
    ...
    throws ServletException, IOException {
        ...
        if(loggedin) {
          ...
          /*  String isHPC  = prop.getProperty("isHPC");
            ...
            String[] topicFacs = prop.getProperty("list","").split("\\s+");
            //run through the facilities user can see and get the xml
            for(int i = 0 ; i <facs.size();i++){
                for(int j=0; j <topicFacs.length;j++){
                    try{
                        ...
                        if(facilityName.equals(topicFacs[j])){
                            ...
                            String meta_username = prop.getProperty((String)facs.get(i)+"_username");
                        ...
                        logger.warn("Unable to get the xml for "+facs.get(i)+" from the database",e);
                        facs.remove(i);
                    }
                }
            }
         ...
         /*   if(prop.getProperty("topics").equals("true")){
                ...
                try{
                    String meta_name = prop.getProperty("meta_name");
                    ...
                    String meta_topic = prop.getProperty("meta_topic");
                    //Document mydoc = TopicManager.buildTopics("jdbc:oracle:thin:@elektra.dl.ac.uk:1521:emat1","emat","tame");
                    Document mydoc = TopicManager.buildTopics(meta_url,meta_username,meta_passwd, meta_topic);
                    ...
                    Saver.save(mydoc,new File(workingDir+File.separator+"profiles"+File.separator+meta_name.toLowerCase()+sessionid));
                    //hard code list fac
