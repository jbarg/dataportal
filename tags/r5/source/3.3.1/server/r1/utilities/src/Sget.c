...
main(int argc, char **argv)
{
    ...
    while ((c=getopt(argc, argv,"N:n:t:T:A:R:pfrvsmMkVbc:W:")) != EOF) {
      switch (c) {
      case 'n':
	copyNum = atoi(optarg);
	break;
	...
	sprintf(inCondition,"VNUM='%s'",versionString);
      else {
          strcat(inCondition,"&VNUM='");
          strcat(inCondition,versionString);
          strcat(inCondition,"'");
      }
    }
...
srbPathName targNameArray[], int flagval, int copyNum, char *ticketId)
{
    ...
    for (i = 0; i < nArgv; i++) {
      do {
		...
		if (flagval & b_FLAG) {
		    char *myColl;
		    srbPathName collNameArray, *contnameArray = NULL;
		    int nVal;
...
usage()
{
  fprintf(stderr, "Usage: Sget [-n n] [-N numThreads] [-bpfrvsmMkV] [-R retries] [-T ticketFile | -t ticket] [-W versionString] [-A condition] srbObj ...  localFile/Dir \n");
