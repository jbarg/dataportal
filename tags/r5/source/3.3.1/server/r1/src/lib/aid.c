...
aid_import_delegate(char * filename)
...
/*for delegation */
gss_cred_id_t delegated_cred_gsi[MAX_FDS] = {
...
/* XXXXXXX #define OBJ_CREATE_NEEDED  -- no longer needed */
...
#if defined(GSI_AUTH) || defined(DCE)
      if (my_creds_gsi_dce == GSS_C_NO_CREDENTIAL) {
          maj_stat = gss_acquire_cred(&min_stat, my_name, 0,
                                    GSS_C_NULL_OID_SET, GSS_C_ACCEPT,
                                    &my_creds_gsi_dce, NULL, NULL);
      }
      else {
	  /* already set up, presumably via a call to aid_import_credential */
	  maj_stat = GSS_S_COMPLETE;  /* OK */
      }
#endif
    }
    if (maj_stat != GSS_S_COMPLETE) {
 ...
 * input parameter - 
 * fileName: the name of the file containing a credential as exported by
 *           aid_export_delegate.
 *  output - the shared variable my_creds_gsi_dce contains the credential.
 *           
 * A client can call this, instead of aid_setup_creds, to set up the
 * AID library (client-side) to use a proxy credential.  (As an test,
 * you can modify clAuth.c to do this.)
 * 
 * This is based on Ananta's aid_import_delegate, with some additional
 * integration he didn't have time to do.
 */
int aid_import_delegate(char * fileName)
{
    OM_uint32 maj_stat, min_stat;
    char myFileName[500];
    gss_buffer_desc imported_cred_filename;
    ...
    strncat(myFileName,fileName,490);
    imported_cred_filename.value=myFileName;
    imported_cred_filename.length=strlen(myFileName);

    /* import credential from file */
    maj_stat = gss_import_cred(&min_stat,       /* minor status */
         &my_creds_gsi_dce,  /* handle to imported delegated credential */
         NULL,   /* desired mechanism - NULL default */
         1,      /* option req 1- mech specific */
         &imported_cred_filename,
                 /* filename to where proxy has been stored */
         0,      /* time req */
         NULL);  /*time rec - specifies remaining time for which it is valid */

    /* if successful, proxy is retrieved from file and cred_handle can be
       utilizing as a credential.  
    */
...
/* Accept an input credential for later use for client-side
   authentication.  First this is called and then aid_setup_creds can
   be called or not, and then when aid_establish_context_clientside is
   called, this credential will be used.  This can be used, for
   example, by the GridFTP Server to pass in the delegated credential
   for the SRB Client library to use.
*/
int
aid_import_credential(gss_cred_id_t input_cred)
{
  my_creds_gsi_dce = input_cred;
}
