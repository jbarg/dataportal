...
int
initialize_ext_graph(char *file_name)
{
  int i;
  FILE *fd;
  int status;
  ...
  sprintf(tmpStr,"%s.ext",file_name);
  fd = fopen(tmpStr,"r");
  if (fd == NULL) {
    status = unixErrConv (errno);
    /*    elog (NOTICE, "initialize_ext_graph:Error in Opening File:'%s'\n",tmpStr);
	  elog (NOTICE, "initialize_ext_graph:UNIX open error. errorCode = %d", status); */
    return (status);
  }

  for (i = mdas_metadata_graph_maxpaths; i < MAX_NO_OF_MDAS_METADATA_GRAPH_PATHS ; i++){
    ...
    */
    sprintf(mdas_metadata_graph_path[i].name1,"%s",tabname1);
    sprintf(mdas_metadata_graph_path[i].name2,"%s",tabname2);
  }
...
initialize_graph(char *file_name)
{
  ...
  initialize_ext_graph(file_name);
  return(0);
}
...
void convert_to_sql(int n, char table_names[][MAX_TOKEN], char *other_conditions,
