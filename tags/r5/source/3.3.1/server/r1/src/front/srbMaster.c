...
#define STARTUP_READ_TIMEOUT	2	/* 1 seconds timeout */
jmp_buf Jenv;
#endif
...
srbMasterMain(int argc, char *argv[])
{
    ...
    status = initThisUserInfo ();	/* Initialize ThisUser */

    if (status < 0) {
	fprintf (stderr, "srbMasterMain: initThisUserInfo error, status = %d\n",
	 status);
	return (status);
    }
#ifdef _WIN32
    ...
    /* taken out - up to sys admin (void) umask((mode_t) 0077); */
#endif
...
serverLoop()
{
    ...
    fprintf (stderr, "This server is configured to use ports %d - %d\n",
	COMM_PORT_NUM_START, COMM_PORT_NUM_START + COMM_PORT_NUM_COUNT -1);
#endif 
...
/* for performance concern, we don't need pre-spawn feature in Windows. */
#ifdef _WIN32
    preSpawnCnt = 0;
    /* XXXX force it for now. Bing, you may want to change it */
    singleStatupPort = 0;
#else
    ...
    if (SingleSvrPort > 0) {
	preSpawnCnt = 0;
    } else if ((tmp = getenv (PRE_SPAWN_CNT)) != NULL) {
	preSpawnCnt = atoi (tmp);
	if (preSpawnCnt > MAX_PRE_SPAWN_CNT) {
	    preSpawnCnt = MAX_PRE_SPAWN_CNT;
	} else if (preSpawnCnt < 0) {
	    preSpawnCnt = 0;
	}
    } else {
...
#ifdef _WIN32
            elog(NOTICE,
	     "serverLoop: serverLoop: %d sockets pending\n",nSelected);
#else
	...
	if (streamConnection(serverFd, myPort) == STATUS_OK) {
...
#ifdef _WIN32
            elog(WARN,
	     "serverLoop: streamConnection error status = %d\n",status);
#else
         ...
         */
#ifdef _WIN32
        status = recv(myPort->sock, myPort->buf.data,
	 strlen (STARTUP_HEADER) + 1, 0);
#else
	memset (myPort->buf.data, 0, sizeof (myPort->buf.data));
        signal(SIGALRM, pipeReadTimeout);
        if (setjmp(Jenv) == 0) {
            alarm (STARTUP_READ_TIMEOUT);
            while ((status = read (myPort->sock, myPort->buf.data,
             strlen (STARTUP_HEADER) + 1)) <= 0) {
                if (errno == EINTR) {
                    continue;
                } else {
                    fprintf(stderr,
                     "serverLoop: read startup msg failed, errno=%d\n", errno);
		    break;
                }
	    }
            alarm (0);
            ...
            fprintf(stderr,
             "serverStartup: read startup msg timed out\n");
	    status = -1;
        }

/* XXXXX original read with no timeout 
...
*/
	...
	if ((status == strlen (STARTUP_HEADER) + 1) &&
	 strcmp (myPort->buf.data, STARTUP_HEADER) == 0) {
...
#ifndef _WIN32
	        /* need to do a dummy write if prespawn is > 1 or the
	         * server's read won't be done for some reason */
	        write (myPort->pipeFd1, &myPort->pipeFd1, 
	         sizeof (myPort->pipeFd1));
		...
		myPort->preSpawn = NEED_SPAWN_USING_1_PORT;
                while ((status = serverStartupWithOnePort (myPort)) < 0) {
#ifdef _WIN32
                    elog(FATAL,
                     "serverLoop:serverStartup failure, status = %d\n", status);
                    Sleep(1000*STARTUP_RETRY_TIME);
		...
		svrSendStatus (myPort->sock, NO_NEW_CONNECTION);
            }

            streamClose(myPort->sock);
	    myPort->preSpawn = NEED_SPAWN;	/* new srbServer is needed */
	    myPort->sock = -1;
	    myPort->pipeFd1 = -1;
	    myPort->next = NULL;
	 } else {
...
#endif
                if (getpeername(myPort->sock, (struct sockaddr *) &raddr,
                  &laddrlen) < 0) {
	            fprintf(stderr, "serverLoop: Bad packet:%10s\n", 
	             myPort->buf.data);
		} else {
	            fprintf(stderr, "serverLoop: Bad packet:%10s from %s\n", 
	             myPort->buf.data, inet_ntoa (raddr.sin_addr));
		}
#endif
	    }
...
int reaper()
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
        tmpPort = PortHead;
	/* see if srbServer killed before scheduled */
	while (tmpPort != NULL) {
	    if (tmpPort->pid == pid) {
		tmpPort->preSpawn = -1;
		break;
...
serverStartup(Port *port)
{
    ...
    if (doExec((int)hChildWrite) < 0)
    {
        errno = GetLastError();
        elog(WARN, 
	 "child[%d]: serverStartup: spawnv failed (error %d)\n",pid, errno);
        return(STATUS_ERROR);
    }
	...
	close (pipeFd0[0]);
	close (pipeFd1[1]);
        if (doExec(pipeFd0[1], pipeFd1[0], -1))
            fprintf(stderr, 
	     "child[%d]: serverStartup: execv failed, errno=%d\n",
             pid, errno);
        _exit(1);
    }
	...
	if (errno == EINTR) {
	    continue;
    	    ...
    	    fprintf(stderr, 
	     "serverStartup: pipe read failed, errno=%d pid=%d, status=%d\n", 
	     errno, pid, status);
	    kill (pid, SIGTERM);
    	    close (pipeFd0[0]);
    	    close (pipeFd1[1]);
...
#else
static int
doExec(int portFd0, int portFd1, int sock)
#endif
{
#ifdef _WIN32
    char        execbuf[1024];
    ...
    if (portFd0 >= 0) {
        /* Tell the srbServer the descriptor of the  pipe */
       ...
       /* Tell the srbServer the descriptor of the  socket */
        (void) sprintf(portbuf0, "-S%d", sock);
        av[ac++] = portbuf0;
    }
...
/*
 * serverStartupWithOnePort -- Start the srbServer process using one port.
 * no need to use pipe
 *
 * Input : StartupInfo *packet - client's startup packet.
 *         Port *port - The Port associated with the connection.
...
#ifdef _WIN32
    /* XXXXX Bing need to look into this. At least compile won't failed */
    if (doExec((int)port->sock) < 0)
    {
        errno = GetLastError();
        elog(WARN,
         "child[%d]: serverStartupWithOnePort: spawnv failed (error %d)\n",
	 pid, errno);
        return(STATUS_ERROR);
    }
