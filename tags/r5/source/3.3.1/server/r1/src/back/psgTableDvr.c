...
int
psgTableOpen(dbTableDesc *exfd, char *resourceLocationName,
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "psgTableOpen: Error in resourceLocationName : %s",
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "psgTableOpen: Malloc error");
    ...
    if ((i =  open_env_psgTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "psgTableOpen: Error in Opening Environment: %i",i);
      psgTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_psgTab( (long int)  stateInfo->environ, 
			   (long int *) &stateInfo->connect, 
			   (unsigned char*) database, 
			   (unsigned char*) uid, 
			   (unsigned char*) pwd))!= 0)  { 

      elog (NOTICE, "psgTableOpen: Error in Connecting to %s: %i",
	    database,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      close_env_psgTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if (i != 0) {
      elog (NOTICE, "psgTableOpen: Error in Extracting Options from Command %s: %i",
	    dataPathName,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      close_env_psgTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
	...
	if ((tmpPtr2 = strstr(tmpPtr, " ")) == NULL &&
	  ...
	  exfd->dbSpecificInfo = (char *)stateInfo; 
	  i = getColumnInfoForpsgTable((long int)stateInfo->environ, 
	...
	*tmpPtr2 = tmpChar;
	...
	if ((tmpPtr3 = strstr(tmpPtr2, ")")) == NULL) {
	  elog (NOTICE, "psgTableOpen: Insert Statement Format Error: %s",
		dataPathName);
	  disconnect_psgTab( (long int)stateInfo->environ, 
	...
	*tmpPtr3 = '\0';
	...
	for (i = 0; i < stateInfo->num_of_cols; i++) {
	  ...
	  stateInfo->xmlcolNameOpen[i] = NULL;
	  stateInfo->xmlcolNameClose[i] = NULL;
	  if (getColTypeFromColNamepsgTab((long int)stateInfo->environ, 
				(long int)stateInfo->connect,
	...
	if ((tmpPtr = strstr(dataPathName," from ")) == NULL &&
	  ...
	  elog (NOTICE, "psgTableOpen: Delete Statement Format Error: %s",
		dataPathName);
	  disconnect_psgTab( (long int)stateInfo->environ, 
			     (long int)stateInfo->connect, MDAS_CAT_ROLLBACK);
	  close_env_psgTab( (long int) stateInfo->environ); 
	...
	*tmpPtr3 = '\0';
	...
	if (i < 0) {
	  elog (NOTICE, "psgTableOpen:Error: %i in  getMultiInsertInformation");
	  close_env_psgTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return(i);
	}
	if ((stateInfo->storedSQLBuffer = 
	  ...
	  elog (NOTICE, "psgTableOpen:Error: Memory Allocation Error");
	  close_env_psgTab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return INP_ERR_FORMAT;
	}
	...
	return MDAS_SUCCESS;
        ...
        elog (NOTICE, "psgTableOpen:Error: Not a Known Type SQL Statement: %s",
	      dataPathName);
	close_env_psgTab( (long int) stateInfo->environ); 
	free(stateInfo); stateInfo = NULL;
	return INP_ERR_FORMAT;
	...
	strstr(dataPathName,"SELECT") != dataPathName) {
      ...
      if ((i=exec_no_res_sql_psgTab(stateInfo->environ,stateInfo->connect,
			(unsigned char *) dataPathName))!= 0) { 
	elog (NOTICE, "psgTableOpen: Error in Executing SQL %s: %i",
	      dataPathName,i);
	psgTableErrorReport(NULL, stateInfo->environ);
	disconnect_psgTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
    ...
    addToVarList(&(stateInfo->constantValues), "QT","UPDATE", "CHAR");
    if ((i=exec_sql_psgTab(stateInfo->environ,stateInfo->connect,
    ...
    if ((i= get_no_of_columns_psgTab(stateInfo->statement, 
				  &numCols))!= 0) {
      elog (NOTICE, "psgTableOpen: Error in Getting Num of Cols: %i",i);
      psgTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_psgTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_psgTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i = bind_default_row_psgTab((long int) stateInfo->statement, 
		   stateInfo->data,  
		   stateInfo->data_size, &numCols)) != 0) {
      elog (NOTICE, "psgTableOpen: Error in Binding Row %i: %i",ii,i);
      psgTableErrorReport(stateInfo->statement, stateInfo->environ);
      disconnect_psgTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
      close_env_psgTab( (long int) stateInfo->environ); 
    ...
    for (j=0; j < numCols; j++) {
      ...
      myprecision = 1;
      scale = -1;
      if ((i = get_column_info_psgTab((long int) stateInfo->statement,j+1, 
				   &displaysize,mycolname,
				   &coltype, buf_size, &mycolnamelen,
				   &nullable,
				   &myprecision,&scale)) != 0) {
	elog (NOTICE, 
	      "psgTableOpen: Error in Getting Column Information %i: %i",ii,i);
	psgTableErrorReport(stateInfo->statement, stateInfo->environ);
	done_with_default_result_psgTab((long int)stateInfo->statement, 
				     stateInfo->data,  
				     stateInfo->data_size, 
	...
	return DB_ERR_MALLOC;
      ...
      getDataTypeFromMap_psgTab(stateInfo->coltype[j],coltype);
...
int 
psgTableClose(dbTableDesc  *exfd)
{
  tableStateInfo *stateInfo;
  int i,ii;
  ...
  if (!strcmp(stateInfo->sqlType, "SELECT") &&
      ...
      stateInfo->num_of_cols > 0) {
    if ((i = done_with_default_result_psgTab((long int)stateInfo->statement, 
		       stateInfo->data,  
		       stateInfo->data_size, stateInfo->num_of_cols)) != 0) {
      elog (NOTICE, "psgTableClose: Error in Unbinding and Closing %i",i);
      psgTableErrorReport(NULL, stateInfo->environ);
      return(i);
    }    
  }
  if (!strcmp(stateInfo->sqlType, "MULTIINSERT")) {
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "psgTableClose: Error in Constructing SQL To Clean List: %i",ii);
	    return(ii);
	}
	if (ii == 1) {
  ...
  if ((i = disconnect_psgTab( (long int)stateInfo->environ, 
			   (long int)stateInfo->connect, MDAS_CAT_COMMIT)) != 0) {
    elog (NOTICE, "psgTableClose: Error in Disconnecting:%i",i);
    psgTableErrorReport(NULL, stateInfo->environ);
    return(DB_TAB_DISCONNECT_ERROR);
  }
  if ((i = close_env_psgTab((long int)stateInfo->environ)) != 0) {
    elog (NOTICE, "psgTableClose: Error in Closing Environment:%i",i);
    psgTableErrorReport(NULL, stateInfo->environ);
...
int 
psgTableRead(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(psgTableReadXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(psgTableReadXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(psgTableReadHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(psgTableReadHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(psgTableReadHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(psgTableReadTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(psgTableReadPACK(exfd,buffer,length));
...
int 
psgTableReadTEMPLATE(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  while ((i = get_row_psgTab((long int) stateInfo->statement)) == 0)       
    {
...
int 
psgTableReadHTMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  if ((i = get_row_psgTab((long int) stateInfo->statement)) != 0) {
    stateInfo->endOfData = 1;
    if (i == MCAT_NO_DATA_FOUND){
  ...
  while (i == 0)       
	...
	if (k <= ll) {
      ...
      i = get_row_psgTab((long int) stateInfo->statement);
    }
  stateInfo->endOfData = 1;
  if (i == MCAT_NO_DATA_FOUND) {
    strcpy(fillStr,"</TABLE>\n</BODY>\n</HTML>\n");
...
int 
psgTableReadHTMLNEST(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableReadHTMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableReadPACK(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableReadHEAD(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableReadXMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableReadXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
psgTableWrite(dbTableDesc *exfd, char *buffer, int length) 
{
/*
 ...
 */
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(psgTableWriteXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(psgTableWriteXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(psgTableWriteHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(psgTableWriteHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(psgTableWriteHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(psgTableWriteTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "SQL"))
    return(psgTableWriteSQL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(psgTableWritePACK(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "VALUES"))
    return(psgTableWriteVALUES(exfd,buffer,length));
  else
    return(psgTableWriteSQL(exfd,buffer,length));
}

int 
psgTableWriteSQL(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (mybuf == NULL)  {
    elog (NOTICE, "psgTableWrite: Memory Allocation Error");
    return(DB_ERR_MALLOC);
    }
...
int 
psgTableWriteVALUES(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
 tableStateInfo *stateInfo;
 int i, ii,j,k;
  ...
  while (*mybuf1 != '\0') {
    ...
    *mybuf2 = '\0';
    ...
    if ((i=exec_no_res_sql_psgTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "psgTableWrite: Error in Executing SQL %s: %i",
	    sqlString,i);
      free(mybuf);
      free(sqlString);
      return(i);
...
int 
psgTableWritePACK(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
psgTableWriteHTMLREL(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int
psgTableWriteHTMLTREE(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
psgTableWriteHTMLNEST(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
psgTableWriteXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
psgTableWriteXMLTREE(dbTableDesc *exfd, char *inbuffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
  ...
  while (1) {
    for (cntData = 0 ; cntData < stateInfo->num_of_cols ; cntData++) {
      ...
      if (i == DATA_TAG_ERROR) {
	elog (NOTICE, "psgTableWrite: Error in Data Tag for Buffer: %s", mybuf);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
	...
	*(tmpmmPtr+mmSize - 1 ) =  '\0';
	...
	elog (NOTICE, "psgTableWrite: Error in Data Tag: %s", dataTagOpen);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
...
int 
psgTableWriteTEMPLATE(dbTableDesc *exfd, char *inbuffer, int length)
{
 ...
 if (!strcmp(stateInfo->sqlType,"MULTIINSERT")) {
  while (1) {
	...
	elog (NOTICE, "psgTableWrite: Error in Processing Choice Rule: %i", j);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(j);
      }
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "psgTableWrite: Error in Constructing SQL  in List: %i",ii);
	    if (collapsePtr != NULL) free(collapsePtr);
	    collapsePtr = NULL;
	    return(ii);
	}
	...
	for (j = 0 ;  j < MAX_SQL_TAGS_COUNT ; j++) {
	  ...
	  if (!strcmp(stateInfo->compSql.tags[i][j], dataTag)) {
	    if (stateInfo->compSql.tagusage[i][j] == TAG_FULL) {
		  ...
		  < 0) {
		elog (NOTICE, 
		   "psgTableWrite: Error in Constructing SQL '%i' in List: %i",
		      i ,ii);
		if (collapsePtr != NULL) free(collapsePtr);
		collapsePtr = NULL;
		return(ii);
  ...
  while (1) {
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (!strcmp(dataTag,stateInfo->xmlcolName[i])) 
      ...
      if (i == stateInfo->num_of_cols) {
	elog (NOTICE, "psgTableWrite: Error in Data Tag: %s", dataTag);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	for (i = 0; i <= cntData; i++) {
	  if (stateInfo->data[i] != emptyStr)
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "psgTableCreate: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if(strstr(tableCreateStmt,"create") == NULL &&
       strstr(tableCreateStmt,"CREATE") == NULL) {
        elog (NOTICE, "psgTableCreate: Error: Not a Create Statement : %s",
        tableCreateStmt);
        return INP_ERR_RES_FORMAT;
    }

    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "psgTableCreate: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_psgTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "psgTableCreate: Error in Creating Environment: %i",i);
      psgTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_psgTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "psgTableCreate: Error in Connecting to %s: %i",
	    database,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      close_env_psgTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_psgTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "psgTableCreate: Error in Executing SQL %s: %i",
	    tableCreateStmt,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      disconnect_psgTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
 ...
 */
    ...
    if (mySplitStr(resourceLocationName, hostAddr, tmpStr, ':') == NULL) {
        elog (NOTICE, "psgTableUnlink: Error in resourceLocationName : %s",
        resourceLocationName);
        return INP_ERR_RES_FORMAT;
    }
    if (mySplitStr(tmpStr, database, instance, ':') == NULL) {
    ...
    if((stateInfo = malloc(sizeof (tableStateInfo))) == NULL) {
      elog (NOTICE, "psgTableUnlink: Malloc error");
      return DB_ERR_MALLOC;
    }
    ...
    if ((i =  open_env_psgTab((long int *)&stateInfo->environ))!= 0)  {
      elog (NOTICE, "psgTableUnlink: Error in Unlinking Environment: %i",i);
      psgTableErrorReport(NULL, stateInfo->environ);
      free(stateInfo); stateInfo = NULL;
      return(DB_TAB_OPEN_ENV_ERROR);
    }
    if ((i =  connect_psgTab( (long int)  stateInfo->environ, 
      ...
      elog (NOTICE, "psgTableUnlink: Error in Connecting to %s: %i",
	    database,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      close_env_psgTab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
    ...
    if ((i=exec_no_res_sql_psgTab(stateInfo->environ,stateInfo->connect,
      ...
      elog (NOTICE, "psgTableUnlink: Error in Executing SQL %s: %i",
	    queryString,i);
      psgTableErrorReport(NULL, stateInfo->environ);
      disconnect_psgTab( (long int)stateInfo->environ, 
		      (long int)stateInfo->connect, MDAS_CAT_ROLLBACK) ;
...
executeCompoundSql(tableStateInfo *stateInfo, char *sqlbuf)
{
  ...
  if (reverseExecution(stateInfo)) {
	...
	break;
      myChar = *mybuf2;
      *mybuf2 = '\0';
      if ((i=exec_no_res_sql_psgTab(environ, connect,
