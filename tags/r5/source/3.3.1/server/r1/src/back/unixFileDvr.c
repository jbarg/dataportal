...
#ifdef SAMFS_STAGE
int 
samfsFStage (char *path, int flag)
{
    int status;
    ...
    if (status < 0) {
	status = unixErrConv (errno);
        elog (NOTICE, "samfsFStage: sam_stage error, status = %d\n",
         status);
    }
...
/* unixChown - Handles the chown call.
 *
 * Input : char *filename - The path name to chown
 *	   char *fileUser - not used
 *	   char *fileGroup - the group to change to.
 *	   int chownFlag - not used
 *
 * Output : Returns the status of chown
 */

int
unixChown (char *filename, char *fileUser, char *fileGroup, int chownFlag)
{
    struct group *myGroup;
    int status;

    if (fileGroup == NULL || strlen (fileGroup) == 0) {
	return 0;
    }

    myGroup = getgrnam ((const char *) fileGroup);
    if (myGroup == NULL) {
        status = unixErrConv (EINVAL);
        elog (NOTICE, "UNIX chown error for %s. Invalid group %s", 
         filename, fileGroup);
        return (status);
    }

    status = chown(filename, -1, myGroup->gr_gid);

    if (status < 0) {
        status = unixErrConv (errno);
        elog (NOTICE, "UNIX chown error for %s. errorCode = %d", 
	 fileGroup, status);
        return (status);
    } else
        return (status);
