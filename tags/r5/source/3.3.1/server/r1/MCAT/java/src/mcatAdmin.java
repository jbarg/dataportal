...
/*****************************************************************************/
/*                                                                           */
/*  mcatAdmin.java is the main module in the MCAT/SRB Java admin tool.       */
/*  It creates a panel with pull-down menus which, when selected,            */
/*  create the other objects which in turn create their own panel-windows.   */
/*  It also handles the Help option which creates another main panel         */
/*  like this one but then displays help info when any of the pull-down      */
        ...
        implements      ActionListener
{
    ...
    public static String SHOW_USERS = "Display Users (or Groups) Information";
    public static String NEW_USERS = "Add New Users";
    public static String MOD_USERS = "Modify User (or Group) Info";
    public static String MOD_USERS_GROUP = "Modify User Group Info";
    ...
    mcatModUserGroup myModUserGroup;
    ...
    private JMenuItem       menuManUsersModGroup;
	...
	if (doingHelp==false) {
		...
		this.setTitle("SRB Admin - not connected");
	    }
	    else {
		this.setTitle("SRB Admin - " + fileSystem.getHost());
	    }
	    setSize( 310, 130 );
	}
	else {
...
"This is the Help window for the SRB/MCAT administration tool.\nClick on a pull-down menu item to get a description of that function.\nUse the File/Help pull-down option for a general introduction.\n");
	...
	menuManUsersNew = CreateMenuItem( menuManUsers, ITEM_PLAIN,
        ...
        menuManUsersModGroup = CreateMenuItem( menuManUsers, 
                      ITEM_PLAIN,
                      MOD_USERS_GROUP, null, 'M', 
		      "Modify User Group-related Info" );
	...
	// Add a window listener so we exit when the main window is closed
	...
	if (doingHelp == true) {
        ...
        if (actionCommand.equals(MOD_USERS_GROUP)) {
	    if (myModUserGroup == null) {
		myModUserGroup = new mcatModUserGroup (fileSystem);
	    } else {
		myModUserGroup.setVisible(true);
	    }
	}
    ...
    public void actionPerformedHelp( ActionEvent event )
    {
	   ...
	   SHOW_USERS, NEW_USERS, MOD_USERS, MOD_USERS_GROUP, 
           ADD_GROUP, DEL_USERS,
    ...
    " - Creates one or more Zones; ie adds Zone information to the local MCAT.  These can be either local or remote.  Only one local zone can be created.  When you create a local Zone, be sure to reserve and register the name with the SRB Zone Authority to make it globally unique in the namespace, at http://www.sdsc.edu/srb/zoneAuthority.html\n\nFollow these steps to create a zone:\n\n1) Create the new Domain, using the name used at the remote site.\n2) Create the admin user for the new Domain, using the user name and Domain used at remote site; type is 'sysadmin'.\n3) Create a new Location for the remote site, using the admin user name and domain used at remote site.\n4) Create a remote Zone, using the admin user name, Domain, and Location used at the remote site.\n5) Modify the new admin user, changing their home Zone from yours to the remote Zone.\n\nBe sure to complete step 5 to prevent the remote admin from have full privileges in your zone.  The user account has to be defined for step 4, but then needs to be changed to be part of the remote zone as described in step 5.",

    " - Modifies one or more Zones attributes.  All the significant fields that are specified when a Zone is created can be changed, even including the name and the local/remote flag.  Be sure to register/reserve local zone names with the SRB Zone Authority at http://www.sdsc.edu/srb/zoneAuthority.html\n",

    " - Displays information concerning selected users.  Select a domain, and it will display the users that are part of that domain.  Then select a user, and information on that user will be displayed.\n\nA special domain is 'groups', which will actually return groups instead of users (groups are stored this way in the MCAT).  Click on a group name to get additional information about that group.  Group owners, if there are any for this group, will show up in the address field as <groupowner=user@domain>.  For more group information and control, see the Display Users (or Groups) Information panel and the modify user group information panel.\n",

    " - Adds a new SRB user to your system.\n\nYou enter a name and password for the user, and click on one of the available domains and user types.  Phone number, Email, and Address are optional.  The parent collection is always /home; this may be adjustable in the future.\n\nYou may want to add your own domain before adding users (Tokens/Domain).  Other user types can be also be added (Tokens/Add New Token).\n\nThere are two types of users with special privileges: sysadmin and domainadmin.  sysadmin has all SRB privileges.  domainadmin is allowed to add and delete users within their own domain and to add and remove datatype definitions.\n",

    " - Modifies attributes of SRB users (or groups).\n\nClicking on a domain will list the users that exist in that domain.  You then select an operation and it will display available choices.  This panel provides some operations and the 'Modify User Group Info' panel proivdes for group-relation operations.\n\nIf you select the domain item called 'groups', the groups will be listed.  The only items you may want to change here would be the information fields or password.  Group ownership text appears in the Address fields, but should be modified via the 'Modify User Group Info' panel.\n\nYou can remove the various authentication schemes from a user, but if you remove 'ENCRYPT1' it actually is still available and functional.  This is because the main purpose of the add/remove authentication scheme is to add and remove the Distinguished Names for schemes that use them, and the ENCRYPT1 scheme is a base authentication system in the SRB.  To workaround this limitation, you can change the user's password to an arbitrary value.\n\nIf you change a user's type to, or from, one of the privileged types (sysadamin or domainadmin), the change will take effect after the pre-spawned SRB servers are used.  The processing that is completed as an SRB server is pre-spawned includes some authorization determininations.  A few Sinit/Sexit's will clear these out.\n",

    " - Modifies attributes of SRB users: Group-related items.\n\nClicking on a domain will list the users that exist in that domain.  You then select an operation and it will display available choices.  This panel allows you to add or remove users from a group, or to add or remove users as owners of a group.  For any of these, you need to be either sysadmin or an owner of the group.\n\nUsers always belong to two special  groups: 'public' and a group with the same name as their userid.  If you attempt to remove users from these groups an error will be returned (USER_GROUP_NOT_IN_CAT).  But for completeness, these special groups are included in the group list display.\n\n",
    ...
    " - Adds a new Token.  A Token is a descriptive item in the MCAT database.  There are quite a few predefined Tokens and you can add more via this if you want.  The Tokens can be either a ResourceType, DataType, UserType, Domain, or Action.  The predefined UserTypes include 'assistant manager', 'grad student', 'staff', etc.  But if you wanted another UserType, say 'slacker', you could add it via this.  In this UserType case, a Parent Value is required and it needs to be one of the existing UserTypes (the window used to create new users will show them).\n\nThe extensions and mimetypes apply only to Datatype tokens.\n\nThe extensions are like the Windows extension portion of file names, the part used to identify a file type, for example: .doc, .txt, .c, and .pl.  It is used by the SRB in finding what is the data type of the given file.  For example, if a user Sputs a file called 'foo.doc' it uses the extension info to give it the SRB data type 'MSWord Document' as its datatype.\n\nIf you add a new DataType token, you'll need to provide a Parent Value which places it in a hierarchy.  If you are not sure what to use, 'home' (without the quotes) will work fine.  Also, when you enter the extension(s), you can enter multiple ones separated by commas, for example for the existing 'tiff image', '.tif,.tiff' (without quotes).\n\nThe SRB mimetype is sent as a web mimetype so that the inQ and mySRB browsers can launch the correct application for each type of file that was downloaded.",
...
// after the user enters parameters and clicks execute
	...
	try {
	    ...
	    clearObjs();
	    this.setTitle("SRB Admin - " + myHost);
	    return 1;
	}
        catch (SecurityException se) {
