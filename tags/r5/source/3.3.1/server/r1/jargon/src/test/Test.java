...
These SRB Scommand equivalents are currently available in this API:
 ...
 Sattrs     - javadoc
 Sbload     - LocalFile.copyTo( SRBFile dir ) and SRBFile.copyFrom( LocalFile dir )
 Sbunload   - LocalFile.copyFrom( SRBFile dir ) and SRBFile.copyTo( LocalFile dir )
 Scat       - SRBRandomAccessFile.read
 Scd        - SRBFile constructor
 Schdefres  - SRBFileSystem.setDefaultStorageResource
 Schhost    - SRBAccount.setHost, must reconnect SRBFileSystem.
 Schksum    - SRBFile.checksum
 ...
 SmodR      - SRBFileSystem.srbModifyResource (still low-level, see also SRBAdmin tool)
 SmodifyUser- SRBFileSystem.srbModifyUser  (still low-level, see also SRBAdmin tool)
 ...
 Spullmeta  - SRBFile and SRBFileSystem.query
 Spushmeta  - SRBFile and SRBFileSystem.query
 Sput       - SRBFile.copyFrom
 Spwd       - SRBFile.getAbsolutePath
 Sregister  - SRBFile.register
 Sreplcont	- SRBContainer.replicate
 Sreplicate - SRBFile.replicate
 Srm        - SRBFile.delete
 Srmcont    - SRBContainer.delete
 Srmdir     - SRBFile.delete
 Sufmeta    - see other metadata commands on this list.
 Szone      - SRBFileSystem.srbModifyZone (still low-level, see also SRBAdmin tool)
...
//----------------------------------------------------------------------
	...
	{
		//Turn on debugging. Higher positive integers mean more debugging.
		//System.setProperty("jargon.debug", "1");
		// or use
		//java -Djargon.debug=3 MetaDataTest
		...
		try{
			...
			//SRBAccount.setVersion(SRBAccount.SRB_VERSION_2);
...
System.out.println("\n Add read/write permissions to the file for \"testuser\".");
			...
			MetaDataTest.printQueryResults(srbFile.getPermissions(true));
...
System.out.println(
	...
	" put it in the current local directory as myLocalJargonDir.");
			/**
			...
			//using constructor SRBFile( SRBFileSystem fileSystem, String filePath )
			srbFile = new SRBFile( srbFileSystem, "mySRBJargonTestDir" );
			localFile = new LocalFile( "myLocalJargonDir" );
...
System.out.println("\n Run the MetaDataTest using ");
			/**
			 * Test the metadata.
			 */
			new MetaDataTest(
				new SRBFile( srbFileSystem,
					"mySRBJargonTestDir/anotherSRBJargonTestFile" ));
