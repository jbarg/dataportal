...
public class CrystalDisplay3D2 extends JFrame{
...
/* Initial method run when application starts. */
  public static void main( String args[] ) throws MalformedURLException {
    ...
    CrystalDisplay3D2 c3d = new CrystalDisplay3D2( u, args );
  }
  ...
  public void openFile( URL u, String fileName ){
	...
	// Structure file.
	co = (CrystalObject)( new CrystalStructureLoader( furl.openStream(),
							  (Component)this ) );
	co.name = fileName.substring( fileName.lastIndexOf( '/' )+1, fileName.lastIndexOf( '.' ) );
	...
	co = (CrystalObject)( new CrystalDataLoader( furl.openStream(),
						     (Component)this ) );
	co.initialiseFeatures( bohrOrigin );
	co.setCentreA( angstromOrigin );
	addMenu.add( co.getAddMenu( optionsMenu ) );
      }
      else if ( fileName.endsWith( "c3d.gz" ) ){
	// Gzipped IsoSurface data file
	co = (CrystalObject)( new CrystalDataLoader( new GZIPInputStream( furl.openStream() ),
						     (Component)this ) );
	co.initialiseFeatures( bohrOrigin );
	co.setCentreA( angstromOrigin );
	addMenu.add( co.getAddMenu( optionsMenu ) );
