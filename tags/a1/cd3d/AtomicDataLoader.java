...
public class AtomicDataLoader{
  public static void main( String args[] ) throws Exception{
	...
	if ( st.sval.startsWith( "Atom" ) ){
	  ...
	  //	  if ( atomicNumber> 56 ) System.exit( 0 );
	  dt = st.nextToken();
	  symbol = st.sval;
	  while ( st.ttype != StreamTokenizer.TT_NUMBER &&
		  st.ttype != StreamTokenizer.TT_EOL ) dt = st.nextToken();
	  ...
	  // System.out.println( "Token type = " + tokenType( st.ttype ) );
	  dt = st.nextToken();
	  col1 = (float)st.nval;
	  dt = st.nextToken();
	  col2 = (float)st.nval;
  ...
  public static String tokenType( int tt ){
    String s = "";
    switch( tt ){
