...
int chkFlag, int pathType)
{
	...
	if (chkFlag == CHK_FILE_PATH) {		/* check the file path only */
	    ...
	    status = mkVaultDir (storSysType, fileNameBuf, pathType);
	}
 ...
 */
int
mkVaultDir (int storSysType, char *filePath, int pathType)
{
    struct stat statbuf;
    int pathlen, status;
    int vaultLen, tmpLen;
    ...
    while (tmpLen > 0) {
        if (exfstat(storSysType, NULL, dirname, (char *) &statbuf) == 0)
	...
	/* Put back the '\' */
	dirname[tmpLen] = '/';
	status = exfmkdir(storSysType, NULL, dirname, 0750);
        if (status != 0) {
...
int mode, int chdirFlag, srb_long_t size, int remoteConnFlag)
{
    ...
    if (remoteFlag < 0) {
	    ...
	    mkVaultDir (storSysType, fileName, FILE_T);
	}
#endif
...
srb_long_t myOffset, srb_long_t mySize)
{
    ...
    if (bytes_read < 0) {
	  ...
	  fileName, bytes_read);
        exfclose (storSysType, hostAddr, fd);
        *ichksum = 0;
        return (bytes_read);
    }

#ifdef foo	/* XXXXX close called  by the caller now */   
    if (exfclose (storSysType, hostAddr, fd) < 0) {
      fprintf(NOTICE, "svrFileSum: Error closing file %s", fileName);
    }
#endif
