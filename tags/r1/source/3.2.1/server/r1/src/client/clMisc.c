...
portalCreate () {
    ...
    curPortNum = COMM_PORT_NUM_START;

    while (1) {

	if (curPortNum >= COMM_PORT_NUM_START + COMM_PORT_NUM_COUNT)
	    curPortNum = COMM_PORT_NUM_START;
...
*/
    ...
    while (connectCnt < MAX_CONN_SVR_CNT) {
        if ((status = connect (lsock, (struct sockaddr *) &sin, sizeof (sin))) 
	 < 0) {
	    connectCnt ++;
    ...
    if (cookie != 0) {
 	cookie = htonl (cookie);
#ifdef _WIN32
         nbytes = send(lsock, &cookie, sizeof (cookie), 0);
#else
...
#else
        srbHtonll (&tmpdirent->d_ino);
        srbHtonll (&tmpdirent->d_off);
#endif
        tmplen += savlen;
	 ...
	 sizeof (myHeader));
      ...
      switch (myHeader.operation) {
       ...
       case GET_OPR:
         ...
         if (toread > 0) {
            sprintf (myPortalInput->errorMessage, 
             "portalTransfer: %d bytes read at offset %d does not equal to header %d\n",
         ...
         if (curOffset != myHeader.offset) {
            ...
            if (curOffset != myHeader.offset) {
               sprintf (myPortalInput->errorMessage,
                "portalTransfer: lseek of %s to %d error, errno = %d\n",
         ...
         if (nbytes != myHeader.length) {
            sprintf (myPortalInput->errorMessage,
             "portalTransfer: %d bytes written at offset %d does not equal to header %d\n",
       ...
       case PUT_OPR:
	    ...
	    if (localFd < 0) {
         ...
         if (toread <= 0) {
            if (toread < 0) {
               sprintf (myPortalInput->errorMessage,
                "portalTransfer: read of %s to %d error, errno = %d\n",
                  myPortalInput->locFilePath, myHeader.offset, errno);
            }
            status = unixErrConv (errno);
            break;
...
#ifdef _WIN32
      nbytes = recv(fd,(void *) myHeader, sizeof (struct relayHeader), 0);
#else
      nbytes = read (fd, (void *) myHeader,
        sizeof (struct relayHeader));
#endif
     if (nbytes != sizeof (struct relayHeader)) {
        fprintf (stderr,
