...
void get_ctype_value(char cval[], char name[], char val[])
{
    char *tmpPtr;
		...
		int *rsrc_id, int *out_repl_enum)
{
  ...
  if (given_repl_enum < 0) {
    sprintf(sqlq, "select  /*+ FIRST_ROWS */ t1.rsrc_id, t1.repl_enum from %sMDAS_AR_REPL t1 where t1.rsrc_name = '%s'", 
	  MDASSCHEMENAME, rsrc_name);
  }
  else {
    sprintf(sqlq, "select  /*+ FIRST_ROWS */ t1.rsrc_id, t1.repl_enum from %sMDAS_AR_REPL t1 where t1.rsrc_name = '%s' and t1.repl_enum = %i)", 
...
{
  int i,j,k;
	  sprintf(sqlq, "select  /*+ FIRST_ROWS */ t1.data_grp_id from %sMDAS_TD_DATA_GRP t1, %sMDAS_AU_GROUP t2, %sMDAS_AD_GRP_ACCS t4,  %sMDAS_TD_DS_ACCS t5  where t1.data_grp_name = '%s'  AND t2.user_id =  %i and   t4.user_id  =  t2.group_user_id AND t5.access_privilege like '%%%s%%' AND t4.data_grp_id = t1.data_grp_id AND t4.access_id = t5.access_id",
	  MDASSCHEMENAME, MDASSCHEMENAME, MDASSCHEMENAME, 
...
int get_max_version_num(int obj_data_id, int repl_enum)
{
...
int register_dataset_info(int cat_type, char *obj_data_name, char *obj_user, 
			char *obj_access_name, char *obj_domain_name, 
			char *obj_type_name,   
			char *my_obj_path_name,   char *resource_name,
			char *obj_group_name,  srb_long_t obj_size)
{ 
			...
			char * domain_name)
{
  ...
  else {/* its collection related metadata */
  ...
  switch (retraction_type)
    ...
    case D_DELETE_FROM_EXTMD_TABLE:
                         /* data_1 contains one or more attribute names in that table 
			    separated by | and
			    data_2 contains the comparison-values corresponding to
			    those attributes. Attribute names data_id and collection_id
			    are keywords and will be substituted. 
			    Use 0 as place-holder
			 */
			 break; 
    case D_INSERT_INTO_EXTMD_TABLE:
                         /* data_1 contains one or more attribute in that table and
			    data_2 contains the values corresponding to
			    those attributes. New row is inserted
			 */
			 break; 
    case D_MODIFY_IN_EXTMD_TABLE:
			    ...
			    data_3 contains one or more attribute in that table and
			    data_4 contains the comparison-values corresponding to
...
int modify_dataset_info(int cat_type, char *obj_data_name,
			char *collection_name,
...
{ 
  ...
  char new_data_name[MAX_TOKEN * 2];
...
*/
   ...
   if (retraction_type > SYS_ACTION_TYPE_MIN && retraction_type < SYS_ACTION_TYPE_MAX) {
     ...
     if (inContainerObject == 0) 
       i =get_data_id_with_collection(obj_data_name, obj_path_name, 
		   resource_name,
		   obj_user_name, collection_name,  domain_name,
		   &obj_data_id, &replication_num, &obj_user_id);
     else
...
#else
			   sprintf(sqlq, "update MDAS_AD_REPL set version_num = '0' where data_id = %i and repl_enum = %i and version_num in (select max(version_num) from %sMDAS_AD_REPL where data_id = %i and repl_enum = %i)",obj_data_id, replication_num,MDASSCHEMENAME,obj_data_id,replication_num);
			   i = ExecSqlDb2(henv, hdbc, sqlq);
#endif
			 }
 ...
 elog(NOTICE,"d1=%s,d2=%s.\n",data_value_1,data_value_2);
			   ...
			   *tmpPtr = '\0';
			   oldVerNum =  atoi(data_value_2);
			   newVerNum =  atoi(tmpPtr+1);
			 }			 
			 ...
			 if (newVerNum == -1) {
			   if (all_replicas)
			     intval2 = get_max_version_num(obj_data_id,-1);
			   else
			     intval2 = get_max_version_num(obj_data_id,intval);
			   if (intval2 < 0) 
			     return (intval2);
			 }
			 else 
			   intval2 = newVerNum;
elog(NOTICE,"v=%i,r=%i,or=%i,ov=%i.\n",intval2,intval,oldReplNum,oldVerNum);
			 sprintf(atname[0],"version_num");
			 sprintf(atname[1],"repl_enum");
			 sprintf(atname[2],"data_id");
			 sprintf(atname[3],"repl_enum");
			 sprintf(atname[4],"version_num");
			 sprintf(atval[0],"%i",intval2);
			 sprintf(atval[1],"%i",intval);
			 sprintf(atval[2],"%i",obj_data_id);
			 sprintf(atval[3],"%i",oldReplNum);
			 sprintf(atval[4],"%i",oldVerNum);
			 if (oldVerNum == -1) 
    ...
    case D_UPDATE_PIN:
			 ...
			 if (check_exists(sqlq) != 0) {
			   ...
			   if (all_replicas == 0) {
			     sprintf(sqlq,"select  /*+ FIRST_ROWS */ * from %sMDAS_AD_MISC1 where  data_id = %i and repl_enum = %i and (dpinowner =  %i or dpin = 0)", MDASSCHEMENAME, 
				     obj_data_id,replication_num,obj_user_id);
			   }
			   else {
			     sprintf(sqlq,"select  /*+ FIRST_ROWS */ * from %sMDAS_AD_MISC1 where  data_id = %i  and (dpinowner =  %i or dpin = 0)", MDASSCHEMENAME, obj_data_id,obj_user_id);
			   }
			   if (strlen(given_version_num) > 0 && 
			       all_replicas == 0){
...
{ 
  ...
  get_ctype_value(data_group_id, "MDAS_TD_DATA_GRP.data_grp_name", group_name);
   ...
   if (retraction_type > CURATOR_ACTION_TYPE_MIN && retraction_type < CURATOR_ACTION_TYPE_MAX) {
...
*******/
    ...
    if (retraction_type == U_CHANGE_PASSWORD || retraction_type == U_BULK_INSERT_UDEF_META_DATA_FOR_USER ) {
      obj_user_id = get_user_id_in_domain(obj_user_name,user_domain_name);
      if (obj_user_id < 0) { return (USER_NOT_IN_DOMN);}
      reg_registrar_id = obj_user_id;
    }
      ...
      i = is_system_userid(reg_registrar_id);
      if (i != 0) return(i);
      newUserDomain = strstr(data_value_1,"@");
  ...
  get_time(timeval);
   ...
   switch (retraction_type)
    ...
    case U_DELETE_GROUP:
			 ...
			 tabledIsOkay = 1;
			 intval = get_user_id_in_domain(data_value_2, 
							"groups");
			  tabledIsOkay = 0;
...
int move_dataset(int cat_type, char *obj_data_name, 
		 ...
		 char *domain_name)
{ 
 char   my_new_obj_path_name[MAX_TOKEN];
 char   my_old_obj_path_name[MAX_TOKEN];
 char tmpStr[MAX_TOKEN];
  ...
  make_quote_to_two_quote(old_obj_path_name,my_old_obj_path_name);
    i =get_data_id_with_collection(obj_data_name, my_old_obj_path_name, 
				   old_resource_name, 
		   obj_user_name, collection_name,  domain_name, 
		   &obj_data_id, &replication_num, &obj_user_id);
  if (i < 0)
  ...
  sprintf(atval[1],"%i",obj_data_id);
  sprintf(atval[2],"%i",replication_num);
  i = changeAttrValue(atname, atval,"MDAS_AD_REPL", 3);
  if (i !=0) return (MDAS_AD_REPL_UPDATE_ERROR);
  sprintf(atname[0],"rsrc_id");
  sprintf(atname[1],"data_id");
  sprintf(atname[2],"repl_enum");
  ...
  char tmpStr[MAX_TOKEN];
   ...
   strcpy(reg_action_name,"copy");
    ...
    /** Raja Aug 12, 2002 - changed to ignore obj path name ****/
      ...
      * container with a good and bad copy. my_old_obj_path_name[0] = '\0'; */
...
int auditDatasetAccess(int cat_type, char *obj_user_name,
		       char *obj_data_name, char *collection_name,
		       char *my_obj_path_name,
			 char *resource_name, 
...
{ 
  ...
  char distinInfo[MAX_TOKEN];
   ...
   if ( i != 0)    
     {
       sprintf(query_string,"user registry API: Failed Password:%s,%s\n",
	  registrar_password,user_name);
       return(SYS_USER_AUTHORIZATION_ERROR);
     }
  ...
  sprintf(query_string, "insert into %s%s (user_id,priv_key) values (%i,'%s')",
	  MDASSCHEMENAME, "MDAS_AU_AUTH_KEY", ival[0],user_password);
  res = ExecSqlDb2(henv, hdbc, query_string);
  if (res != 0)  
...
int check_permit_in_dataid_by_access_id(int data_id, int user_id, int access_id)
{
  ...
  sprintf(sqlq, "select distinct t1.data_id from %sMDAS_AD_ACCS t1, %sMDAS_AU_GROUP t3 where  t1.data_id = %i AND t3.user_id = %i AND t3.group_user_id = t1.user_id AND t1.access_id >= %i",
...
int check_permit_in_dataid(char *obj_access_name, int data_id, int user_id)
{
   ...
   int i;
     ...
     sprintf(sqlq, "select distinct t2.access_constraint from %sMDAS_AD_ACCS t1, %sMDAS_TD_DS_ACCS t2, %sMDAS_AU_GROUP t3 where  t1.data_id = %i  AND t3.user_id = %i AND t3.group_user_id = t1.user_id AND  t2.access_privilege like '%%%s%%' and t1.access_id >= t2.access_id",
	     MDASSCHEMENAME, MDASSCHEMENAME,MDASSCHEMENAME,
...
int open_db2_interaction(int cat_type)
{
  ...
  return(open_db2_interaction_internal(
		  &henv, &hdbc));
}
...
int
deleteMultiAccs(char* registrarId, char *data_group_id, int userId, int accessId, char *acType)
{
  int i,j,a, res;
