...
/******************** TEST get_loid_info TEST ************************/
  /*  TEST CASE:   test_srb_mdas_get 001.001 sekar <passwd> all legion  */
	...
	if ( argc  != 7 && argc  != 8 ) {
	  printf("Error: Use test_srb_mdas_get <dataset-name> <user-name> <passwd> <access-name> <domain-name> <colection-name> <rows-required>\n");
	  exit(-1);
	}
...
print_results()
{
   ...
   sprintf(cval,"%sMDAS_ADC_REPL",MDASSCHEMENAME);
   ...
   res_data_name = (char *)get_from_result_struct(
             (mdasC_sql_result_struct *) myresult, cval,"data_name");

   i = 0;
   while (i < myresult->row_count)
     {
       printf("DATA  NAME    : %s\n",res_data_name + i*  MAX_DATA_SIZE);
       ...
       condition [0] = '\0';
       if (strstr(data_name,"&") != NULL) {
	 splitbychar (data_name,tmpDataName, (char *)&condition[1], '&');
	 condition[0] = '&';
       }
	 sprintf(tmpPath,"%s",res_data_path_name + i*  MAX_DATA_SIZE);
	 status = getFullPathName (tmpPath, "",
				   m_user_name, domain_name,
				   condition, data_name, collection_name,
				   newPath);
	 if (status != 0) 
	   printf("WRONG FULL PATH: %i\n",status);
	 else
	   printf("FULL PATH     : %s\n",newPath);
       i++;
       printf(" ------------------------------------------------------------ \n");
