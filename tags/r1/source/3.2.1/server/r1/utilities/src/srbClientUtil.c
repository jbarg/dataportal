...
void vNiceVerboseProgressMessage (int flagval, int iIntoSrb, char *srbName,
			      char *UnixName, long lBytes, long lTotalSize, float fSec, float fBs);
...
void vNiceVerboseProgressMessage (int flagval, int iIntoSrb, char *srbName,
				   char *UnixName, long lBytes, long lTotalSize, 
				   float fSec, float fBs)
{
  ...
  eta =  (unsigned long) ((lTotalSize - lBytes)/fBs);
  sprintf (etastr, "%02ld:%02ld:%02ld", eta / 3600,
	   (eta % 3600) / 60, eta % 60);
  
  len = fprintf (stdout,
		 "\r%.0f MB | %.2e B/s | %.0f s | ETA: %8s | %d%%",
		 (float) (lBytes / (1024.0 * 1024.0)), fBs, fSec, etastr,
		 (int) (100 * (lBytes * 1.0 / lTotalSize)));
...
#ifdef foo	/* fail for incontainer obj. no need to verify anyway */
  if (iReadChecksumFromSrb (conn, srbDataName, srbCollection, &ulSum2) != 0) {
    fprintf(stderr,"iWriteCheckSumIntoSrb: Error in verification attempt of the checksum from MCAT :\"%s/%s\"\n", 
	    collName,dataName);
    srb_perror (2, i, "", SRB_RCMD_ACTION|SRB_LONG_MSG);
    return(-1);
  }
  if (ulSum != ulSum2) {
    fprintf(stderr,"iWriteCheckSumIntoSrb: Error comparing checksums:\"%s/%s local checksum = %ld, MCAT checksum = %ld\"\n", 
	    collName,dataName, ulSum, ulSum2);
    srb_perror (2, i, "", SRB_RCMD_ACTION|SRB_LONG_MSG);
    return(-1);
  }
#endif	/* foo */
...
int firstInitSrbClientEnv()
{
  ...
  {
#ifdef _WIN32
	  if(get_home_env() != NULL)
		sprintf(EnvFilePath, "%s/%s",get_home_env(),"srb/MdasEnv");
	  else
		  sprintf(EnvFilePath,"/srb/MdasEnv");
#else
...
#ifdef _WIN32
	  if(get_home_env() != NULL)
		sprintf(AuthFilePath, "%s/%s",get_home_env(),"srb/MdasAuth");
	  else
		  sprintf(AuthFilePath,"/srb/MdasAuth");
#else
          strcpy(buf, getenv("HOME"));
...
#ifdef _WIN32
      fd = SrbNt_fopen(AuthFilePath,"r");
#else
      fd = fopen(AuthFilePath,"r");
#endif

      if (fd == NULL) {
          printf("firstInitSrbClientEnv: Unable to open mdasAuthFile:%s\n",
            AuthFilePath);
          return (UTIL_ERROR_AUTH_FILE_OPEN);
      }
      while ((i = getNextStr(fd, buf, sizeof(buf))) != EOF) {
        if (buf[0] == '#') {
          if (i == 0) {  /* previous getNextStr did not finish a line */
...
#ifdef _WIN32
   if(get_home_env() != NULL)
       sprintf(EnvFilePath,"%s/%s.%s",get_home_env(),"srb/MdasEnv",
         SRB_NT_SESSION_EXT);
   else
        sprintf(EnvFilePath,"/srb/MdasEnv");
   fd = SrbNt_fopen(EnvFilePath,"r");
...
#ifdef _WIN32
	if(get_home_env() != NULL)
	    sprintf(EnvFilePath,"%s/%s.%s",get_home_env(),"srb/MdasEnv",SRB_NT_SESSION_EXT);
	else
	    sprintf(EnvFilePath,"srb/MdasEnv.%s",SRB_NT_SESSION_EXT);
#else
       parpid = getppid();
       myHome = getenv("HOME");
       if(myHome == NULL )
...
#ifdef _WIN32
      myHome = get_home_env();
      sprintf(AuthFilePath, "%s/%s",myHome,"srb/MdasAuth");
#else
      myHome = getenv("HOME");
      if(myHome != NULL)
         sprintf(AuthFilePath, "%s/%s",myHome,".srb/.MdasAuth");
...
mdasC_sql_result_struct *dataResult)
{
    ...
    if (likeFlag > 0) {  /* We have a like char */
        genCollLikeSQL (tmpCollName, qval[DATA_GRP_NAME], 0);
    } else {
    ...
    if ((flagval & s_FLAG) || 
     ...
     ((flagval & m_FLAG) == 0 && getenv ("srbParallel") == NULL) || 
      fileExist > 1 || copyNum >= 0) {
        status = localToSrbFileCopyS (conn, catType, localPath, 
	 targObj, targColl, datasize, flagval, fileExist, dataType,
	  newpathname, newResource, copyNum);
    } else {
        status = localToSrbFileCopyP (conn, catType, localPath,
         targObj, targColl, datasize, flagval, fileExist, dataType,
...
int copyNum)
{
    ...
    float fSec;
#ifdef _WIN32
    SYSTEMTIME startTime, sTimeLast, endTime;
#else
    struct timeval diffTime, startTime, endTime, sTimeLast, sTimeDiff;
#endif
	...
	{                           /* A file */
	  lTotalSize = (long) statbuf.st_size;
	}
    }
	    ...
	    RStatus = UTIL_ERROR_TARGET_ALREADY_EXIST;
            ...
            if ((flagval & a_FLAG) != 0) {      /* overwrite all replica */
                ...
                if (copyNum >= 0) {
                    ...
                    out_fd = srbObjOpen (conn, tmpDataName, O_WRONLY|O_TRUNC,
                     srbCollection);
		} else {
                    out_fd = srbObjOpen (conn, srbDataName, O_WRONLY|O_TRUNC,
                     srbCollection);
		}
                if (out_fd < 0) {
                    fprintf (stderr,
...
#ifdef _WIN32
    sTimeLast.wSecond = startTime.wSecond;
#else
    sTimeLast.tv_sec = startTime.tv_sec;
#endif
	...
	if ((flagval & V_FLAG)) {
	  ...
	  if (iBuffCounter < 2) {
		...
		vNiceVerboseProgressMessage (flagval, INTO_SRB, srbDataName, 
		 localPath, (long) bytesCopied,
		 (long) lTotalSize, fSec, fBs);
	    }
...
#ifdef foo
      if ((flagval & checksum_FLAG) && bytesCopied > 0) {
	if (0 != clFileSum (localPath, &ulCheckSum)) {
	    fprintf (stderr,
	     "Error: LocalToSrbFileCopyS(), local file checksum failed\n");
	    srb_perror (2, (int) bytesCopied, "",
		SRB_RCMD_ACTION | SRB_LONG_MSG);
	} else {
	    if (iWriteCheckSumIntoSrb(conn, ulCheckSum, srbDataName, 
	      srbCollection) < 0 ) {
	        fprintf (stderr,
	         "iWriteCheckSumIntoSrb(), register checksum failed\n");
	        srb_perror (2, (int) bytesCopied, "",
		  SRB_RCMD_ACTION | SRB_LONG_MSG);
	    }
	}
...
int destExist, char *datatype, char *newpathname, char *newResource)
{
    ...
    if ((flagval & checksum_FLAG) && bytesCopied > 0) {
        if (0 != clFileSum (localPath, &ulCheckSum)) {
	  fprintf (stderr,
	   "LocalToSrbFileCopyP(), local file checsum failed\n");
	  srb_perror (2, (int) bytesCopied, "",
		      SRB_RCMD_ACTION | SRB_LONG_MSG);
	} else {
	    if (iWriteCheckSumIntoSrb(conn, ulCheckSum, srbDataName, 
	     srbCollection) < 0 ) {
	      fprintf (stderr,
	       "iWriteCheckSumIntoSrb(), insert checksum into MCAT failed\n");
	      srb_perror (2, (int) bytesCopied, "",
		SRB_RCMD_ACTION | SRB_LONG_MSG);
	    }
        }
...
#ifdef foo	/* call dataToDataCopy1 instead */
        if (flagval & v_FLAG)
#ifdef _WIN32
            GetSystemTime(&startTime);
#else
...
char *srbCollection, srb_long_t size, int flagval)
{
    ...
    struct stat statbuf;
    int tmp, cnt;
    srb_long_t bytesCopied = 0, lTotalSize = 1;
    float fSec;
#ifdef _WIN32
    SYSTEMTIME startTime, sTimeLast, endTime;
#else
...
#ifdef _WIN32
        out_fd = SrbNtFileBinaryOpen(localPath, O_WRONLY|O_TRUNC);
#else
        out_fd = open (localPath, O_WRONLY|O_TRUNC, 0666);
#endif
        if (out_fd < 0) {
            fprintf (stderr,
	...
	if ((flagval & V_FLAG)) { 
	  ...
	  if (iBuffCounter < 2) {
	    ...
	    if (sTimeDiff.tv_sec >= 1) 
	      vNiceVerboseProgressMessage (flagval, INTO_SRB, srbDataName, 
		localPath, (long) bytesCopied, (long) lTotalSize, fSec, fBs);
	    iLastSec = sTimeLast.tv_sec;
#endif
	  } else {
            if (iBuffCounter > 4) iBuffCounter=0;
...
char *datatype, char *newPath, srb_long_t datasize, int existFlag, int flagval)
{
    ...
    if (existFlag == 0) {
        ...
        if ((flagval & a_FLAG) != 0) {      /* overwrite all replica */
            ...
            out_fd = srbObjOpen (conn, destObjID, O_WRONLY|O_TRUNC,
             destCollection);
        }
        if (out_fd < 0) {
            fprintf (stderr,
...
int copyNum)
{
    ...
    if (flagval & S_FLAG && !(flagval & l_FLAG) && mySrcFileOut->chksum == 0) {
...
char *cTargSize, char *cTargChksum)
{
    ...
    if (srcChksum <= 0) {
        status = srbObjChksum (conn, dataName, collection, 0, NULL, tmpChksum);
        if (status < 0) {
            fprintf (stderr, "srbObjChksum of %s/%s failed\n",
             collection, dataName);
            srb_perror (2, status, "", SRB_RCMD_ACTION|SRB_LONG_MSG);
    ...
    if (targType < 0) {   /* does not exist */
        ...
        if ((flagval & l_FLAG) == 0) {
            lstatus = dataToDataCopy1 (conn, catType, flagval, targType,
             ...
             resourceName, "", "");
#ifdef foo      /* no longer needed */
            if (lstatus >= 0) {
                if ((status = iWriteCheckSumIntoSrb(conn, srcChksum,
                 targCh, targPar)) < 0 ) {
                  fprintf (stderr,
                   "iWriteCheckSumIntoSrb(), MCAT failed.\n");
