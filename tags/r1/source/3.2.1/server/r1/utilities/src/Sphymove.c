...
int
bCollMv (srbConn *conn, int catType, mdasC_sql_result_struct *collResult,
...
char *containerName, int copyNum)
{
	...
	if (collCnt > 0 && (flagval & b_FLAG) == 0) {
	    fprintf (stderr, "Source %s is a collection. use -b option\n",
	    nameArray[i].inpArgv);
            return (CLI_ERR_COLLNAME);
        }
    ...
    for (i = 0; i < nArgv; i++) {
        ...
        if (collResult[i].row_count > 0) {      /* source is coll */
            status = bCollMv (conn, catType, &collResult[i], 
...
*/
    ...
    for (i = 0; i < collResult->row_count;i++) {
		...
		fprintf (stderr,
		 "bCollMv: srbBulkMove failed. Query of %s gave no result\n",
		  myColl);
		/* not a real error */
	    } else if (status == DATA_NOT_IN_CAT && containerName != NULL &&
	      strlen (containerName) > 0) {
		fprintf (stderr,
		 "bCollMv: srbBulkMove failed. Container %s may not exist\n",
		  containerName);
	        RStatus = myStatus = status;
	    } else {
                fprintf(stderr, "bCollMv: srbBulkMove error for %s :%i\n",
                  myColl, status);
                srb_perror (2, status, "", SRB_RCMD_ACTION|SRB_LONG_MSG);
	        RStatus = myStatus = status;
	    }
...
usage (char *prog)
{
        ...
        fprintf(stderr,
          "Usage: %s -b [-c container] [-s sourceResource] [-S targetResource] srbCollection ...\n",prog);
}
