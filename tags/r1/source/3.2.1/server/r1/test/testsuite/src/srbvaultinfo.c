...
main(int argc, char **argv)
{
	...
	const char* progname = "srbvaultinfo" ;
	int protocol ;
	...
	{
		fprintf(stderr, "srbvaultinfo: aborting on srbUserArgs error\n");
		exit(42) ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbvaultinfo : trying clConnect with\n\t host = %s\n\t port = %s\n\t auth = %s\n", host, port, auth) ;
	conn = clConnect (host, port, auth);

	/* check to see that the backend connection was successfully made */
	if (clStatus(conn) != CLI_CONNECTION_OK)
	{
		fprintf(stderr,"srbvaultinfo: Connection to srbMaster failed.\n");
		fprintf(stderr,"%s",clErrorMessage(conn));
		clFinish(conn);
		exit(1);
	}
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbvaultinfo: connected.\n") ;

	/* get info on the SRB vaults */
	vaultQueHead = srbVaultInfo(conn) ;

	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbvaultinfo: got vaultQueHead.\n") ;

	/* print out the VaultInfo link list */
	fprintf(stdout, "VaultInfo for %s :\n", host) ;
	srbPrintVaultInfo(vaultQueHead) ;

	/* free memory taken by the link list */
	srbFreeVaultInfo(vaultQueHead) ;

	/* close the connection */
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbvaultinfo: closing connection ...\n") ;
	clFinish(conn);
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbvaultinfo: finished.\n\n") ;
