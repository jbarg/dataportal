	    ...
	    char *domainName)
{
  ...
  if (!strcmp(rval[5],rval[6])) { /* new collection */
    sprintf(sqlq, "select data_grp_id from %s%s where data_grp_name = '%s'",
	  MDASSCHEMENAME,"MDAS_TD_DATA_GRP",rval[0]);
    if (check_exists(sqlq) == 0) /* it is already there */
      return(MDAS_SUCCESS);
	    ...
	    char *domainName)
{
  ...
  if (!strcmp(rval[5],rval[16])) { /* new data */
    sprintf(sqlq, "select data_id from %s%s t1, %s%s t2 where t2.data_grp_name = '%s' and  t1.data_name ='%s' and t1.repl_enum= %i and t1.version_num = '%s' and t1.data_grp_id = t2.data_grp_id",
	    MDASSCHEMENAME,"MDAS_AD_REPL",MDASSCHEMENAME,"MDAS_TD_DATA_GRP",
	    rval[6], rval[0],atoi(rval[1]),rval[22]);
    if (check_exists(sqlq) == 0) /* it is already there */
      return(MDAS_SUCCESS);
