...
#ifdef MYMALLOC
# Within elog.c here, change back the redefines of malloc back to normal
...
mymalloc(char *file,int line, int x)
{
    void *p;
        p = malloc(x);
        elog(NOTICE, "MYMALLOC: %s:%i:%i=%x",file,line,x,p);
       return(p);
}
...
{
      elog(NOTICE, "MYFREE: %s:%i=%x",file,line, p);
        free(p);
}
void *
mystrdup(char *file,int line, char *x)
{
    void *p;
        p = strdup(x);
        elog(NOTICE, "MYSTRDUP: %s:%i=%d",file,line,p);
       return(p);
}
