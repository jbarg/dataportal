 ...
 */
    ...
    if (i != 0) {
      elog (NOTICE, "db2TableOpen: Error in Extracting Options from Command %s: %i",
	    dataPathName,i);
      db2TableErrorReport(NULL, stateInfo->environ);
      close_env_db2Tab( (long int) stateInfo->environ); 
      free(stateInfo); stateInfo = NULL;
	...
	if ((tmpPtr2 = strstr(tmpPtr, " ")) == NULL &&
	  ...
	  exfd->dbSpecificInfo = (char *)stateInfo; 
	  i = getColumnInfoFordb2Table((long int)stateInfo->environ, 
	...
	*tmpPtr2 = tmpChar;
	...
	if ((tmpPtr3 = strstr(tmpPtr2, ")")) == NULL) {
	  elog (NOTICE, "db2TableOpen: Insert Statement Format Error: %s",
		dataPathName);
	...
	*tmpPtr3 = '\0';
	...
	for (i = 0; i < stateInfo->num_of_cols; i++) {
	  ...
	  stateInfo->xmlcolNameOpen[i] = NULL;
	  stateInfo->xmlcolNameClose[i] = NULL;
	  if (getColTypeFromColNamedb2Tab((long int)stateInfo->environ, 
				(long int)stateInfo->connect,
	...
	if ((tmpPtr = strstr(dataPathName," from ")) == NULL &&
	  ...
	  elog (NOTICE, "db2TableOpen: Delete Statement Format Error: %s",
		dataPathName);
	  disconnect_db2Tab( (long int)stateInfo->environ, 
			     (long int)stateInfo->connect, MDAS_CAT_ROLLBACK);
	  close_env_db2Tab( (long int) stateInfo->environ); 
	...
	*tmpPtr3 = '\0';
	...
	if (i < 0) {
	  elog (NOTICE, "db2TableOpen:Error: %i in  getMultiInsertInformation");
	  close_env_db2Tab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return(i);
	}
	if ((stateInfo->storedSQLBuffer = 
	  ...
	  elog (NOTICE, "db2TableOpen:Error: Memory Allocation Error");
	  close_env_db2Tab( (long int) stateInfo->environ); 
	  free(stateInfo); stateInfo = NULL;
	  return INP_ERR_FORMAT;
	}
	...
	return MDAS_SUCCESS;
        ...
        elog (NOTICE, "db2TableOpen:Error: Not a Known Type SQL Statement: %s",
	      dataPathName);
	close_env_db2Tab( (long int) stateInfo->environ); 
	free(stateInfo); stateInfo = NULL;
	return INP_ERR_FORMAT;
    ...
    for (j=0; j < numCols; j++) {
      ...
      myprecision = 1;
      scale = -1;
      if ((i = get_column_info_db2Tab((long int) stateInfo->statement,j+1, 
	...
	return DB_ERR_MALLOC;
      ...
      getDataTypeFromMap_db2Tab(stateInfo->coltype[j],coltype);
...
db2TableClose(dbTableDesc  *exfd)
{
  ...
  if (!strcmp(stateInfo->sqlType, "MULTIINSERT")) {
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "db2TableClose: Error in Constructing SQL To Clean List: %i",ii);
	    return(ii);
	}
	if (ii == 1) {
...
db2TableRead(dbTableDesc *exfd, char *buffer, int length) 
{
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(db2TableReadXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(db2TableReadXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(db2TableReadHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(db2TableReadHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(db2TableReadHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(db2TableReadTEMPLATE(exfd,buffer,length));
...
int 
db2TableReadTEMPLATE(dbTableDesc *exfd, char *buffer, int length)
{
  ...
  while ((i = get_row_db2Tab((long int) stateInfo->statement)) == 0)       
    {
...
int 
db2TableReadHTMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableReadHTMLNEST(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableReadHTMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableReadHEAD(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableReadXMLTREE(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableReadXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
...
int 
db2TableWrite(dbTableDesc *exfd, char *buffer, int length) 
{
/*
 ...
 */
  ...
  if (!strcmp(stateInfo->dataType, "XMLREL"))
    return(db2TableWriteXMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "XMLTREE"))
    return(db2TableWriteXMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLREL"))
    return(db2TableWriteHTMLREL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLTREE"))
    return(db2TableWriteHTMLTREE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "HTMLNEST"))
    return(db2TableWriteHTMLNEST(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "TEMPLATE"))
    return(db2TableWriteTEMPLATE(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "SQL"))
    return(db2TableWriteSQL(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "PACK"))
    return(db2TableWritePACK(exfd,buffer,length));
  else if (!strcmp(stateInfo->dataType, "VALUES"))
    return(db2TableWriteVALUES(exfd,buffer,length));
  else
    return(db2TableWriteSQL(exfd,buffer,length));
}

int 
db2TableWriteSQL(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
  tableStateInfo *stateInfo;
  int i;
  ...
  if (mybuf == NULL)  {
    elog (NOTICE, "db2TableWrite: Memory Allocation Error");
    return(DB_ERR_MALLOC);
    }
...
int 
db2TableWriteVALUES(dbTableDesc *exfd, char *sqlInsertString, int length) 
{
 tableStateInfo *stateInfo;
 int i, ii,j,k;
...
int 
db2TableWritePACK(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
db2TableWriteHTMLREL(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int
db2TableWriteHTMLTREE(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
db2TableWriteHTMLNEST(dbTableDesc *exfd, char *buffer, int length) 
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
...
int 
db2TableWriteXMLREL(dbTableDesc *exfd, char *buffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
...
int 
db2TableWriteXMLTREE(dbTableDesc *exfd, char *inbuffer, int length)
{
  tableStateInfo *stateInfo;
  int i,j,k,kk,ll, ii, jj;
  int cntData, mmSize, bufSize;
  ...
  while (1) {
    for (cntData = 0 ; cntData < stateInfo->num_of_cols ; cntData++) {
      ...
      if (i == DATA_TAG_ERROR) {
	elog (NOTICE, "db2TableWrite: Error in Data Tag for Buffer: %s", mybuf);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
	...
	*(tmpmmPtr+mmSize - 1 ) =  '\0';
	...
	elog (NOTICE, "db2TableWrite: Error in Data Tag: %s", dataTagOpen);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(DATA_TAG_ERROR);
      }
...
int 
db2TableWriteTEMPLATE(dbTableDesc *exfd, char *inbuffer, int length)
{
 ...
 if (!strcmp(stateInfo->sqlType,"MULTIINSERT")) {
  while (1) {
	...
	elog (NOTICE, "db2TableWrite: Error in Processing Choice Rule: %i", j);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	return(j);
      }
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (ii < 0 ) {
	    elog (NOTICE, 
		  "db2TableWrite: Error in Constructing SQL  in List: %i",ii);
	    if (collapsePtr != NULL) free(collapsePtr);
	    collapsePtr = NULL;
	    return(ii);
	}
	...
	for (j = 0 ;  j < MAX_SQL_TAGS_COUNT ; j++) {
	  ...
	  if (!strcmp(stateInfo->compSql.tags[i][j], dataTag)) {
	    if (stateInfo->compSql.tagusage[i][j] == TAG_FULL) {
		  ...
		  < 0) {
		elog (NOTICE, 
		   "db2TableWrite: Error in Constructing SQL '%i' in List: %i",
		      i ,ii);
		if (collapsePtr != NULL) free(collapsePtr);
		collapsePtr = NULL;
		return(ii);
  ...
  while (1) {
	...
	*(tmpmmPtr + unusedBufSize  ) =  '\0';
	...
	if (!strcmp(dataTag,stateInfo->xmlcolName[i])) 
      ...
      if (i == stateInfo->num_of_cols) {
	elog (NOTICE, "db2TableWrite: Error in Data Tag: %s", dataTag);
	if (collapsePtr != NULL) free(collapsePtr);
	collapsePtr = NULL;
	for (i = 0; i <= cntData; i++) {
	  if (stateInfo->data[i] != emptyStr)
...
executeCompoundSql(tableStateInfo *stateInfo, char *sqlbuf)
{
  ...
  if (reverseExecution(stateInfo)) {
	...
	break;
      myChar = *mybuf2;
      *mybuf2 = '\0';
      if ((i=exec_no_res_sql_db2Tab(environ, connect,
