...
main(int argc, char **argv)
{
	...
	{
		fprintf(stderr, "srbomove: aborting on srbUserArgs error\n");
		exit(42) ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbomove : trying clConnect with\n\t host = %s\n\t port = %s\n\t auth = %s\n", host, port, auth) ;
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbomove: connected.\n\n") ;


	/* move */
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr,"srbomove: Attempting to call srbObjMove with\n\t catType = %d\n\t objID = %s\n\t collectionName = %s\n\t origPathName = %s\n\t origResourceName = %s\n\t newPathName = %s\n\t newResourceName = %s\n", catType, objID, coll, origPathName, origResourceName, newPathName, newResourceName );

	status = srbObjMove(conn, catType, objID, NULL, coll, newResourceName, newPathName, NULL) ;
	if (status < 0)
	{
		fprintf(stderr,"srbomove: Unable to move object \"%s\"\n", objID );
		fprintf(stderr,"%s",clErrorMessage(conn));
		clFinish(conn);
		exit (1);
	}
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr,"srbomove: Copied object \"%s\"\n", objID );


	/* close the connection */
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbomove: closing connection ...\n") ;
	clFinish(conn);
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbomove: finished.\n\n") ;
