...
/*-------------------------------------------------------------------------
 *
 * srbtabodel.c-- truncate table and unregister srbObject
 *
 * See README file or execute with -help flag.
 *
 *-------------------------------------------------------------------------
 */
/** srbtabodel -host $HOST -port $PORT -auth $AUTH $VERBOSE -id $TESTTABIDTRUNCATE -coll $PARENTCOLL/$TESTCOLL
**/
...
{
	srbConn *conn;
	const char* progname = "srbtabodel" ;
	int protocol ;
	...
	{
		fprintf(stderr, "srbtabodel: aborting on srbUserArgs error\n");
		exit(42) ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtabodel : trying clConnect with\n\t host = %s\n\t port = %s\n\t auth = %s\n", host, port, auth) ;
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtabodel: connected.\n\n") ;
	...
	for (n=0 ; n<nobjs; n++)
	{
		...
		if (fd < 0)
		{
			fprintf(stderr,"srbtabodel: Unable to open object (fd = %d)\n", fd);
			fprintf(stderr,"%s",clErrorMessage(conn));
			clFinish(conn);
			exit (1);
		}
		if (srbargc[SRB_USER_ARG_VERB] != 0)
		  fprintf(stderr,"srbtabodel: open'ed and truncated object with fd = %d\n", fd);
		srbObjClose (conn, fd);
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr, "srbtabodel: truncation object closed successfully.\n\n") ;




		/* unregister external store */
		if ((tmpStr = strstr(externstore,"&")) != NULL)
		  *tmpStr = '\0';
		status = srbUnregisterDataset(conn, externstore, coll);
		if (status < 0)
		{
			fprintf(stderr,"srbtabodel: Unable to unregister object \"%s\" of collection \"%s\" (status = %i)\n", externstore, coll, status );
			fprintf(stderr,"%s",clErrorMessage(conn));
			clFinish(conn);
			exit (1);
		}
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr,"srbtabodel: Unregistered object \"%s\" of collection \"%s\"\n", externstore, coll );
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtabodel: closing connection ...\n") ;
	clFinish(conn);
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtabodel: finished.\n\n") ;
