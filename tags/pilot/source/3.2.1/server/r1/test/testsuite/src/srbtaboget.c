...
{
	srbConn *conn;
	const char* progname = "srbtaboget" ;
	int protocol ;
...
for (k =0; k < argc ; k++)
  printf("argv[%d] =%s**\n",k,argv[k]);
	...
	srbargc[SRB_USER_ARG_ID] = 1 ; /* used to be SRB_ARG_REQUIRED*/
	...
	{
		fprintf(stderr, "srbtaboget: aborting on srbUserArgs error\n");
		exit(42) ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr,"srbtaboget: bufsize = %d\n", bufsize);
	buf = (char*) malloc(sizeof(char)*bufsize) ;
	if (buf == (char*) NULL)
	{
		fprintf(stderr,"srbtaboget: buf malloc failed.\n");
		exit(1);
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtaboget : trying clConnect with\n\t host = %s\n\t port = %s\n\t auth = %s\n", host, port, auth) ;
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtaboget: connected.\n\n") ;
	...
	for (n=0 ; n<nfiles; n++)
	{
		...
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr, "srbtaboget: open'ed \"%s\" for write\n", localfile);
		...
		if (result_count != 1)
		{
			fprintf(stderr,"srbtaboget: result_count (%d) from srbGetDataDirInfo != 1\n", status);
			clFinish(conn);
			exit (1);
		}
		filebytes = atoi((char *) myresult.sqlresult[0].values) ;
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr,"srbtaboget: object \"%s\" is size %d\n", objID, filebytes);

		/* get external store */
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr,"srbtaboget: Attempting to call srbObjOpen with\n\t objID = %s\n\t collectionName = %s\n", objID, coll );
		fd = srbObjOpen(conn, objID, O_RDONLY, coll);
		if (fd < 0)
		{
			fprintf(stderr,"srbtaboget: Unable to open object (fd = %d)\n", fd);
			fprintf(stderr,"%s",clErrorMessage(conn));
			clFinish(conn);
			exit (1);
		}
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr,"srbtaboget: open'ed object with fd = %d\n", fd);

		/* get data */

		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr, "srbtaboget: reading data ...\n") ;
		rembytes = filebytes ;
		if (rembytes <= 0)
		  rembytes = 9999999;
		while (rembytes > 0)
		{
			...
			if (srbrstat < 0)
			{
				fprintf(stderr, "srbtaboget : ? srbObjRead failed for \"%s\" (srbrstat = %d)\n", objID, srbrstat) ;
				fprintf(stderr,"%s",clErrorMessage(conn));
				fclose(fp) ;
				srbObjClose (conn, fd);
				clFinish(conn);
			...
			if (srbrstat == 0) 
			  break;
			if (srbargc[SRB_USER_ARG_VERB] != 0)
				fprintf(stderr, "srbtaboget: read %d bytes\n", srbrstat) ;
			bufbytes = srbrstat ;
			...
			if ((wstat != 1) || (status != 0))
			{
				fprintf(stderr, "srbtaboget : ? fwrite failed for \"%s\" (wstat = %d, ferror = %d)\n", localfile, wstat, status) ;
				fclose(fp) ;
				srbObjClose (conn, fd);
				clFinish(conn);
				exit(1) ;
			...
			if (srbargc[SRB_USER_ARG_VERB] != 0)
				fprintf(stderr, "srbtaboget: wrote %d bytes\n", wstat*bufbytes) ;
		...
		if (srbargc[SRB_USER_ARG_VERB] != 0)
			fprintf(stderr, "srbtaboget: I/O completed successfully.\n\n") ;
	}
	...
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtaboget: closing connection ...\n") ;
	clFinish(conn);
	if (srbargc[SRB_USER_ARG_VERB] != 0)
		fprintf(stderr, "srbtaboget: finished.\n\n") ;
