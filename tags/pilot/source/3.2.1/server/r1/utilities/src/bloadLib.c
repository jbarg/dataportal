...
#define MAX_ACT_REG_THREAD 4
#else
#define MAX_NUM_BL_THR 2
#define MAX_ACT_REG_THREAD 1
#endif
#define MAX_RETRY_SEC 600
#define MAX_MILI_SECS 200
...
srbPathName *targName, int flagval)
{
	...
	/* For container, take out k_FLAG and K_FLAG for now */
	/* flagval = flagval ^ (k_FLAG | K_FLAG); */
        ...
        if (myFd < 0)  {
            fprintf (stderr, "bloadMain: Failure opening SRB container %s. \n",
             ContainerName);
        ...
        if (curOffset < 0) {
            fprintf (stderr, 
	     "bloadMain: Failure seeking in SRB container %s :%lld \n",
...
#else /* PARA_OPR */
	fprintf (stderr, 
	 "Cannot bulk load container when compiled in non-parallel mode\n");
	return -1;
#endif	/* PARA_OPR */
    } else {
	if (checkCollection (conn, MDAS_CATALOG, targName->outArgv) != 0) {
	    ...
	    strcpy (TmpBloadColl, targName->outArgv);
	}
	genBloadFileName ();        /* generate a new bload file */
	myFd = srbObjCreate (conn, MDAS_CATALOG, TmpBloadFile, NULL,
	 ResourceName, TmpBloadColl, NULL, -1);
        if (myFd < 0)  {
            fprintf (stderr, 
	      "bloadMain: unable to create of temp bload file %s/%s.\n",
    ...
    if (TranThrInx < 0) {
      printf ("Sbload.c:bloadMain(), Run out of maximum allowed pthreads, TranThrInx=%d\n", TranThrInx);
      return(-3);
    }
    ...
    if (flagval & c_FLAG)          /* uses container */
        srbObjClose (conn, myFd);
	...
	if (toread > space) {
	    if (myBufInfo->offset[curInx] > 0) { 
		    ...
		    float fSec;
        	    ...
        	    fSec = (float)STimetd.tv_sec + 
		     ((float)STimetd.tv_usec / 1000000.);
#endif
        	     printf ("time to read %d bytes from local disk = %f\n", 
		      myBufInfo->offset[curInx], fSec);
#ifdef _WIN32
                GetSystemTime(&STimestart);
#else
      		    (void) gettimeofday(&STimestart, (struct timezone *)0);
	...
	*retval = RStatus;
    ...
    while (connectCnt < MAX_CONN_CNT) {
        ...
        if (clStatus(conn) != CLI_CONNECTION_OK) {
            fprintf(stderr,
	     "regResult: Connection to srbMaster failed. Retrying\n");
	...
	fprintf(stderr,
          "regResult: Connect succeeded after retrying %d times.\n",
	  connectCnt);
    }
...
regAndBload (void *myInput)
{
    ...
    conn = srbConnect (myHost, NULL, srbAuth,
     NULL, NULL, NULL, NULL);
    if (clStatus(conn) != CLI_CONNECTION_OK) {
        fprintf(stderr,"regAndBload: Connection to srbMaster failed.\n");
        fprintf(stderr,"regAndBload: %s",clErrorMessage(conn));
        srb_perror (2, clStatus(conn), "", SRB_RCMD_ACTION|SRB_LONG_MSG);
        *retval = -1;
        return ((void *) retval);
    }
...
allocThrInx ()
{
    ...
    while ((numThreads = getNumActThr ())  > MAX_ACT_REG_THREAD) {
      srbThreadSleep (0, (int)(MAX_MILI_SECS * 1000)); 
      /* too many running pthreads, wait for some to finish */
      iTotalErr++;
      if (iTotalErr > (int)(MAX_RETRY_SEC / MAX_MILI_SECS * 1000)) {
        fprintf (stderr,
...
int flagval)
{
    ...
    if (RegThrInx < 0) {
      printf (
       "Sbload.c:procOverFContainer(), Run out of maximum allowed pthreads, RegThrInx=%d\n", RegThrInx);
      return(-7);
    }
