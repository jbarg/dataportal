...
usage()
{
  fprintf(stderr,"SgetR [-alchdDpP] [-z mcatName] [-L n] [-Y n] [-T resourcType] [resourceName] \n");
}
int
main(int argc, char **argv)
{
    ...
    while ((c=getopt(argc, argv,"alchdDpPL:Y:T:H:z:")) != EOF) {
        switch (c) {
	     case 'p':
			selval[RSRC_NAME] = 0;
...
#ifdef FED_MCAT
    i = queryAndPrintAcrossZones(conn, MDAS_CATALOG, mcatName, qval, selval,
				 &myresult,L_val);
    if (i < 0) {
