...
main(int argc, char **argv)
{
    ...
    hasPrinted = 0;
    strcpy(inCondition , "");
    while ((c=getopt(argc, argv,"hardpcDCbi:")) != EOF) {
        switch (c) {
            case 'h':
                usage (argv[0]);
		exit (1);
    ...
    if (IFlag == 0 && argc < optind + 4) {		/* must have at least 4 inputs */
	fprintf(stderr,"Schmod: Must have at least 4 input values\n");
	usage (argv[0]);
	exit (1);
    }
    else if (IFlag == 1 && argc < optind + 2) {              /* must have at least 2 inputs */
        usage (argv[0]);
        exit (1);
    }
	...
	if (strcmp (mode, "set") != 0 && strcmp (mode, "unset") != 0) {
	    fprintf(stderr,"Schmod: mode must be 's' or 'u' for -i option\n");  
            usage (argv[0]);
            exit (1);
	}
    }
	...
	for (i = 0; i < nArgv; i++) {
	  if (curatorFlag == 0) {
		...
		status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
					   "1","","",C_CHANGE_ACL_INHERITANCE_BIT_RECUR);
	      else
		status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "0","","",C_CHANGE_ACL_INHERITANCE_BIT_RECUR);
	    }
	    ...
	    if (IFlag == 1 ) {
              if (!strcmp(mode,"set"))
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "1","","",C_CHANGE_ACL_INHERITANCE_BIT_RECUR_BY_CURATOR);
              else
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "0","","",C_CHANGE_ACL_INHERITANCE_BIT_RECUR_BY_CURATOR);
            }
            else if (!strcmp(mode,"owner"))
	      status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
	...
	for (i = 0; i < nArgv; i++) {
	  if (curatorFlag == 0) {
	    if (IFlag == 1 ) {
              if (!strcmp(mode,"set"))
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "1","","",C_CHANGE_ACL_INHERITANCE_BIT);
	      else
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "0","","",C_CHANGE_ACL_INHERITANCE_BIT);
	    }
	    else if (!strcmp(mode,"owner"))
	      status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
	    ...
	    if (IFlag == 1 ) {
              if (!strcmp(mode,"set"))
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "1","","",C_CHANGE_ACL_INHERITANCE_BIT_BY_CURATOR);
              else
                status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
                                           "0","","",C_CHANGE_ACL_INHERITANCE_BIT_BY_CURATOR);
            }
            else if (!strcmp(mode,"owner"))
	      status = srbModifyCollect (conn, MDAS_CATALOG, nameArray[i].outArgv,
...
void
 usage (char *prog)
{
    fprintf (stderr, "Usage : %s [-h] [-b] [-p] [-a] [-r] [-d|c|D] [-C] w|a|r|n|t|o|c newUserName domainName collection|srbObj ....\n", prog);
    fprintf (stderr, "    w - write;r - read;a - all permission;n - null; t - annotate; c - curate (collection); o - change owner\n");
    fprintf (stderr, "Usage : %s -i [-r] s|u collection ....\n", prog);
    fprintf (stderr, "    s - set inheritance bit for ACL;  r - reset inheritance bit\n");
